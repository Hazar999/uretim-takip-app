...
def _abkant_completed_qty_from_edge_teams(
    ...
    """Uzun ve kısa kenar takım sayılarına göre tamamlanan paneli hesaplar.

    Her takım, ilgili kenar grubunda bir paneli temsil eder. Yalnızca projede
    gerekli olan kenarlar hesaba katılır. İki kenar da gerekli olduğunda panel,
    ancak iki kenar takımı da hazırsa tamamlanmış sayılır; bu nedenle düşük olan
    takım sayısı esas alınır.
    """
    required_totals = []
    normalized_long = _normalize_abkant_bend_requirement(long_requirement)
    normalized_short = _normalize_abkant_bend_requirement(short_requirement)
    ...
    if normalized_long in {"Tek büküm", "Çift büküm"}:
        required_totals.append(max(int(total_long_team_qty or 0), 0))
    if normalized_short in {"Tek büküm", "Çift büküm"}:
        required_totals.append(max(int(total_short_team_qty or 0), 0))

    if not required_totals:
        return 0
    return min(min(required_totals), max(int(requested_qty or 0), 0))
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                if operation_kind == "abkant":
                    ...
                    abkant_teams_per_piece = sum(
                        requirement in {"Tek büküm", "Çift büküm"}
                        for requirement in (
                            abkant_long_bend_type,
                            abkant_short_bend_type,
                        )
                ...
                if processed > 0 or abkant_activity:
                    entries.append({
                        ...
                        "abkant_team_excess": (
                            ...
                            if operation_kind == "abkant"
                            and abkant_long_bend_type != "Yok"
                            and abkant_short_bend_type != "Yok"
                            else 0
                        ),
...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            ...
                            st.caption(
                                "Uzun ve kısa kenar takım sayılarını ayrı girin. İki kenar da "
                                "isteniyorsa sağlam ilerleyen adet düşük takım sayısına göre hesaplanır."
                            )
                            ...
                            with team3:
                                st.number_input(
                                    ...
                                    step=1,
                                    key=fire_key,
                                    help="Kenar takımları tamamlandıktan sonra fire çıkan panel adedi.",
                                )
                            with team4:
                                st.number_input(
                                    "Süre (saat)",
                            ...
                            long_needed_for_remaining = (
                                max(requested_qty - previous_long_team_qty, 0)
                                if long_edge_required
                                else 0
                            )
                            short_needed_for_remaining = (
                                max(requested_qty - previous_short_team_qty, 0)
                            ...
                            excess_text = ""
                            if long_edge_required and short_edge_required:
                                if total_long_after > total_short_after:
                                    excess_text = (
                                        f" Artan: uzun kenarda "
                                        f"{total_long_after - total_short_after} takım."
                                    )
                                elif total_short_after > total_long_after:
                                    excess_text = (
                                        f" Artan: kısa kenarda "
                                        f"{total_short_after - total_long_after} takım."
                                    )
                                else:
                                    excess_text = " Artan takım yok."

                            st.info(
                                f"**Hesap:** {', '.join(calculation_parts)} → bu kayıtta "

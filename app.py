...
def _laser_required_edges(edge_mode: str) -> tuple[bool, bool]:
    """Seçilen kapsama göre uzun ve kısa kenarın gerekli olup olmadığını döndürür."""
...
def _laser_completed_qty_from_edges(
    ...
    """Günler boyunca biriken lazer kenarlarından tamamlanan panel/takım adedini bulur.

    İki kenar da gerekiyorsa düşük olan taraf tamamlanan miktarı belirler. Yalnızca
    bir kenar gerekiyorsa sadece o kenar dikkate alınır. Böylece bugün yalnız kısa
    kenar, ertesi gün uzun kenar kesildiğinde iki günün toplamı birlikte hesaplanır.
    """
    ...
    try:
        ...
        for entry in entries:
            ...
            if operation_kind == "laser":
                ...
                previous_long_edge_qty, previous_short_edge_qty, previous_processed = (
                    cur.fetchone()
                )
                cur.execute(
            ...
            cur.execute(
                ...
                (
                    ...
                    (
                        ...
                        if operation_kind in {"abkant", "laser"}
                ...
                for operation in selected_ops:
                    ...
                    if operation_kind == "laser":
                        ...
                        st.caption(
                            ...
                            "POS iki kenarı da gerektiriyor olsa bile bugün yalnızca bir kenarı "
                            "girebilirsin; diğer kenar sonraki gün kaydedildiğinde sistem toplamdan "
                            "tamamlanan takımı otomatik bulur."
                        )
                        ...
                        st.selectbox(
                            "Bu POS için gereken lazer kenarları",
                            LASER_EDGE_MODE_OPTIONS,
                            key=edge_mode_key,
                            help=(
                                "İki kenar da gerekiyorsa Uzun + kısa kenar seç. Usta bugün "
                                "yalnızca bir kenarı kestiyse diğer kenarı 0 bırakabilirsin."
                            ),
                        )
                        ...
                        for lot_index in range(1, lot_count + 1):
                            ...
                            long_used = (
                                cumulative_completed_teams * long_per_team
                                if long_required
                                else 0
                            )
                            short_used = (
                                cumulative_completed_teams * short_per_team
                                if short_required
                                else 0
                            )
                            ...
                            required_labels = []
                            if long_required:
                                required_labels.append("uzun")
                            if short_required:
                                required_labels.append("kısa")
                            st.info(
                                f"**Kenar kapsamı:** {' + '.join(required_labels)} · "
                                f"**Biriken fazla/bekleyen:** Uzun {long_excess} · Kısa {short_excess}"
                            )
                            if selected_edge_mode == "Uzun + kısa kenar" and (
                                (long_qty > 0 and short_qty == 0)
                                or (short_qty > 0 and long_qty == 0)
                            ):
                                st.warning(
                                    "Bugün yalnızca bir kenar girildi. Kayıt kaydedilecek; "
                                    "diğer kenar sonraki gün tamamlandığında panel adedi ilerleyecek."
                                )
                            st.markdown(
                                f"""**1 panel alanı:**  
{_format_mm2(item['boy_mm'])} × {_format_mm2(item['en_mm'])} ÷ 1.000.000 = **{unit_area_m2:.2f} m²**

**Bugün tamamlanan takım:** {completed_teams}  
**Kenar toplamından oluşan kümülatif takım:** {cumulative_completed_teams}

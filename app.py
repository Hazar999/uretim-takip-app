...
# -----------------------------
# TEK SEFERLİK NEO VAC BOYA GEÇMİŞ KAYDI
# -----------------------------
NEO_VAC_BOYA_BULK_MARKER = "neo_vac_boya_20260722_team_v1"
NEO_VAC_BOYA_DATE = "2026-07-22"
NEO_VAC_BOYA_SEGMENTS = [
    {
        "key": "ilk_9_saat",
        "hours": 9.0,
        "operator": "Nezir Azim",
        "participants": ["Yaşettin Baysal", "Salih Kayar", "Köksal Atıl"],
        "label": "İlk 9 saat",
    },
    {
        "key": "son_3_saat",
        "hours": 3.0,
        "operator": "Nezir Azim",
        "participants": ["Yaşettin Baysal", "Şükrü Atıl", "Köksal Atıl"],
        "label": "Son 3 saat",
    },
]


def _boya_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Bir ekip diliminin toplam saatini adetlere orantılı dağıtır."""
    if not weights:
        return []
...
def _boya_dedupe_operations(operations: list[str]) -> list[str]:
    """Operasyon sırasını korur ve aynı operasyonu anlamca tekilleştirir."""
    result = []
    seen = set()
...
def _is_boya_operation(operation_name: str) -> bool:
    return _operation_kind(operation_name) == "boya"


def _insert_boya_into_plan(
    plan_operations: list[str],
    boya_operation: str,
) -> list[str]:
    """Boya yoksa paketleme/sevkiyattan hemen önce plana ekler."""
    operations = _boya_dedupe_operations(plan_operations)
    if any(_is_boya_operation(name) for name in operations):
        return operations

    insert_at = len(operations)
    for index, operation_name in enumerate(operations):
        if _operation_kind(operation_name) in {"paketleme", "sevkiyat"}:
            insert_at = index
            break
    operations.insert(insert_at, str(boya_operation or "boya"))
    return _boya_dedupe_operations(operations)


def _neo_vac_boya_segment_marker(segment_key: str) -> str:
    return f"{NEO_VAC_BOYA_BULK_MARKER}:{segment_key}"


def _neo_vac_boya_existing_batch(segment_key: str) -> int | None:
    marker = _neo_vac_boya_segment_marker(segment_key)
    conn = get_db_connection()
    try:
        row = conn.execute(
            """
...
def _neo_vac_boya_bulk_already_saved() -> tuple[bool, str]:
    """Aynı iki ekip diliminin ikinci kez kaydedilmesini engeller."""
    conn = get_db_connection()
    try:
        setting_row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_BOYA_BULK_MARKER,),
        ).fetchone()
        if setting_row is not None:
            return True, str(setting_row[0] or "")
    finally:
        conn.close()

    existing_ids = [
        _neo_vac_boya_existing_batch(spec["key"])
        for spec in NEO_VAC_BOYA_SEGMENTS
    ]
    if all(batch_id is not None for batch_id in existing_ids):
        return True, ", ".join(str(batch_id) for batch_id in existing_ids)
    return False, ""


def _get_neo_vac_boya_items() -> pd.DataFrame:
    """Neo Vac POS'larının boya planını ve mevcut boya ilerlemesini getirir."""
    conn = get_db_connection()
    try:
        ...
        for _, item in items.iterrows():
            ...
            boya_logs = [
                ...
                for operation_name, good_qty, _ in log_rows
                if _is_boya_operation(str(operation_name))
            ]

            boya_operation = next(
                (
                    name for name in plan_operations
                    if _is_boya_operation(name)
                ),
                "",
            )
            if not boya_operation and boya_logs:
                boya_operation = boya_logs[-1][0]
            if not boya_operation:
                boya_operation = "boya"

            effective_plan = plan_operations or log_operations
            effective_plan = _insert_boya_into_plan(
                effective_plan,
                boya_operation,
            )
            ...
            row["boya_operation"] = boya_operation
            row["existing_boya_good"] = sum(value for _, value in boya_logs)
...
def _split_boya_quantities(
    rows: list[dict],
    active_specs: list[dict],
) -> dict[str, dict[int, int]]:
    """Kalan boya adetlerini 9/3 saat oranına göre iki ekibe dağıtır."""
    result = {str(spec["key"]): {} for spec in active_specs}
    if not active_specs:
        return result
    if len(active_specs) == 1:
        key = str(active_specs[0]["key"])
        for row in rows:
            result[key][int(row["item_id"])] = int(row["remaining_qty"])
        return result

    first, second = active_specs[0], active_specs[1]
    first_key = str(first["key"])
    second_key = str(second["key"])
    total_hours = float(first["hours"]) + float(second["hours"])
    first_ratio = float(first["hours"]) / total_hours if total_hours else 0.5

    total_qty = sum(max(int(row["remaining_qty"]), 0) for row in rows)
    first_target = int(round(total_qty * first_ratio))

    fractions = []
    first_total = 0
    for row_index, row in enumerate(rows):
        item_id = int(row["item_id"])
        quantity = max(int(row["remaining_qty"]), 0)
        ideal_first = quantity * first_ratio
...
def _build_neo_vac_boya_bulk_plan(selected_oc: str) -> dict:
    """22 Temmuz Neo Vac boya kaydını iki ekip dilimi halinde hazırlar."""
    items = _get_neo_vac_boya_items()
    if items.empty:
    ...
    for item in items.itertuples(index=False):
        ...
        completed_before = max(int(item.existing_boya_good or 0), 0)
        remaining_qty = max(requested_qty - completed_before, 0)
        if remaining_qty <= 0:
            continue

        plan_operations = [str(value) for value in (item.plan_operations or [])]
        boya_operation = str(item.boya_operation or "boya")
        combination_name = str(item.combination_name or "").strip()
        if not combination_name:
            combination_name = combination_name_from_operations(plan_operations)

        if bool(item.plan_was_missing):
            warnings.append(
                f"{item.pos}: kayıtlı operasyon planı bulunmadığı için mevcut "
                "işlem geçmişinden plan oluşturuldu."
            )
        ...
        rows.append({
            ...
            "operation_name": boya_operation,
            "plan_operations": plan_operations,
            "combination_name": combination_name,
        })

    existing_batches = {
        str(spec["key"]): _neo_vac_boya_existing_batch(str(spec["key"]))
        for spec in NEO_VAC_BOYA_SEGMENTS
    }
    active_specs = [
        spec for spec in NEO_VAC_BOYA_SEGMENTS
        if existing_batches[str(spec["key"])] is None
    ]
    allocations = _split_boya_quantities(rows, active_specs)
    row_lookup = {int(row["item_id"]): row for row in rows}

    segment_plans = []
    for spec in NEO_VAC_BOYA_SEGMENTS:
        key = str(spec["key"])
        existing_batch = existing_batches[key]
        if existing_batch is not None:
            segment_plans.append({
                **spec,
                "entries": [],
                "plans": {},
                "existing_batch": int(existing_batch),
            })
            continue
        ...
        quantities = [quantity for _, quantity in segment_rows]
        hour_allocations = _boya_allocate_hours(
            float(spec["hours"]),
            quantities,
        )
        ...
        for (row, quantity), operation_hours in zip(
            ...
            entries.append({
                ...
                "remaining_before": int(row["remaining_qty"]),
            ...
            plans[int(row["item_id"])] = {
                ...
                "operations": _boya_dedupe_operations(
...
def _save_neo_vac_boya_bulk_plan(plan: dict) -> list[int]:
    """İki ekip dilimini kaydeder; hata olursa bu denemedeki kayıtları geri alır."""
    if not plan or not plan.get("segments"):
        raise ValueError("Kaydedilecek Neo Vac boya kaydı bulunamadı.")

    already_saved, saved_value = _neo_vac_boya_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu Neo Vac toplu boya kaydı daha önce uygulandı. "
            f"Kayıt numaraları: {saved_value or '-'}"
        )
    ...
    try:
        for segment in plan["segments"]:
            existing_batch = segment.get("existing_batch")
            if existing_batch is not None:
                batch_ids.append(int(existing_batch))
                continue

            entries = list(segment.get("entries") or [])
            if not entries:
                raise ValueError(
                    f"{segment['label']} için dağıtılacak boya adedi bulunamadı."
                )

            marker = _neo_vac_boya_segment_marker(str(segment["key"]))
            team_text = _join_participants([
                segment["operator"],
                *segment["participants"],
            ])
            note = (
                f"{marker} | Neo Vac bütün POS'lar toplu boya geçmiş kaydı | "
                f"Ekip: {team_text}"
            )
            batch_id = save_operation_batch(
                {
                    "tarih": NEO_VAC_BOYA_DATE,
                    "operator_ismi": str(segment["operator"]),
                    "calisma_tipi": "Tam zamanlı",
            ...
            newly_created.append(int(batch_id))

        if len(batch_ids) != len(NEO_VAC_BOYA_SEGMENTS):
            raise ValueError("12 saatlik boya kaydının iki ekip dilimi de oluşturulamadı.")
        ...
        for batch_id in reversed(newly_created):
            try:
...
def render_neo_vac_bulk_boya_tool():
    """Neo Vac boya geçmişini iki ekip dilimiyle tek seferde kaydeder."""
    st.markdown("**Neo Vac toplu boya kaydı**")
    st.caption(
        "22/07/2026 · Toplam 12 saat · İlk 9 saat: Nezir Azim, "
        "Yaşettin Baysal, Salih Kayar ve Köksal Atıl · Son 3 saat: "
        "Salih Kayar yerine Şükrü Atıl."
    )

    already_saved, batch_value = _neo_vac_boya_bulk_already_saved()
    if already_saved:
        st.success(
            "Bu toplu boya kaydı daha önce uygulandı; ikinci kez eklenmeyecek."
        )
        if batch_value:
            st.caption(f"Oluşturulan kayıt numaraları: {batch_value}")
        return
    ...
    label_to_oc = {
        ...
        for label, row in zip(
            labels,
            project_options.itertuples(index=False),
        )
    }
    if len(labels) == 1:
        ...
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_boya_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_boya_bulk_plan(selected_oc)
    for warning in plan.get("warnings", []):
        st.warning(warning)

    preview_rows = []
    for segment in plan.get("segments", []):
        if segment.get("existing_batch") is not None:
            st.info(
                f"{segment['label']} daha önce oluşturulmuş: "
                f"kayıt no {segment['existing_batch']}."
            )
        team_text = _join_participants([
            segment["operator"],
            *segment["participants"],
        ...
        for entry in segment.get("entries", []):
            preview_rows.append({
                ...
                "Ekip": team_text,
                "POS": entry["pos"],
                "Boya Adedi": int(entry["good_qty"]),
                "Saat": float(entry["operation_hours"]),
            })

    if not preview_rows:
        st.success("Neo Vac bütün POS'ların boya adetleri zaten tamamlanmış.")
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Ekip dilimi", int(preview["Çalışma Dilimi"].nunique()))
    m2.metric("POS satırı", int(len(preview)))
    m3.metric("Toplam boya adedi", int(preview["Boya Adedi"].sum()))
    m4.metric("Toplam saat", f"{preview['Saat'].sum():.1f}")
    st.dataframe(preview, use_container_width=True, hide_index=True)

    segment_summary = (
        preview.groupby(["Çalışma Dilimi", "Ekip"], as_index=False)
        .agg(
            POS=("POS", "nunique"),
            Adet=("Boya Adedi", "sum"),
            Saat=("Saat", "sum"),
    ...
    st.markdown("**Ekip ve saat dağılımı**")
    st.dataframe(segment_summary, use_container_width=True, hide_index=True)
    st.caption(
        "Kalan boya adetleri 9 saat / 3 saat oranında dağıtılır. "
        "İlk ekipte Salih Kayar, son 3 saatte Şükrü Atıl yer alır. "
        "Fire 0 olarak kaydedilir."
    )

    confirm = st.checkbox(
        "Bu 12 saatlik boya kaydını kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_boya_bulk_confirm",
    )
    if st.button(
        "Neo Vac Boya Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="neo_vac_boya_bulk_save_button",
    ):
        try:
            batch_ids = _save_neo_vac_boya_bulk_plan(plan)
            st.session_state["neo_vac_boya_bulk_success"] = (
                "Neo Vac toplu boya kayıtları tamamlandı. Kayıt numaraları: "
                + ", ".join(map(str, batch_ids))
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu boya kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu boya kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_boya",
                help="22/07/2026 tarihli 12 saatlik Neo Vac boya geçmişini iki ekip dilimiyle hazırlar.",
            ):
                success_message = st.session_state.pop(
                    "neo_vac_boya_bulk_success",
                    None,
                )
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_boya_tool()

...
def _operation_kind(operation_name: str) -> str:
    ...
    if "laser" in normalized or "lazer" in normalized:
        return "laser"
...
def main():
    ...
    st.caption("Sürüm 2.13.2 GERÇEK DURLUM LOGOSU · İşçi, OC, POS, işlem, adet, fire ve yetkinlik takibi")
    if is_manager:
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.13.2 · GERÇEK DURLUM LOGOSU
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def yeni_kayit_page(reasons, combinations, is_manager: bool):
    ...
    def build_entries(selected_items: pd.DataFrame, plans: dict, selected_map: dict):
        ...
        saved_entry_map = {
            (int(saved_entry["item_id"]), str(saved_entry["operation_name"])): saved_entry
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                plate_key = f"laserplates_{item_id}_{normalized_operation}_{version}"
                material_key = f"material_{item_id}_{normalized_operation}_{version}"
                thickness_key = f"thickness_{item_id}_{normalized_operation}_{version}"
                ...
                processed = int(st.session_state.get(processed_key, saved_entry.get("processed_qty", 0)))
                fire = int(st.session_state.get(fire_key, saved_entry.get("fire_qty", 0)))
                operation_hours = float(st.session_state.get(hours_key, saved_entry.get("operation_hours", 0.0)))
                ...
                if processed > 0:
                    entries.append({
                        ...
                        "laser_plate_qty": int(st.session_state.get(plate_key, saved_entry.get("laser_plate_qty", 0))) if operation_kind == "laser" else 0,
                        "material_type": str(st.session_state.get(material_key, saved_entry.get("material_type", ""))) if operation_kind == "laser" else "",
                        "thickness_mm": float(st.session_state.get(thickness_key, saved_entry.get("thickness_mm", 0.0))) if operation_kind == "laser" else 0.0,
                        "abkant_work_mode": str(st.session_state.get(work_mode_key, saved_entry.get("abkant_work_mode", ""))) if operation_kind == "abkant" else "",
                        "abkant_coworker": (
                            str(st.session_state.get(coworker_key, saved_entry.get("abkant_coworker", "")))
                            if operation_kind == "abkant"
                            and str(st.session_state.get(work_mode_key, saved_entry.get("abkant_work_mode", ""))) == "Biriyle beraber çalıştı"
                            else ""
                        ),
                        "bend_type": str(st.session_state.get(bend_key, saved_entry.get("bend_type", ""))) if operation_kind == "abkant" else "",
                        "calculated_area_mm2": max(processed - fire, 0) * float(item["unit_area_mm2"]),
                    })
        return entries
        ...
        for _, item in selected_items.sort_values("_sort_pos").iterrows():
            ...
            with st.expander(
                ...
                for operation in selected_ops:
                    ...
                    saved_entry = saved_entry_lookup.get((item_id, operation), {})
                    defaults_for_widgets = {
                        ...
                        plate_key: int(saved_entry.get("laser_plate_qty", 0)),
                        material_key: str(saved_entry.get("material_type", "Çelik") or "Çelik"),
                        thickness_key: float(saved_entry.get("thickness_mm", 0.0)),
                    ...
                    row1, row2, row3, row4, row5 = st.columns([2.0, 1.0, 0.9, 1.0, 1.0])
                    with row1:
                    ...
                    with row2:
                        st.number_input("İşlem yapılan", min_value=0, step=1, key=processed_key)
                    with row3:
                        st.number_input("Fire", min_value=0, step=1, key=fire_key)
                    with row4:
                        st.number_input(
                            "Süre (saat)",
                            min_value=0.0,
                    ...
                    if operation_kind == "laser":
                        laser1, laser2, laser3 = st.columns(3)
                        with laser1:
                            st.number_input(
                                "Yapılan plaka adedi",
                                min_value=0,
                    ...
                    current_processed = int(st.session_state.get(processed_key, 0) or 0)
                    current_fire = int(st.session_state.get(fire_key, 0) or 0)
                    current_good = max(current_processed - current_fire, 0)
                    current_area_mm2 = current_good * float(item["unit_area_mm2"])
                    if operation_kind in {"kaynak", "cita", "boya"}:
                        st.info(
                            f"Otomatik yapılan iş alanı: {_format_m2(current_area_mm2)} m² "
                            f"({current_good} sağlam adet × {_format_m2(item['unit_area_mm2'])} m²)"
                        )
                    st.divider()
    ...
    try:
        return pd.read_sql_query(
            ...
            SELECT
                ...
                w.calculated_area_mm2 / 1000000.0 AS Hesaplanan_Alan_m2,
...
def yonetici_paneli_page():
    ...
    if not operation_history.empty:
        ...
        st.markdown("#### Malzeme ve ölçü bazlı m² verimi")
        ...
        if laser_history.empty:
            ...
            material_efficiency["m2_saat"] = (
                material_efficiency["sağlam_alan_m2"]
                / material_efficiency["işlem_saati"].replace(0, 1)
            ).round(3)
            ...
            st.dataframe(
                material_efficiency[
                    [
                        ...
                        "plaka_adedi", "sağlam_adet", "sağlam_alan_m2",
                        "işlem_saati", "m2_saat",
                    ]
                ].rename(
                    columns={
                        ...
                        "m2_saat": "Verim (m²/saat)",
                    }
                ),
                use_container_width=True,
                hide_index=True,
            )
            best_row = material_efficiency.iloc[0]
            st.success(
                f"En yüksek görünen verim: {best_row['malzeme']} · "
                f"{best_row['ölçü']} · {best_row['kalinlik_mm']:.1f} mm · "
                f"{best_row['m2_saat']:.3f} m²/saat"
            )

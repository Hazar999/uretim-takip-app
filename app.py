...
NEO_VAC_CITA_BULK_MARKER = "neo_vac_cita_20260721_azamat_v1"
NEO_VAC_CITA_OPERATOR = "Azamat Remetov"
NEO_VAC_CITA_DATE = "2026-07-21"
NEO_VAC_CITA_TOTAL_HOURS = 8.0
NEO_VAC_CITA_POS_MIN = 10
NEO_VAC_CITA_POS_MAX = 360
...
def _neo_vac_cita_bulk_already_saved() -> tuple[bool, str]:
    """Aynı tek seferlik çıta kaydının ikinci kez oluşmasını engeller."""
    conn = get_db_connection()
    try:
        setting_row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_CITA_BULK_MARKER,),
        ).fetchone()
...
def _insert_cita_into_plan(plan_operations: list[str], cita_operation: str) -> list[str]:
    """Çıta yoksa mevcut operasyon sırasını bozmadan uygun noktaya ekler."""
    operations = [str(value).strip() for value in plan_operations if str(value).strip()]
    if any(_operation_kind(name) == "cita" for name in operations):
        return operations

    # Çıta genellikle abkanttan sonra, kaynak/boya/paketleme öncesinde yer alır.
    insert_before_kinds = {"kaynak", "boya", "paketleme", "sevkiyat"}
    insert_at = len(operations)
    for index, operation_name in enumerate(operations):
        if _operation_kind(operation_name) in insert_before_kinds:
            insert_at = index
            break
    operations.insert(insert_at, cita_operation)
    return operations


def _get_neo_vac_cita_items() -> pd.DataFrame:
    """Neo Vac projesindeki POS10-POS360 çıta ilerlemelerini getirir."""
    conn = get_db_connection()
    try:
        ...
        items["_pos_no"] = (
            items["pos"].astype(str).str.extract(r"(\d+)")[0]
            .fillna(0)
            .astype(int)
        )
        items = items[
            items["_pos_no"].between(
                NEO_VAC_CITA_POS_MIN,
                NEO_VAC_CITA_POS_MAX,
                inclusive="both",
            )
        ].copy()
        if items.empty:
        ...
        for _, item in items.iterrows():
            ...
            cita_logs = [
                ...
                if _operation_kind(str(operation_name)) == "cita"
            ]

            cita_operation = next(
                (name for name in plan_operations if _operation_kind(name) == "cita"),
                "",
            )
            if not cita_operation and cita_logs:
                cita_operation = cita_logs[-1][0]
            if not cita_operation:
                cita_operation = "çıta"

            # Plan henüz kaydedilmediyse önceki operasyon loglarından güvenli bir plan kur.
            effective_plan = plan_operations or log_operations
            effective_plan = _insert_cita_into_plan(effective_plan, cita_operation)

            row = item.to_dict()
            row["plan_operations"] = effective_plan
            row["plan_was_missing"] = not bool(plan_operations)
            row["cita_operation"] = cita_operation
            row["existing_cita_good"] = sum(value for _, value in cita_logs)
...
def _build_neo_vac_cita_bulk_plan(selected_oc: str) -> dict:
    """21/07/2026 tarihli Neo Vac çıta tamamlama planını hazırlar."""
    items = _get_neo_vac_cita_items()
    if items.empty:
    ...
    for item in items.itertuples(index=False):
        ...
        if bool(item.plan_was_missing):
            warnings.append(
                f"{item.pos}: kayıtlı operasyon planı bulunmadığı için mevcut işlem "
                "geçmişinden plan oluşturuldu."
            )
        ...
        rows.append({
            ...
            "operation_name": cita_operation,
            "plan_operations": plan_operations,
            "combination_name": combination_name,
        })

    quantities = [int(row["good_qty"]) for row in rows]
    hour_allocations = _bulk_allocate_hours(NEO_VAC_CITA_TOTAL_HOURS, quantities)
...
def _save_neo_vac_cita_bulk_plan(plan: dict) -> int:
    """Hazırlanan çıta planını tek batch olarak kaydeder."""
    entries = list((plan or {}).get("entries") or [])
    plans = dict((plan or {}).get("plans") or {})
    if not entries:
        raise ValueError("Kaydedilecek kalan çıta adedi bulunamadı.")

    already_saved, saved_value = _neo_vac_cita_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu Neo Vac toplu çıta kaydı daha önce uygulandı. "
            f"Kayıt numarası: {saved_value or '-'}"
        )

    note = (
        f"{NEO_VAC_CITA_BULK_MARKER} | Neo Vac POS10-POS360 "
        "toplu çıta tamamlama kaydı"
    )
    # Toplu çıta planında aynı operasyon adı birden fazla kaynaktan gelmiş
    # olabilir. Veritabanına göndermeden önce POS bazında tekrarları temizle.
    safe_plans = {}
    for item_id, item_plan in plans.items():
        unique_operations = []
        seen_operation_names = set()
        for raw_operation in item_plan.get("operations", []):
            operation = " ".join(str(raw_operation or "").split())
            if not operation:
                continue
            operation_key = operation.casefold()
            ...
            seen_operation_names.add(operation_key)
            unique_operations.append(operation)
        safe_plans[int(item_id)] = {
            "combination_name": str(item_plan.get("combination_name", "") or ""),
            "operations": unique_operations,
        }

    batch_id = save_operation_batch(
        {
            "tarih": NEO_VAC_CITA_DATE,
            "operator_ismi": NEO_VAC_CITA_OPERATOR,
            "calisma_tipi": "Tam zamanlı",
            "calisma_saati": NEO_VAC_CITA_TOTAL_HOURS,
            "neden": "",
            "notlar": note,
        },
        entries,
    ...
    try:
        ...
        conn.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (NEO_VAC_CITA_BULK_MARKER, str(int(batch_id)), now),
        )
        conn.commit()
        _clear_data_caches()
    finally:
...
def render_neo_vac_bulk_cita_tool():
    """Neo Vac çıta geçmişini tek seferlik yönetici aracıyla kaydeder."""
    st.markdown("**Neo Vac toplu çıta kaydı**")
    st.caption(
        "Tarih: 21/07/2026 · Operatör: Azamat Remetov · Toplam süre: 8 saat · "
        "POS10-POS360 arasındaki bütün kalan çıta adetleri tamamlanır."
    )

    already_saved, batch_value = _neo_vac_cita_bulk_already_saved()
    if already_saved:
        st.success("Bu toplu çıta kaydı daha önce uygulandı; ikinci kez eklenmeyecek.")
        if batch_value:
    ...
    items = _get_neo_vac_cita_items()
    if items.empty:
        st.error("Neo Vac projesinde POS10-POS360 arasında kayıt bulunamadı.")
        return
    ...
    if len(labels) == 1:
        ...
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_cita_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_cita_bulk_plan(selected_oc)
    ...
    for entry in plan.get("entries", []):
        preview_rows.append({
            "Tarih": "21/07/2026",
            "POS": entry["pos"],
            "Tamamlanacak Çıta Adedi": int(entry["good_qty"]),
            "Saat": float(entry["operation_hours"]),
        })

    if not preview_rows:
        st.success("POS10-POS360 arasındaki bütün çıta adetleri zaten tamamlanmış.")
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3 = st.columns(3)
    m1.metric("POS sayısı", int(len(preview)))
    m2.metric("Toplam çıta adedi", int(preview["Tamamlanacak Çıta Adedi"].sum()))
    m3.metric("Toplam saat", f"{preview['Saat'].sum():.1f}")
    st.dataframe(preview, use_container_width=True, hide_index=True)
    st.caption(
        "Yalnızca sistemde bulunan POS10-POS360 satırları kullanılır. Önceden çıtası "
        "tamamlanmış POS tekrar yazılmaz; 8 saat kalan adetlere orantılı dağıtılır."
    )

    confirm = st.checkbox(
        "Bu Neo Vac çıta kaydını kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_cita_bulk_confirm",
    )
    if st.button(
        "Neo Vac Çıta Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="neo_vac_cita_bulk_save_button",
    ):
        try:
            batch_id = _save_neo_vac_cita_bulk_plan(plan)
            st.session_state["neo_vac_cita_bulk_success"] = (
                f"Neo Vac toplu çıta kaydı tamamlandı. Kayıt numarası: {batch_id}."
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu çıta kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu çıta kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_cita",
                help="21 Temmuz 2026 Neo Vac POS10-POS360 çıta geçmişini tek seferde kaydeder.",
            ):
                success_message = st.session_state.pop(
                    "neo_vac_cita_bulk_success", None
                )
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_cita_tool()

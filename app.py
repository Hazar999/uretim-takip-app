...
def _bulk_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Toplam saati POS adetlerine orantılı dağıtır; toplam değişmez."""
    if not weights:
        return []
...
NEO_VAC_KAYNAK_BULK_MARKER = "neo_vac_kaynak_20260721_sadettin_v1"
NEO_VAC_KAYNAK_OPERATOR = "Sadettin Şeker"
NEO_VAC_KAYNAK_DATE = "2026-07-21"
NEO_VAC_KAYNAK_TOTAL_HOURS = 12.0
NEO_VAC_KAYNAK_POS_QTY = [
    ("POS190", 18),
    ("POS130", 2),
    ("POS110", 1),
    ("POS140", 1),
...
def _neo_vac_kaynak_bulk_already_saved() -> tuple[bool, str]:
    """Aynı tek seferlik kaynak kaydının ikinci kez oluşmasını engeller."""
    conn = get_db_connection()
    try:
        setting_row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_KAYNAK_BULK_MARKER,),
        ).fetchone()
        ...
        batch_row = conn.execute(
            ...
            LIMIT 1
            """,
            (f"%{NEO_VAC_KAYNAK_BULK_MARKER}%",),
        ).fetchone()
        if batch_row is not None:
            return True, str(int(batch_row[0]))
        return False, ""
    finally:
        conn.close()


def _insert_kaynak_into_plan(
    plan_operations: list[str],
    kaynak_operation: str,
) -> list[str]:
    """Kaynak yoksa mevcut sırayı bozmadan boya/taşlama öncesine ekler."""
    operations = [
        str(value).strip()
        for value in plan_operations
        if str(value).strip()
    ]
    if any(_operation_kind(name) == "kaynak" for name in operations):
        return operations
    ...
    for index, operation_name in enumerate(operations):
        normalized = _normalize_header(operation_name)
        if (
            "taslama" in normalized
            or _operation_kind(operation_name) in {
                "boya", "paketleme", "sevkiyat"
            }
        ):
            insert_at = index
            break
    operations.insert(insert_at, kaynak_operation)
    return operations


def _get_neo_vac_kaynak_items() -> pd.DataFrame:
    """Fotoğraftaki Neo Vac POS'larını ve kaynak ilerlemelerini getirir."""
    wanted_positions = {pos for pos, _ in NEO_VAC_KAYNAK_POS_QTY}
    conn = get_db_connection()
    try:
        ...
        items["pos"] = items["pos"].astype(str).map(_pos_label)
        items = items[items["pos"].isin(wanted_positions)].copy()
        if items.empty:
            return items

        quantity_map = dict(NEO_VAC_KAYNAK_POS_QTY)
        cur = conn.cursor()
        enriched_rows = []
        for _, item in items.iterrows():
            ...
            log_operation_rows = cur.execute(
                """
            ...
            log_operations = [str(row[0]) for row in log_operation_rows]
            kaynak_logs = [
                (str(operation_name), int(good_qty or 0))
                for operation_name, good_qty, _ in log_operation_rows
                if _operation_kind(str(operation_name)) == "kaynak"
            ]

            kaynak_operation = next(
                (
                    name
                    for name in plan_operations
                    if _operation_kind(name) == "kaynak"
                ),
                "",
            )
            if not kaynak_operation and kaynak_logs:
                kaynak_operation = kaynak_logs[-1][0]
            if not kaynak_operation:
                kaynak_operation = "kaynak"

            effective_plan = plan_operations or log_operations
            effective_plan = _insert_kaynak_into_plan(
                effective_plan,
                kaynak_operation,
            )
            ...
            row["kaynak_operation"] = kaynak_operation
            row["existing_kaynak_good"] = sum(
                value for _, value in kaynak_logs
            )
            row["photo_qty"] = int(quantity_map[str(item["pos"])])
            enriched_rows.append(row)

        result = pd.DataFrame(enriched_rows)
        order_map = {
            pos: index for index, (pos, _) in enumerate(NEO_VAC_KAYNAK_POS_QTY)
        }
        result["_photo_order"] = result["pos"].map(order_map)
        return (
            result.sort_values(["oc_no", "_photo_order"])
            .drop(columns=["_photo_order"])
            .reset_index(drop=True)
        )
    finally:
        conn.close()
...
def _build_neo_vac_kaynak_bulk_plan(selected_oc: str) -> dict:
    """21/07/2026 tarihli fotoğraftaki kaynak kayıt planını hazırlar."""
    items = _get_neo_vac_kaynak_items()
    if items.empty:
        return {
            "entries": [],
            "plans": {},
            "warnings": [],
            "missing_pos": [pos for pos, _ in NEO_VAC_KAYNAK_POS_QTY],
        }

    items = items[items["oc_no"].astype(str) == str(selected_oc)].copy()
    found_positions = set(items["pos"].astype(str).tolist())
    missing_positions = [
        pos for pos, _ in NEO_VAC_KAYNAK_POS_QTY
    ...
    for item in items.itertuples(index=False):
        ...
        existing_good = max(int(item.existing_kaynak_good or 0), 0)
        photo_qty = max(int(item.photo_qty or 0), 0)
        remaining_before = max(requested_qty - existing_good, 0)

        plan_operations = [str(value) for value in (item.plan_operations or [])]
        kaynak_operation = str(item.kaynak_operation or "kaynak")
        ...
        if photo_qty > remaining_before:
            warnings.append(
                f"{item.pos}: fotoğraftaki {photo_qty} adet, mevcut kaynak "
                f"kalanı olan {remaining_before} adedi aşıyor; kayıt fazla üretim "
                "olarak görünebilir."
            )
        ...
        rows.append({
            ...
            "operation_name": kaynak_operation,
            "plan_operations": plan_operations,
            "combination_name": combination_name,
        })

    quantity_weights = [int(row["good_qty"]) for row in rows]
    hour_allocations = _bulk_allocate_hours(
        NEO_VAC_KAYNAK_TOTAL_HOURS,
        quantity_weights,
    )

    entries = []
    plans = {}
    for row, operation_hours in zip(rows, hour_allocations):
...
def _save_neo_vac_kaynak_bulk_plan(plan: dict) -> int:
    """Fotoğraftaki kaynak planını tek mesaili batch olarak kaydeder."""
    entries = list((plan or {}).get("entries") or [])
    plans = dict((plan or {}).get("plans") or {})
    missing_positions = list((plan or {}).get("missing_pos") or [])
    if missing_positions:
        raise ValueError(
            "Seçilen Neo Vac OC içinde bulunmayan POS'lar var: "
            + ", ".join(missing_positions)
        )
    if not entries:
        raise ValueError("Kaydedilecek kaynak satırı bulunamadı.")

    already_saved, saved_value = _neo_vac_kaynak_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu Neo Vac toplu kaynak kaydı daha önce uygulandı. "
            f"Kayıt numarası: {saved_value or '-'}"
        )

    note = (
        f"{NEO_VAC_KAYNAK_BULK_MARKER} | Fotoğraftaki Neo Vac "
        "toplu kaynak kaydı"
    )
    batch_id = save_operation_batch(
        {
            "tarih": NEO_VAC_KAYNAK_DATE,
            "operator_ismi": NEO_VAC_KAYNAK_OPERATOR,
            "calisma_tipi": "Mesaili",
            "calisma_saati": NEO_VAC_KAYNAK_TOTAL_HOURS,
            "neden": "",
            "notlar": note,
        },
        entries,
    ...
    try:
        ...
        conn.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (NEO_VAC_KAYNAK_BULK_MARKER, str(int(batch_id)), now),
        )
        conn.commit()
        _clear_data_caches()
    finally:
...
def render_neo_vac_bulk_kaynak_tool():
    """Fotoğraftaki Neo Vac kaynak adetlerini tek seferlik kaydeder."""
    st.markdown("**Neo Vac toplu kaynak kaydı**")
    st.caption(
        "Tarih: 21/07/2026 · Operatör: Sadettin Şeker · Çalışma tipi: Mesaili · "
        "Toplam süre: 12 saat · Fotoğraftaki POS ve adetler aynen kaydedilir."
    )

    already_saved, batch_value = _neo_vac_kaynak_bulk_already_saved()
    if already_saved:
        st.success(
            "Bu toplu kaynak kaydı daha önce uygulandı; ikinci kez eklenmeyecek."
        )
        if batch_value:
    ...
    items = _get_neo_vac_kaynak_items()
    if items.empty:
        st.error("Neo Vac projesinde fotoğraftaki POS'lar bulunamadı.")
        return
    ...
    if len(labels) == 1:
        ...
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_kaynak_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_kaynak_bulk_plan(selected_oc)
    missing_positions = list(plan.get("missing_pos", []))
    if missing_positions:
        st.error(
            "Seçilen OC içinde bulunamayan POS'lar: "
            + ", ".join(missing_positions)
        )
    ...
    for entry in plan.get("entries", []):
        preview_rows.append({
            "Tarih": "21/07/2026",
            "POS": entry["pos"],
            "Kaynak Adedi": int(entry["good_qty"]),
            "Önceki Kalan": int(entry["remaining_before"]),
            "Saat": float(entry["operation_hours"]),
        })

    if not preview_rows:
        st.error("Kaydedilecek kaynak satırı oluşturulamadı.")
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3 = st.columns(3)
    m1.metric("POS sayısı", int(len(preview)))
    m2.metric("Toplam kaynak adedi", int(preview["Kaynak Adedi"].sum()))
    m3.metric("Toplam saat", f"{preview['Saat'].sum():.1f}")
    st.dataframe(preview, use_container_width=True, hide_index=True)
    st.caption(
        "Fotoğraftan okunan toplam 17 POS ve 70 adet kullanılır. "
        "12 saat adetlere orantılı dağıtılır; fire 0'dır."
    )

    confirm = st.checkbox(
        "Bu Neo Vac kaynak kaydını kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_kaynak_bulk_confirm",
    )
    if st.button(
        "Neo Vac Kaynak Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=(not confirm or bool(missing_positions)),
        key="neo_vac_kaynak_bulk_save_button",
    ):
        try:
            batch_id = _save_neo_vac_kaynak_bulk_plan(plan)
            st.session_state["neo_vac_kaynak_bulk_success"] = (
                f"Neo Vac toplu kaynak kaydı tamamlandı. Kayıt numarası: {batch_id}."
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu kaynak kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu kaynak kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_kaynak",
                help="21 Temmuz 2026 tarihli fotoğraftaki Neo Vac kaynak adetlerini tek seferde kaydeder.",
            ):
                success_message = st.session_state.pop(
                    "neo_vac_kaynak_bulk_success", None
                )
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_kaynak_tool()

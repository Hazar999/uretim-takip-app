    ...
    try:
        ...
        cur.execute(
            ...
            INSERT INTO other_work_logs (
                ...
                is_aciklamasi, neden, notlar, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                str(data["tarih"]),
...
def expand_worker_attribution(history: pd.DataFrame) -> pd.DataFrame:
    """
    Abkant ve boya ekip arkadaşlarını kişi bazlı analizlere dahil eder.
        ...
        for entry in entries:
            ...
            cur.execute(
                ...
                INSERT INTO operation_work_logs (
                    ...
                    abkant_teams_per_piece, participants_text, piece_weight_kg,
                    bend_type, calculated_area_mm2, fire_note, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    batch_id,
...
def main():
    ...
    st.caption("Sürüm 2.17.0 · HEDEF / EKİP / DİĞER ÇALIŞMA")
    st.markdown(
        """
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.17.0 · HEDEF / EKİP / DİĞER ÇALIŞMA
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                if operation_kind == "abkant":
                    ...
                    processed = abkant_team_qty // abkant_teams_per_piece
                else:
                    abkant_teams_per_piece = 1
                    abkant_team_qty = 0
                    processed = int(
...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            team1, team2, team3, team4, team5 = st.columns(
                                [1.2, 1.2, 1.0, 1.0, 1.0]
                            )
                            with team1:
                                st.number_input(
                                    "Üretilen takım sayısı",
                                    min_value=0,
                                    step=1,
                            ...
                            with team2:
                                st.number_input(
                                    ...
                                    help="Örneğin 4 takım 1 adet oluşturuyorsa 4 girin.",
                                )
                            with team3:
                                st.number_input(
                                    "Fire (adet)",
                            ...
                            with team5:
                                ...
                                st.button(
                                    ...
                                    args=(
                                        abkant_team_qty_key,
                                        remaining_qty * teams_per_piece_for_fill,
                                    ),
                                )
                            ...
                            current_processed_from_teams = (
                                current_team_qty // current_teams_per_piece
                            )
                            current_excess_teams = (
                                current_team_qty % current_teams_per_piece
                            )
                            st.info(
                                f"**{current_team_qty} takım ÷ "
                                f"{current_teams_per_piece} takım/adet = "
                                f"{current_processed_from_teams} adet**"
                            )
                            if current_excess_teams:
                                st.warning(
                                    f"{current_excess_teams} takım henüz tam bir adet oluşturmuyor."
                                )
                        ...
                        if operation_kind == "boya":
                            ...
                            st.multiselect(
                                "Beraber çalışan boya ekibi",
                                paint_options,
                                key=participants_key,
                                help="Ana çalışan dışında bu boya işine katılan tüm kişileri seçin.",
                            )
        ...
        with b2:
            if st.button("Kayıt Özetini Gör →", type="primary", use_container_width=True):
                ...
                for entry in entries:
                    ...
                    if operation_kind == "abkant":
                        if int(entry.get("abkant_team_qty", 0) or 0) <= 0:
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: Üretilen takım sayısını girmelisin."
                            )
                        if int(entry.get("abkant_teams_per_piece", 0) or 0) <= 0:
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: 1 adet için gereken takım sayısını girmelisin."
                            )
    ...
    for entry in entries:
        summary_rows.append({
            ...
            "Boya Ekibi": entry.get("participants_text", ""),
    ...
    try:
        return pd.read_sql_query(
            ...
            SELECT
                ...
                w.participants_text AS Boya_Ekibi,
...
def render_period_operation_pies(history: pd.DataFrame, other_logs: pd.DataFrame):
    st.markdown("### Etap bazlı süre ve m² dağılımı")
    st.caption("Günlük, aylık veya yıllık dönemde her etabın toplam saatini ve yaptığı m²'yi gösterir.")
    ...
    if not other.empty:
        ...
        if other_hours > 0 and selected_project == "Tümü":
            summary = pd.concat([
                summary,
                pd.DataFrame([{
                    "Etap": "Diğer Çalışma",
                    "Toplam Saat": other_hours,
                    "Toplam m²": 0.0,
                }])
            ], ignore_index=True)
    ...
    summary["Toplam Saat"] = pd.to_numeric(summary["Toplam Saat"], errors="coerce").fillna(0).round(1)
    summary["Toplam m²"] = pd.to_numeric(summary["Toplam m²"], errors="coerce").fillna(0).round(1)
    summary["m²/Saat"] = (
        summary["Toplam m²"] / summary["Toplam Saat"].replace(0, pd.NA)
    ).fillna(0).round(1)
    summary = summary.sort_values("Toplam m²", ascending=False)

    left, right = st.columns(2)
    with left:
        _vega_pie_chart(
            summary[summary["Toplam m²"] > 0],
            "Etap", "Toplam m²", f"{period} etap m² dağılımı"
        )
    with right:
        _vega_pie_chart(
            summary[summary["Toplam Saat"] > 0],
            "Etap", "Toplam Saat", f"{period} etap saat dağılımı",
...
def render_manager_efficiency_snapshot(history: pd.DataFrame):
    ...
    if not day_other.empty:
        other_worker = (
            day_other.groupby("operator_ismi", as_index=False)
            .agg(
    ...
    if not day_other.empty:
        with st.expander("Günün diğer çalışma kayıtları", expanded=False):
            st.dataframe(
                day_other[[
                    "operator_ismi", "calisma_saati", "is_aciklamasi",
                    "calisma_tipi", "notlar"
                ]].rename(columns={
                    "operator_ismi": "Çalışan",
                    "calisma_saati": "Saat",
...
def yonetici_paneli_page():
    ...
    if not other_work_logs.empty:
        ...
        st.dataframe(
            other_display[[
                "tarih", "operator_ismi", "is_aciklamasi",
                "calisma_saati", "calisma_tipi", "notlar"
            ]].rename(columns={
                "tarih": "Tarih",
                "operator_ismi": "Çalışan",
                "is_aciklamasi": "Yapılan İş",

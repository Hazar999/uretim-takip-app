...
def main():
    ...
    st.caption("İşçi çalışma süresi, POS, kombinasyon, adet ve fire bilgilerini kaydeder. — Sürüm 2.4 KENAR YÜKLEME + OTOMATİK OC/POS")
    if is_manager:
    ...
    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 2.4 KENAR YÜKLEME + OTOMATİK OC/POS")
        manager_login_panel()
        st.divider()
...
def yeni_kayit_page(reasons, combinations, is_manager: bool):
    ...
    project_name = str(selected_item["project_name"])
    ...
    detail4.metric("İstenen toplam alan", f"{_format_mm2(requested_area_mm2)} mm²")

    if is_manager:
        manager1, manager2, manager3 = st.columns(3)
        manager1.metric(
            "Çıktı ekranında üretilen",
            f"{output_produced_qty} adet",
        )
        manager2.metric(
            "Çıktı ekranında kalan",
    ...
    selected_combo = next(
        c for c in combinations if c["ad"] == kombinasyon_adi
    )
    ...
    with c2:
        default_hours = 9.0 if calisma_tipi == "Tam zamanlı" else 6.0
    ...
    with c3:
        neden = st.selectbox(
            "Neden",
            [""] + reasons,
            disabled=(calisma_tipi == "Tam zamanlı"),
        )
    with c4:
        st.number_input(
    ...
    max_daily_production = max(siparis_adedi, 0)
    c6, c7, c8 = st.columns(3)
    with c6:
        saglam_tamamlanan = st.number_input(
            "Bugün üretilen sağlam adet",
            min_value=0,
            max_value=max_daily_production,
            value=0,
            step=1,
        )
    with c7:
        ...
        st.metric(
            "Üretim yükü",
            f"{uretim_yuku} adet",
            help="İstenen üretim adedi + fire adedi",
        )
    with c8:
        kalan = max(siparis_adedi - saglam_tamamlanan, 0)
        st.metric("Bu kayıtta kalan ihtiyaç", f"{kalan} adet")
    ...
    st.caption(
        "İstenen adet ve fire toplamına göre ürünler açılır. "
        "Tamamlanan operasyonları işaretle."
    )
    ...
    st.checkbox(
        ...
        help=(
            "Bütün ürünlerin bütün operasyonları tamamlandıysa "
            "tek tıkla hepsini işaretler."
        ),
    )
        ...
        with st.expander(
            ...
            with top_cols[1]:
                fire_operasyonu = st.selectbox(
                    ...
                    fire_options,
                    key=f"fireop_{urun_no}",
                    disabled=not fire_var,
                )
            with top_cols[2]:
                fire_notu = st.text_input(
                    "Fire notu",
                    key=f"firenote_{urun_no}",
                    disabled=not fire_var,
                )
    ...
    if st.button("Kaydı Veritabanına Ekle", type="primary"):
        ...
        if siparis_adedi <= 0:
            errors.append("İstenen üretim adedi 0'dan büyük olmalı.")
        ...
        if saglam_tamamlanan > siparis_adedi:
            errors.append(
                "Bugün üretilen adet istenen adetten büyük olamaz."
            )

...
def main():
    ...
    st.caption("İşçi çalışma süresi, POS, kombinasyon, adet ve fire bilgilerini kaydeder. — Sürüm 2.5 MESAİ + ÇOKLU POS")
    if is_manager:
    ...
    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 2.5 MESAİ + ÇOKLU POS")
        manager_login_panel()
        if is_manager:
...
def yeni_kayit_page(reasons, combinations, is_manager: bool):
    ...
    if multiple_pos:
        ...
        completed_operations = st.multiselect(
            "Bu üretimlerde tamamlanan operasyonlar",
            operations,
            default=operations,
            help=(
                "Seçilen operasyon durumu, bu hızlı girişteki bütün POS adetlerine "
                "uygulanır. Farklı durumdaki POS'ları ayrı kayıt girebilirsin."
            ),
        ...
        editor_source = pd.DataFrame(
            {
                "item_id": chosen["item_id"].astype(int),
                "POS": chosen["pos"].astype(str),
                "İstenen Adet": chosen["requested_qty"].astype(int),
                "Önceden Üretilen": chosen["produced_qty"].astype(int),
                "Kalan": chosen["remaining_qty"].astype(int),
                "Boy (mm)": chosen["boy_mm"].astype(float),
                "En (mm)": chosen["en_mm"].astype(float),
                "Birim Alan (mm²)": chosen["unit_area_mm2"].astype(float),
                "Bugün Üretilen": 0,
                "Fire Adedi": 0,
            }
        )
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "Fire Adedi": st.column_config.NumberColumn(
                    "Fire Adedi",
                    min_value=0,
                    step=1,
                    required=True,
            ...
            key=f"multi_pos_editor_{selected_oc}",
        )

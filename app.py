...
def expand_worker_attribution(history: pd.DataFrame) -> pd.DataFrame:
    """
    Abkantta beraber çalışan kişiyi kişi bazlı analizlere dahil eder.

    Şirketin toplam üretimi iki kez sayılmamalıdır. Bu nedenle bu genişletilmiş
    veri yalnızca çalışan/kişi bazlı özetlerde kullanılmalıdır. Ortak çalışan,
    aynı abkant işinin adet, alan ve operasyon süresini kendi katkısı olarak görür.
    """
    ...
    required = {"operator_ismi", "operasyon", "beraber_calistigi"}
    if not required.issubset(base.columns):
        return base

    coworker = base["beraber_calistigi"].fillna("").astype(str).str.strip()
    operator = base["operator_ismi"].fillna("").astype(str).str.strip()
    is_joint_abkant = (
        base["operasyon"].map(_operation_kind).eq("abkant")
        & coworker.ne("")
        & coworker.ne(operator)
    )
    if "abkant_calisma_sekli" in base.columns:
        mode = base["abkant_calisma_sekli"].fillna("").astype(str).str.strip()
        is_joint_abkant &= mode.eq("Biriyle beraber çalıştı")

    shared = base.loc[is_joint_abkant].copy()
    if shared.empty:
        return base

    shared["ana_operator"] = shared["operator_ismi"].astype(str)
    shared["operator_ismi"] = shared["beraber_calistigi"].astype(str).str.strip()
    shared["katilim_rolu"] = "Beraber çalışan"
    return pd.concat([base, shared], ignore_index=True, sort=False)
...
def save_operation_batch(batch_data: dict, entries: list[dict], plans: dict[int, dict]) -> int:
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (batch_data["operator_ismi"], batch_data["tarih"], batch_id),
        )
        previous_day_hours = float(cur.fetchone()[0] or 0)
        cumulative_day_hours = previous_day_hours + float(batch_data["calisma_saati"])
        ...
        for entry in entries:
            ...
            cur.execute(
                ...
                INSERT INTO operation_work_logs (
                    ...
                    material_type, thickness_mm, abkant_work_mode,
                    abkant_coworker, bend_type, calculated_area_mm2,
...
def main():
    ...
    st.caption("Sürüm 2.16.7 · ABKANT ORTAK ÇALIŞAN / STABİL")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.16.7 — ABKANT ORTAK ÇALIŞAN VERİME DAHİL · STABİL TURSO
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.16.7 · ABKANT ORTAK ÇALIŞAN / STABİL
            </div>
            """,
            unsafe_allow_html=True,
        )
                    ...
                    if operation_kind == "laser":
                        ...
                        if lot_count_key not in st.session_state:
                            st.session_state[lot_count_key] = max(
                                len(saved_entries_for_operation),
                                1,
                            )

                        lot_count = max(
                            int(st.session_state.get(lot_count_key, 1) or 1),
                            1,
                        )

                        st.markdown(
                            f"**Kalınlık / plaka kayıtları:** {lot_count} satır"
                        )
                        st.caption(
                            "Her farklı kalınlık, malzeme veya plaka üretimi için "
                            "sağ taraftaki + Ekle düğmesine bas."
                        )
                        ...
                        for lot_index in range(1, lot_count + 1):
                            ...
                            defaults_for_widgets = {
                                processed_key: int(saved_lot.get("processed_qty", 0)),
                                ...
                                material_key: str(saved_lot.get("material_type", "Çelik") or "Çelik"),
                                thickness_key: float(saved_lot.get("thickness_mm", 0.0)),
                            }
                            for widget_key, widget_default in defaults_for_widgets.items():
                                if widget_key not in st.session_state:
                                    st.session_state[widget_key] = widget_default

                            st.markdown(f"##### Lazer üretim grubu {lot_index}")
                            laser1, laser2, laser3, laser4 = st.columns(4)
                            with laser1:
                                st.number_input(
                                    "Üretilen adet",
                                    min_value=0,
                            ...
                            with laser4:
                                st.number_input(
                                    "Kullanılan plaka adedi",
                                    min_value=0,
                                    step=1,
                                    key=plate_key,
                                )

                            laser5, laser6 = st.columns(2)
                            with laser5:
                                st.selectbox(
                                    "Malzeme",
                            ...
                            current_area_m2 = (
                                current_good * float(item["unit_area_mm2"])
                                / 1_000_000
                            )
                            adet_saat = (
                                current_good / current_hours
                                if current_hours > 0
                                else 0.0
                            )
                            m2_saat = (
                            ...
                            adet_plaka = (
                                current_good / current_plates
                                if current_plates > 0
                                else 0.0
                            )
                            st.info(
                                f"Bu grupta yapılan sağlam alan: **{current_area_m2:.1f} m²** "
                                f"({current_good} sağlam adet)"
                            )
                            st.caption(
                                f"Verim: {adet_saat:.1f} adet/saat · "
                                f"{m2_saat:.1f} m²/saat · "
                                f"{adet_plaka:.1f} adet/plaka"
                            )
                        ...
                        with add_lot_col:
                            if st.button(
                                "＋ Yeni Kalınlık / Plaka Grubu Ekle",
                        ...
                        with remove_lot_col:
                            if st.button(
                                ...
                                for removable_key in (
                                    f"processed_{item_id}_{normalized_operation}_{removed_suffix}",
                        ...
                        row1, row2, row3, row4 = st.columns([1.2, 1.0, 1.0, 1.0])
                        with row1:
                            st.number_input(
                                "İşlem yapılan",
                                min_value=0,
                        ...
                        with row4:
                            ...
                            st.button(
                                ...
                                disabled=(remaining_qty == 0),
                                use_container_width=True,
                                key=f"fill_{item_id}_{normalized_operation}_{version}",
                                on_click=set_session_value,
                                args=(processed_key, remaining_qty),
                            )

                        if operation_kind == "abkant":
                            abkant1, abkant2, abkant3 = st.columns(3)
                            with abkant1:
                                st.selectbox(
                                    "Çalışma şekli",
        ...
        with b2:
            if st.button("Kayıt Özetini Gör →", type="primary", use_container_width=True):
                ...
                for entry in entries:
                    ...
                    if operation_kind == "abkant":
                        if not str(entry.get("bend_type", "")).strip():
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: Büküm türünü seçmelisin."
                            )
                        if (
    ...
    if editable.empty:
        ...
        edited = st.data_editor(
            ...
            disabled=[
                ...
                "Birim Alan (m²)", "Önceden Üretilen", "Kalan",
            ],
            column_config={
                "item_id": None,
...
def render_manager_efficiency_snapshot(history: pd.DataFrame):
    """Yönetici panelinde doğrudan görünen kısa şirket özeti."""
    person_day, _ = _prepare_company_efficiency(history)
    if person_day.empty:
        return

    latest_date = person_day["tarih_dt"].max()
    latest = person_day[person_day["tarih_dt"].eq(latest_date)].copy()
    raw = history.copy()
    raw["tarih_dt"] = pd.to_datetime(raw["tarih"], errors="coerce")
    raw_latest = raw[raw["tarih_dt"].eq(latest_date)].copy()
    raw_latest["alan_m2"] = (
        pd.to_numeric(raw_latest["saglam_alan_mm2"], errors="coerce").fillna(0)
        / 1_000_000
    )
    raw_latest["operasyon_turu"] = raw_latest["operasyon"].map(_operation_kind)
    company_total_m2 = float(raw_latest["alan_m2"].sum())
    company_paint_m2 = float(
        raw_latest.loc[raw_latest["operasyon_turu"].eq("boya"), "alan_m2"].sum()
    )
    st.markdown("### Günlük kişi bazlı m² verimi")
    st.caption(
        f"{latest_date.strftime('%d.%m.%Y')} günü · Ortak abkant işi iki çalışanın da katkısında görünür; şirket toplamı tek kez sayılır."
    )
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Günlük şirket toplamı", f"{company_total_m2:.1f} m²")
    k2.metric("Günlük boya", f"{company_paint_m2:.1f} m²")
    k3.metric("Cut Out", f"{int(latest['cut_out_adet'].sum())} adet")
    best = latest.sort_values("toplam_m2", ascending=False).iloc[0]
    k4.metric("Günün en yüksek m²'si", f"{best['toplam_m2']:.1f} m²", delta=str(best["operator_ismi"]), delta_color="off")

    left, right = st.columns([1.15, 0.85])
    with left:
        _vega_bar_chart(
            latest.sort_values("toplam_m2", ascending=False),
            "operator_ismi", "toplam_m2",
            "Bugün çalışan bazlı katkı alanı", "Katkı alanı (m²)"
        )
    with right:
        pie_data = latest[latest["toplam_m2"] > 0][["operator_ismi", "toplam_m2"]]
        _vega_pie_chart(pie_data, "operator_ismi", "toplam_m2", "Bugünkü çalışan katkı payı")

    display = latest.rename(columns={
        "tarih_dt": "Tarih", "operator_ismi": "Çalışan",
        "toplam_m2": "Toplam m²", "boya_m2": "Boya m²",
        "laser_m2": "Laser m²", "cut_out_adet": "Laser Cut Out Adet",
        "sağlam_adet": "Sağlam Adet", "fire": "Fire",
        "operasyon_saati": "İşlem Saati", "m2_saat": "m²/Saat",
    })
    st.dataframe(
        display[[
            "Tarih", "Çalışan", "Toplam m²", "Boya m²", "Laser m²",
            "Laser Cut Out Adet", "İşlem Saati", "m²/Saat", "Fire"
        ]],
        use_container_width=True, hide_index=True,
        column_config={
            "Toplam m²": st.column_config.NumberColumn(format="%.1f"),
            "Boya m²": st.column_config.NumberColumn(format="%.1f"),
            "Laser m²": st.column_config.NumberColumn(format="%.1f"),
...
def grafikler_page():
    ...
    with st.container(border=True):
        f1, f2, f3 = st.columns([1.45, 1.0, 1.0])
        with f1:
            selected_range = st.date_input(
                ...
                min_value=min_date,
                max_value=max_date,
                key="analysis_date_range_v215",
            )
        with f2:
            workers = ["Tümü"] + sorted(
                person_history["operator_ismi"].dropna().astype(str).unique().tolist()
            )
            selected_worker = st.selectbox(
                "Çalışan",
                workers,
                key="analysis_worker_filter_v215",
            )
        with f3:
            operations = ["Tümü"] + sorted(
                history["operasyon"].dropna().astype(str).unique().tolist()
            )
            selected_operation = st.selectbox(
                "Operasyon",
                operations,
                key="analysis_operation_filter_v215",
            )
    ...
    total_m2 = float(filtered["alan_m2"].sum())
    ...
    k1.metric("Toplam sağlam alan", f"{total_m2:.1f} m²")
    k2.metric("Günlük ortalama", f"{avg_daily_m2:.1f} m²")
    k3.metric("Kişi/gün ortalaması", f"{worker_day_avg:.1f} m²")
    ...
    with tab_operations:
        ...
        with left:
            _vega_bar_chart(
                ...
                "operasyon",
                "alan_m2",
                "Operasyon bazlı toplam sağlam alan",
                "Alan (m²)",
            )
        with right:
            _vega_bar_chart(
...
def yonetici_paneli_page():
    ...
    if not operation_history.empty:
        ...
        st.caption("Abkantta beraber çalışan kişi de aynı işin adet, m² ve operasyon süresi katkısında görünür. Şirket toplamında iş yalnızca bir kez sayılır.")

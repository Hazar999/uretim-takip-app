...
def main():
    ...
    st.caption("İşçi çalışma süresi, POS, kombinasyon, adet ve fire bilgilerini kaydeder. — Sürüm 2.3 ANA EKRAN + OC SEÇİMİ")
    if is_manager:
        st.success("Yönetici görünümü: Tüm kayıtlar, grafikler, raporlar ve silme işlemleri açık.")
    else:
        st.info("Çalışan görünümü: Ana ekrandan OC seçip günlük üretim girişi yapabilirsin.")

    worker_pages = ["Ana Ekran", "Yeni Kayıt", "Üretime Devam Et"]
    manager_pages = [
        "Ana Ekran",
        "Yeni Kayıt",
        "Üretime Devam Et",
    ...
    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 2.3 ANA EKRAN + OC SEÇİMİ")
        manager_login_panel()
        st.divider()

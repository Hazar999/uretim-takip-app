    ...
    try:
        ...
        for entry in entries:
            ...
            if operation_kind == "abkant":
                ...
                new_long_single = int(
                    entry.get("abkant_long_single_bend_qty", 0) or 0
                )
                new_long_double = int(
                    entry.get("abkant_long_double_bend_qty", 0) or 0
                )
                new_short_single = int(
                    entry.get("abkant_short_single_bend_qty", 0) or 0
                )
                new_short_double = int(
                    entry.get("abkant_short_double_bend_qty", 0) or 0
                )
                ...
                (
                    previous_long_single,
                    previous_long_double,
                    previous_short_single,
                    previous_short_double,
                    previous_processed,
                ) = cur.fetchone()
                cur.execute(
                ...
                completed_after = _abkant_completed_qty(
                    int(previous_long_single or 0) + new_long_single,
                    int(previous_long_double or 0) + new_long_double,
                    int(previous_short_single or 0) + new_short_single,
                    int(previous_short_double or 0) + new_short_double,
                    long_bend_type,
                    short_bend_type,
                    requested_for_item,
                )
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                abkant_long_single_qty = (
                    ...
                    if operation_kind == "abkant"
                    and abkant_long_bend_type == "Tek büküm"
                    else 0
                )
                abkant_long_double_qty = (
                    ...
                    if operation_kind == "abkant"
                    and abkant_long_bend_type == "Çift büküm"
                    else 0
                )
                abkant_short_single_qty = (
                    ...
                    if operation_kind == "abkant"
                    and abkant_short_bend_type == "Tek büküm"
                    else 0
                )
                abkant_short_double_qty = (
                    ...
                    if operation_kind == "abkant"
                    and abkant_short_bend_type == "Çift büküm"
                    else 0
                )
                ...
                if operation_kind == "abkant":
                    # Yalnızca projede seçilmiş büküm türünün adedi kayda girer.
                    # Gizlenen eski alanlar session_state içinde kalsa bile hesaba katılmaz.
...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            ...
                            with edge1:
                                st.markdown(f"**Uzun kenar: {selected_long_type}**")
                                if selected_long_type == "Tek büküm":
                                    st.number_input(
                                        "Uzun kenar yapılan adet",
                                        min_value=0,
                                        step=1,
                                    ...
                                    current_long_qty = int(
                                        st.session_state.get(
                                            abkant_long_single_bend_qty_key, 0
                                        ) or 0
                                    )
                                elif selected_long_type == "Çift büküm":
                                    st.number_input(
                                        "Uzun kenar yapılan adet",
                                        min_value=0,
                                        step=1,
                                    ...
                                    current_long_qty = int(
                                        st.session_state.get(
                                            abkant_long_double_bend_qty_key, 0
                                        ) or 0
                                    )
                                elif selected_long_type == "Yok":
                                    st.info("Uzun kenarda büküm yapılmayacak.")
                                else:
                                    st.caption("Önce uzun kenar durumunu seçin.")

                            with edge2:
                                st.markdown(f"**Kısa kenar: {selected_short_type}**")
                                if selected_short_type == "Tek büküm":
                                    st.number_input(
                                        "Kısa kenar yapılan adet",
                                        min_value=0,
                                        step=1,
                                    ...
                                    current_short_qty = int(
                                        st.session_state.get(
                                            abkant_short_single_bend_qty_key, 0
                                        ) or 0
                                    )
                                elif selected_short_type == "Çift büküm":
                                    st.number_input(
                                        "Kısa kenar yapılan adet",
                                        min_value=0,
                                        step=1,
                                    ...
                                    current_short_qty = int(
                                        st.session_state.get(
                                            abkant_short_double_bend_qty_key, 0
                                        ) or 0
                                    )
                                elif selected_short_type == "Yok":
                                    st.info("Kısa kenarda büküm yapılmayacak.")
                                else:
                                    st.caption("Önce kısa kenar durumunu seçin.")

                            st.caption(
                                "Her kenar için projede istenen büküm türü bir kez seçilir. "
                                "Sonraki kayıtlarda aynı POS için seçim otomatik gelir."
                            )
                            ...
                            active_today_totals = []
                            if selected_long_type in {"Tek büküm", "Çift büküm"}:
                                active_today_totals.append(current_long_qty)
                            if selected_short_type in {"Tek büküm", "Çift büküm"}:
                                active_today_totals.append(current_short_qty)

                            if active_today_totals:
                                today_possible_complete = min(active_today_totals)
                                st.info(
                                    f"Bugünkü giriş: **{current_long_qty} uzun kenar · "
                                    f"{current_short_qty} kısa kenar**. "
                                    f"Seçilen proje kuralına göre bugün kendi içinde en fazla "
                                    f"**{today_possible_complete} adet** tamamlanabilir."
                                )

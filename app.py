            ...
            <div class="durlum-brand">
                ...
                <div>
                    ...
                    <h1 class="durlum-title">
                        Üretim <span>Kontrol</span> Paneli
                    </h1>
                    <div class="durlum-subtitle">
                        OC, POS, operasyon, verim, mesai ve fire süreçleri tek ekranda
                    </div>
                </div>
            </div>
...
def main():
    ...
    st.caption("Sürüm 2.11 PREMIUM ARAYÜZ · İşçi, OC, POS, işlem, adet, fire ve yetkinlik takibi")
    if is_manager:
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-brand">
                ...
                <div>
                    <div class="sidebar-brand-title">durlum üretim</div>
                    <div class="sidebar-brand-subtitle">Operasyon & verim yönetimi</div>
                </div>
            </div>
            <div class="sidebar-version">
                Sürüm 2.11 · PREMIUM ARAYÜZ
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def yeni_kayit_page(reasons, combinations, is_manager: bool):
    ...
    for _, item in selected_items.sort_values("_sort_pos").iterrows():
        ...
        with st.expander(
            ...
            for operation in selected_ops:
                ...
                completed_qty = (
                    int(progress_row["completed_qty"])
    ...
    performance = calculate_entries_performance(entries, float(work_hours))
    total_processed = int(performance["total_processed"])
    total_fire = int(performance["total_fire"])
    total_good = int(performance["total_good"])
    ...
    s4.metric(
        "Sağlam alan",
        f"{_format_mm2(performance['total_good_area_mm2'])} mm²",
    )

    v1, v2, v3, v4 = st.columns(4)
    v1.metric(
        "Adet / saat",
        f"{performance['qty_per_hour']:.2f}",
    )
    v2.metric(
        "mm² / saat",
        f"{_format_mm2(performance['area_per_hour'])}",
    )
    v3.metric(
        "Fire oranı",
        f"%{performance['fire_rate_pct']:.2f}",
    )
    v4.metric(
        "Gidişat",
        performance["status"],
    ...
    if entries:
        if performance["status"] == "Yavaş":
            st.warning(
                "Gidişat yavaş görünüyor. Karşılaştırma: "
                + performance["reference_source"]
            )
        elif performance["status"] == "Hızlı":
            st.success(
                "Gidişat hızlı görünüyor. Karşılaştırma: "
                + performance["reference_source"]
            )
        elif performance["status"] == "Normal":
            st.info(
                "Gidişat normal aralıkta. Karşılaştırma: "
                + performance["reference_source"]
            )
    ...
    for breach in performance["fire_breaches"]:
        st.error(
            ...
            f"Fire oranı %{breach['fire_rate_pct']:.1f}. "
...
def kayitlar_page():
    ...
    with operation_tab:
        if operation_batches.empty:
            st.info("Henüz operasyon bazlı kayıt bulunmuyor.")
        else:
...
def ozet_page():
    ...
    if overview.empty and operation_history.empty:
        st.info("Özet için henüz yüklenmiş OC veya operasyon kaydı yok.")
        return

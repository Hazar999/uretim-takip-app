...
NEO_VAC_PLAZMA_BULK_MARKER = "neo_vac_plazma_20260720_21_oguzhan_v1"
NEO_VAC_PLAZMA_OPERATOR = "Oğuzhan Çevik"
NEO_VAC_PLAZMA_DAY_SPECS = [
    {"date": "2026-07-20", "hours": 9.0},
    {"date": "2026-07-21", "hours": 9.0},
]


def _plazma_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Günlük toplam saati adetlere orantılı dağıtır; toplam değişmez."""
    if not weights:
        return []
...
def _is_plazma_operation(operation_name: str) -> bool:
    normalized = _normalize_header(operation_name)
    return normalized in {"plazma", "plasma"}


def _dedupe_operation_names(operations: list[str]) -> list[str]:
    """Operasyon sırasını koruyarak aynı isimleri tekilleştirir."""
    result = []
    seen = set()
...
def _insert_plazma_into_plan(
    plan_operations: list[str],
    plazma_operation: str,
) -> list[str]:
    """Plazma yoksa mevcut operasyon sırasına güvenli biçimde ekler."""
    operations = _dedupe_operation_names(plan_operations)
    if any(_is_plazma_operation(name) for name in operations):
        return operations

    # Plazma kesim işlemi olduğundan lazer/cut-out sonrasına; büküm ve montaj
    # operasyonlarından önceye yerleştirilir.
    insert_at = 0
    for index, operation_name in enumerate(operations):
        if _operation_kind(operation_name) in {"laser", "laser_cut_out"}:
            insert_at = index + 1

    if insert_at == 0:
        later_stage_tokens = (
            "abkant", "abkand", "cita", "kaynak", "taslama",
            "boya", "paket", "sevkiyat",
        )
        insert_at = len(operations)
        for index, operation_name in enumerate(operations):
            normalized = _normalize_header(operation_name)
            if any(token in normalized for token in later_stage_tokens):
                insert_at = index
                break

    operations.insert(insert_at, str(plazma_operation or "plazma"))
    return _dedupe_operation_names(operations)


def _neo_vac_plazma_day_marker(work_date: str) -> str:
    return f"{NEO_VAC_PLAZMA_BULK_MARKER}:{work_date}"


def _neo_vac_plazma_day_existing_batch(work_date: str) -> int | None:
    day_marker = _neo_vac_plazma_day_marker(work_date)
    conn = get_db_connection()
    try:
        row = conn.execute(
            """
...
def _neo_vac_plazma_bulk_already_saved() -> tuple[bool, str]:
    """Toplu plazma kaydının ikinci kez oluşmasını engeller."""
    conn = get_db_connection()
    try:
        setting_row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_PLAZMA_BULK_MARKER,),
        ).fetchone()
    ...
    existing_ids = [
        _neo_vac_plazma_day_existing_batch(spec["date"])
        for spec in NEO_VAC_PLAZMA_DAY_SPECS
    ]
...
def _get_neo_vac_plazma_items() -> pd.DataFrame:
    """Neo Vac projesindeki bütün POS'ların plazma ilerlemelerini getirir."""
    conn = get_db_connection()
    try:
        ...
        for _, item in items.iterrows():
            ...
            plazma_logs = [
                ...
                if _is_plazma_operation(str(operation_name))
            ]

            plazma_operation = next(
                (
                    name for name in plan_operations
                    if _is_plazma_operation(name)
                ),
                "",
            )
            if not plazma_operation and plazma_logs:
                plazma_operation = plazma_logs[-1][0]
            if not plazma_operation:
                plazma_operation = "plazma"

            effective_plan = plan_operations or log_operations
            effective_plan = _insert_plazma_into_plan(
                effective_plan,
                plazma_operation,
            )
            ...
            row["plazma_operation"] = plazma_operation
            row["existing_plazma_good"] = sum(
                value for _, value in plazma_logs
            )
...
def _split_plazma_quantities(
    rows: list[dict],
    active_dates: list[str],
) -> dict[str, dict[int, int]]:
    """Kalan adetleri aktif günlere mümkün olduğunca dengeli dağıtır."""
    result = {work_date: {} for work_date in active_dates}
    if not active_dates:
        return result
    if len(active_dates) == 1:
        work_date = active_dates[0]
        for row in rows:
            result[work_date][int(row["item_id"])] = int(row["remaining_qty"])
        return result

    totals = {work_date: 0 for work_date in active_dates}
    for row in rows:
        item_id = int(row["item_id"])
        quantity = max(int(row["remaining_qty"]), 0)
        base, remainder = divmod(quantity, len(active_dates))
        for work_date in active_dates:
            result[work_date][item_id] = base
            totals[work_date] += base

        for _ in range(remainder):
            target_date = min(
                active_dates,
                key=lambda value: (totals[value], active_dates.index(value)),
            )
            result[target_date][item_id] += 1
            totals[target_date] += 1
    return result


def _build_neo_vac_plazma_bulk_plan(selected_oc: str) -> dict:
    """20 ve 21 Temmuz Neo Vac plazma kayıtlarını hazırlar."""
    items = _get_neo_vac_plazma_items()
    if items.empty:
    ...
    for item in items.itertuples(index=False):
        ...
        rows.append({
            ...
            "operation_name": plazma_operation,
            "plan_operations": plan_operations,
            "combination_name": combination_name,
        })

    existing_day_batches = {
        spec["date"]: _neo_vac_plazma_day_existing_batch(spec["date"])
        for spec in NEO_VAC_PLAZMA_DAY_SPECS
    }
    active_specs = [
        spec for spec in NEO_VAC_PLAZMA_DAY_SPECS
        if existing_day_batches[spec["date"]] is None
    ]
    active_dates = [spec["date"] for spec in active_specs]
    allocations = _split_plazma_quantities(rows, active_dates)

    day_plans = []
    row_lookup = {int(row["item_id"]): row for row in rows}
    for spec in NEO_VAC_PLAZMA_DAY_SPECS:
        work_date = str(spec["date"])
        existing_batch = existing_day_batches[work_date]
        ...
        allocated = allocations.get(work_date, {})
        day_rows = [
            (row_lookup[item_id], int(quantity))
            for item_id, quantity in allocated.items()
            if int(quantity) > 0
        ]
        quantities = [quantity for _, quantity in day_rows]
        hour_allocations = _plazma_allocate_hours(
            float(spec["hours"]),
            quantities,
        )
        ...
        for (row, quantity), operation_hours in zip(
            ...
            plans[int(row["item_id"])] = {
                ...
                "operations": _dedupe_operation_names(
...
def _save_neo_vac_plazma_bulk_plan(plan: dict) -> list[int]:
    """Hazırlanan iki günlük plazma planını güvenli biçimde kaydeder."""
    if not plan or not plan.get("days"):
        raise ValueError("Kaydedilecek Neo Vac plazma günü bulunamadı.")

    already_saved, saved_value = _neo_vac_plazma_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu Neo Vac toplu plazma kaydı daha önce uygulandı. "
            f"Kayıt numaraları: {saved_value or '-'}"
        )

    batch_ids = []
    for day in plan["days"]:
        existing_batch = day.get("existing_batch")
        if existing_batch is not None:
        ...
        if not entries:
            raise ValueError(
                f"{day['date']} günü için dağıtılacak plazma adedi bulunamadı."
            )

        day_marker = _neo_vac_plazma_day_marker(str(day["date"]))
        note = (
            f"{day_marker} | Neo Vac bütün POS'lar toplu plazma geçmiş kaydı"
        )
        batch_id = save_operation_batch(
            {
                "tarih": str(day["date"]),
                "operator_ismi": NEO_VAC_PLAZMA_OPERATOR,
                "calisma_tipi": "Tam zamanlı",
                "calisma_saati": float(day["hours"]),
                "neden": "",
                "notlar": note,
            },
            entries,
    ...
    if len(batch_ids) != len(NEO_VAC_PLAZMA_DAY_SPECS):
        raise ValueError("İki günlük plazma kaydının tamamı oluşturulamadı.")
...
def render_neo_vac_bulk_plazma_tool():
    """Neo Vac plazma geçmişini iki günlük tek seferlik araçla kaydeder."""
    st.markdown("**Neo Vac toplu plazma kaydı**")
    st.caption(
        "Operatör: Oğuzhan Çevik · 20/07/2026: 9 saat · "
        "21/07/2026: 9 saat · Neo Vac bütün POS'ların kalan plazma adetleri "
        "iki güne dengeli dağıtılır."
    )

    already_saved, batch_value = _neo_vac_plazma_bulk_already_saved()
    if already_saved:
        st.success(
            "Bu toplu plazma kaydı daha önce uygulandı; ikinci kez eklenmeyecek."
        )
        if batch_value:
    ...
    if len(labels) == 1:
        ...
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_plazma_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_plazma_bulk_plan(selected_oc)
    for warning in plan.get("warnings", []):
        st.warning(warning)

    preview_rows = []
    for day in plan.get("days", []):
        if day.get("existing_batch") is not None:
            st.info(
                f"{day['date']} tarihli toplu kayıt daha önce oluşturulmuş: "
                f"kayıt no {day['existing_batch']}."
            )
        for entry in day.get("entries", []):
            preview_rows.append({
                "Tarih": day["date"],
                "POS": entry["pos"],
                "Plazma Adedi": int(entry["good_qty"]),
                "Saat": float(entry["operation_hours"]),
            })

    if not preview_rows:
        st.success(
            "Neo Vac bütün POS'ların plazma adetleri zaten tamamlanmış."
        )
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Kayıt günü", int(preview["Tarih"].nunique()))
    m2.metric("POS satırı", int(len(preview)))
    m3.metric("Toplam plazma adedi", int(preview["Plazma Adedi"].sum()))
    m4.metric("Toplam saat", f"{preview['Saat'].sum():.1f}")
    st.dataframe(preview, use_container_width=True, hide_index=True)

    daily_summary = (
        preview.groupby("Tarih", as_index=False)
        .agg(
            POS=("POS", "nunique"),
            Adet=("Plazma Adedi", "sum"),
            Saat=("Saat", "sum"),
        )
    )
    ...
    st.caption(
        "Her POS'un kalan plazma adedi iki güne mümkün olduğunca dengeli "
        "paylaştırılır. Her günün 9 saati, o gün girilen adetlere orantılı "
        "dağıtılır. Fire 0 olarak kaydedilir."
    )

    confirm = st.checkbox(
        "Bu iki günlük plazma kaydını kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_plazma_bulk_confirm",
    )
    if st.button(
        "Neo Vac Plazma Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="neo_vac_plazma_bulk_save_button",
    ):
        try:
            batch_ids = _save_neo_vac_plazma_bulk_plan(plan)
            st.session_state["neo_vac_plazma_bulk_success"] = (
                "Neo Vac toplu plazma kayıtları tamamlandı. Kayıt numaraları: "
                + ", ".join(map(str, batch_ids))
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu plazma kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu plazma kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_plazma",
                help=(
                    "Neo Vac bütün POS'ların plazma adetlerini 20 ve 21 Temmuz "
                    "tarihlerine 9'ar saat olarak dağıtır."
                ),
            ):
                success_message = st.session_state.pop(
                    "neo_vac_plazma_bulk_success", None
                )
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_plazma_tool()

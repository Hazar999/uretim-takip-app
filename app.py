...
def _shared_remote_database(database_url: str, auth_token: str):
    """Turso bağlantısını her Streamlit yeniden çalışmasında yeniden açmaz."""
    if libsql is None:
        raise RuntimeError(
...
class _SharedConnectionLease:
    """
    Ortak Turso bağlantısını güvenli biçimde ödünç verir.

    Mevcut kodun her sorgudan sonra yaptığı ``close()`` çağrısı gerçek uzak
    bağlantıyı kapatmaz; yalnızca kilidi bırakır. Böylece her tıklamada yeniden
    TLS/HTTP bağlantısı kurulmasının oluşturduğu gecikme ortadan kalkar.
    """

    def __init__(self, resource: dict):
        self._connection = resource["connection"]
        self._lock = resource["lock"]
...
def get_db_connection():
    """
    Bulutta yeniden kullanılan kalıcı Turso bağlantısı, yerelde SQLite döndürür.

    Bulutta bağlantı bilgileri yoksa geçici SQLite'a kayıt yapılmasını engeller;
    böylece uygulama yeniden dağıtıldığında veri kaybı yaşanmaz.
    """
    database_url = _read_secret("TURSO_DATABASE_URL")
    auth_token = _read_secret("TURSO_AUTH_TOKEN")

    if database_url and auth_token:
        resource = _shared_remote_database(database_url, auth_token)
        return _SharedConnectionLease(resource)
...
def _clear_data_caches():
    """Yazma işlemlerinden sonra kısa süreli okuma önbelleklerini temizler."""
    try:
        st.cache_data.clear()
    except Exception:
        pass
...
def main():
    ...
    st.caption("Sürüm 2.16.3 · KALICI DB / GÜVENLİ HIZ OPTİMİZASYONU")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.16.3 — KALICI VERİTABANI · GÜVENLİ HIZLANDIRMA
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.16.1 · KALICI DB / HIZLANDIRILMIŞ
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def yeni_kayit_page(reasons, combinations, is_manager: bool):
    ...
    cumulative_hours = existing_daily_hours + work_hours
    ...
    with b2:
        ...
        if st.button(
            ...
            if existing_daily_hours + work_hours > 24.0:
                final_errors.append("Bu çalışan için günlük toplam çalışma süresi 24 saati aşamaz.")
            if work_type == "Yarı zamanlı" and not reason:
...
def render_manager_efficiency_snapshot(history: pd.DataFrame):
    ...
    st.caption(
        f"{latest_date.strftime('%d.%m.%Y')} günü · Tüm etapların alanı POS ölçüsünden otomatik hesaplanır."
    )
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Günlük toplam", f"{latest['toplam_m2'].sum():.1f} m²")
    k2.metric("Günlük boya", f"{latest['boya_m2'].sum():.1f} m²")
    ...
    with left:
        _vega_bar_chart(
            ...
            "Bugün çalışan bazlı üretim alanı", "Üretim alanı (m²)"
        )
    with right:
        pie_data = latest[latest["toplam_m2"] > 0][["operator_ismi", "toplam_m2"]]
        _vega_pie_chart(pie_data, "operator_ismi", "toplam_m2", "Bugünkü m² üretim payı")
...
def grafikler_page():
    ...
    with st.container(border=True):
        ...
        with f2:
            workers = ["Tümü"] + sorted(
                history["operator_ismi"].dropna().astype(str).unique().tolist()
            )
    ...
    if selected_worker != "Tümü":
        mask &= history["operator_ismi"].astype(str) == selected_worker
    if selected_operation != "Tümü":
        mask &= history["operasyon"].astype(str) == selected_operation
    ...
    worker_operation = (
        filtered.groupby(["operator_ismi", "operasyon"], as_index=False)
        .agg(alan_m2=("alan_m2", "sum"))
    )
    worker_operation["alan_m2"] = worker_operation["alan_m2"].round(1)

    worker_daily = (
        filtered.groupby(["tarih_dt", "operator_ismi"], as_index=False)
        .agg(
            m2=("alan_m2", "sum"),

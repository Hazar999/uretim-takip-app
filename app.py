...
def update_operation_batch_record(
    ...
    """Operasyon kaydını silmeden ana bilgiler ve üretim satırlarını günceller."""
    if edited_rows.empty:
        raise ValueError("Güncellenecek işlem satırı bulunamadı.")
    ...
    try:
        ...
        cur.execute(
            "SELECT DISTINCT item_id FROM operation_work_logs WHERE batch_id = ?",
            (int(batch_id),),
        )
        affected_item_ids = {int(row[0]) for row in cur.fetchall()}
        ...
        allowed_ids = {
            int(row[0]) for row in cur.execute(
                "SELECT id FROM operation_work_logs WHERE batch_id = ?",
                (int(batch_id),),
            ).fetchall()
        }
        updated_count = 0
        for _, row in edited_rows.iterrows():
            log_id = int(row["kayıt_no"])
            ...
            if fire_qty > processed_qty:
                raise ValueError(
                    f"{row.get('OC', '')} / {row.get('POS', '')}: "
                    "Fire adedi işlem yapılan adetten büyük olamaz."
                )
            good_qty = processed_qty - fire_qty
            operation_hours = max(
                float(row.get("İşlem Süresi (Saat)", 0) or 0), 0.0
            )
            fire_note = str(row.get("Fire Notu", "") or "")

            item_row = cur.execute(
                """
                SELECT w.item_id, i.unit_area_mm2
                FROM operation_work_logs w
                INNER JOIN production_output_items i ON i.id = w.item_id
                WHERE w.id = ? AND w.batch_id = ?
                """,
                (log_id, int(batch_id)),
            ).fetchone()
            if item_row is None:
            ...
            cur.execute(
                ...
                SET processed_qty = ?, fire_qty = ?, good_qty = ?,
                    operation_hours = ?, calculated_area_mm2 = ?, fire_note = ?
    ...
    with operation_tab:
        if operation_batches.empty:
            ...
            edited_details = st.data_editor(
                ...
                use_container_width=True,
                hide_index=True,
                disabled=["kayıt_no", "OC", "POS", "İşlem"],
                column_config={
                    "kayıt_no": None,
                    "İşlem Yapılan": st.column_config.NumberColumn(
                        "İşlem Yapılan", min_value=0, step=1, format="%d"
                    ),
                    "Fire": st.column_config.NumberColumn(
                        "Fire", min_value=0, step=1, format="%d"
                    ),
                    "İşlem Süresi (Saat)": st.column_config.NumberColumn(
                        "İşlem Süresi (Saat)", min_value=0.0, step=0.25, format="%.2f"
                    ),
                },
            ...
            if st.button(
                ...
                try:
                    ...
                    st.success(
                        ...
                        f"{result['updated_detail_count']} işlem satırı değiştirildi; "

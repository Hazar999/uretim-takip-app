        ...
        query = f"""
            ...
            operation_totals AS (
                SELECT
                    ...
                    COALESCE(SUM(laser_long_edge_qty), 0) AS laser_long_edge_qty,
                    COALESCE(SUM(laser_short_edge_qty), 0) AS laser_short_edge_qty,
                    COALESCE(MAX(laser_long_per_team), 2) AS laser_long_per_team,
                    COALESCE(MAX(laser_short_per_team), 2) AS laser_short_per_team,
                    COALESCE(
                        MAX(NULLIF(laser_edge_mode, '')),
                        'Uzun + kısa kenar'
                    ) AS laser_edge_mode,
                    COALESCE(SUM(abkant_team_qty), 0) AS abkant_team_qty,
                    COALESCE(MAX(abkant_teams_per_piece), 1) AS abkant_teams_per_piece,
                    COALESCE(SUM(abkant_long_bend_qty), 0)
                        AS abkant_long_team_qty,
                    COALESCE(SUM(abkant_short_bend_qty), 0)
                        AS abkant_short_team_qty,
                    COALESCE(SUM(abkant_long_single_bend_qty), 0)
                        AS abkant_long_single_qty,
                    COALESCE(SUM(abkant_long_double_bend_qty), 0)
                        AS abkant_long_double_qty,
                    COALESCE(SUM(abkant_short_single_bend_qty), 0)
                        AS abkant_short_single_qty,
                    COALESCE(SUM(abkant_short_double_bend_qty), 0)
        ...
        for entry in entries:
            ...
            cur.execute(
                ...
                (
                    ...
                    (
                        float(entry.get("calculated_area_mm2", 0) or 0)
                        if operation_kind == "laser"
                ...
                if processed > 0 or abkant_activity:
                    entries.append({
                        ...
                        "abkant_long_single_bend_qty": (
                            abkant_long_single_qty if operation_kind == "abkant" else 0
                        ),
                        "abkant_long_double_bend_qty": (
                            abkant_long_double_qty if operation_kind == "abkant" else 0
                        ),
                        "abkant_short_single_bend_qty": (
                            abkant_short_single_qty if operation_kind == "abkant" else 0
                        ),
                        "abkant_short_double_bend_qty": (
                            abkant_short_double_qty if operation_kind == "abkant" else 0
                        ),
                        ...
                        "abkant_long_bend_type": (
                            abkant_long_bend_type if operation_kind == "abkant" else ""
                        ),
                        "abkant_short_bend_type": (
                            abkant_short_bend_type if operation_kind == "abkant" else ""
                        ),

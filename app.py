...
def main():
    ...
    st.caption("Sürüm 2.9.2 KAYITLAR VE GÜN SONU ENTEGRASYONU · İşçi, OC, POS, işlem, adet, fire ve yetkinlik takibi")
    if is_manager:
    ...
    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 2.9.2 KAYITLAR VE GÜN SONU ENTEGRASYONU")
        manager_login_panel()
        if is_manager:
...
def render_operation_day_end_section() -> bool:
    ...
    m5.metric("Fire", f"{total_fire} adet")
    m6.metric("Toplam çalışma", f"{total_hours:.1f} saat")
    m7.metric("Verimlilik", f"{productivity:.2f} adet/saat")
    m8.metric("Çalışılan POS", int(day["POS"].nunique()))

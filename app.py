...
ISS_LASER_BULK_MARKER = "iss_laser_20260722_aykut_30min_v1"
ISS_LASER_OPERATOR = "Aykut Akili"
ISS_LASER_DATE = "2026-07-22"
ISS_LASER_TOTAL_HOURS = 0.5
ISS_LASER_MATERIAL = "Çelik"
ISS_LASER_THICKNESS_MM = 1.5
ISS_LASER_PLATE_QTY = 0


def _iss_bulk_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Toplam süreyi POS adetlerine orantılı dağıtır ve toplamı aynen korur."""
    if not weights:
        return []
    safe_weights = [max(float(value or 0), 0.0) for value in weights]
    weight_sum = sum(safe_weights)
    if weight_sum <= 0:
        result = [round(float(total) / len(weights), 4) for _ in weights]
    else:
        result = [
            round(float(total) * value / weight_sum, 4)
            for value in safe_weights
        ]
    result[-1] = round(result[-1] + round(float(total) - sum(result), 4), 4)
    return result


def _iss_laser_existing_batch() -> int | None:
    conn = get_db_connection()
    try:
        row = conn.execute(
            """
...
def _iss_laser_bulk_already_saved() -> tuple[bool, str]:
    conn = get_db_connection()
    try:
        row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (ISS_LASER_BULK_MARKER,),
        ).fetchone()
        if row is not None:
    ...
    existing_batch = _iss_laser_existing_batch()
    return (existing_batch is not None, str(existing_batch or ""))


def _get_iss_laser_items() -> pd.DataFrame:
    """Adında ISS geçen üretim çıktısı satırlarını ve mevcut lazer durumunu getirir."""
    conn = get_db_connection()
    try:
        ...
        def is_iss_project(value) -> bool:
            raw = unicodedata.normalize("NFKD", str(value or ""))
            raw = "".join(
                char for char in raw if not unicodedata.combining(char)
            ).lower()
            return bool(re.search(r"(?<![a-z0-9])iss(?![a-z0-9])", raw))

        items = items[items["project_name"].map(is_iss_project)].copy()
        if items.empty:
            return items
        ...
        for _, item in items.iterrows():
            ...
            log_rows = cur.execute(
                """
                SELECT operation_name, COALESCE(SUM(good_qty), 0)
                FROM operation_work_logs
                WHERE item_id = ?
            ...
            all_logs = [
                (str(operation_name), int(good_qty or 0))
                for operation_name, good_qty in log_rows
            ]
            laser_logs = [
                (operation_name, good_qty)
                for operation_name, good_qty in all_logs
            ...
            laser_operation = next(
                (
                    operation_name
                    for operation_name in plan_operations
                    if _operation_kind(operation_name) == "laser"
                ),
                "",
            )
            if not laser_operation and laser_logs:
                laser_operation = laser_logs[-1][0]
            if not laser_operation:
                laser_operation = "laser"

            if not plan_operations:
                seen = set()
                for operation_name, _ in all_logs:
                    key = operation_name.casefold()
                    if operation_name and key not in seen:
                        seen.add(key)
                        plan_operations.append(operation_name)

            if not any(
                _operation_kind(operation_name) == "laser"
                for operation_name in plan_operations
            ):
                plan_operations.insert(0, laser_operation)

            row = item.to_dict()
            row["plan_operations"] = plan_operations
            row["laser_operation"] = laser_operation
            row["existing_laser_good"] = sum(
                good_qty for _, good_qty in laser_logs
            )
...
def _build_iss_laser_bulk_plan(selected_oc: str) -> dict:
    """ISS projesinin kalan tüm lazer adetlerini 22 Temmuz kaydına hazırlar."""
    items = _get_iss_laser_items()
    if items.empty:
        return {"entries": [], "plans": {}, "skipped": []}

    items = items[items["oc_no"].astype(str) == str(selected_oc)].copy()
    if items.empty:
        return {"entries": [], "plans": {}, "skipped": []}
    ...
    for row in items.itertuples(index=False):
        ...
        existing_good = max(int(row.existing_laser_good or 0), 0)
        remaining_qty = max(requested_qty - existing_good, 0)
        if remaining_qty <= 0:
            skipped.append(f"{row.pos}: lazer daha önce tamamlanmış.")
            continue
        pending_rows.append((row, remaining_qty))

    hour_allocations = _iss_bulk_allocate_hours(
        ISS_LASER_TOTAL_HOURS,
        [remaining_qty for _, remaining_qty in pending_rows],
    )

    entries = []
    plans = {}
    for (row, remaining_qty), operation_hours in zip(
        pending_rows, hour_allocations
    ):
        plan_operations = [str(value) for value in (row.plan_operations or [])]
        laser_operation = str(row.laser_operation or "laser")
        if not any(
            _operation_kind(operation_name) == "laser"
            for operation_name in plan_operations
        ):
        ...
        combination_name = str(row.combination_name or "").strip()
        if not combination_name:
            combination_name = combination_name_from_operations(plan_operations)

        entries.append({
            "item_id": int(row.item_id),
            "pos": str(row.pos),
            "operation_name": laser_operation,
            "processed_qty": int(remaining_qty),
            "fire_qty": 0,
            "good_qty": int(remaining_qty),
            ...
            "laser_plate_qty": ISS_LASER_PLATE_QTY,
            "material_type": ISS_LASER_MATERIAL,
            "thickness_mm": ISS_LASER_THICKNESS_MM,
            "laser_long_edge_qty": int(remaining_qty) * 2,
            "laser_short_edge_qty": int(remaining_qty) * 2,
            "laser_long_per_team": 2,
            "laser_short_per_team": 2,
            "abkant_work_mode": "",
            "abkant_coworker": "",
            "abkant_team_qty": 0,
            ...
            "bend_type": "",
            "calculated_area_mm2": (
                int(remaining_qty) * float(row.unit_area_mm2)
            ),
            "fire_note": "",
        })
    ...
    return {"entries": entries, "plans": plans, "skipped": skipped}


def _save_iss_laser_bulk_plan(plan: dict) -> int:
    if not plan or not plan.get("entries"):
        raise ValueError("Kaydedilecek ISS lazer adedi bulunamadı.")

    already_saved, saved_value = _iss_laser_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu ISS toplu lazer kaydı daha önce uygulandı. "
            f"Kayıt numarası: {saved_value or '-'}"
        )
    ...
    note = (
        f"{ISS_LASER_BULK_MARKER} | ISS toplu lazer geçmiş kaydı | "
        "Çelik · 1,5 mm · plaka bilgisi bilinmiyor"
    )
    batch_id = save_operation_batch(
        {
            "tarih": ISS_LASER_DATE,
            "operator_ismi": ISS_LASER_OPERATOR,
            "calisma_tipi": "Yarı zamanlı",
            "calisma_saati": ISS_LASER_TOTAL_HOURS,
            "neden": "ISS projesi lazer kesimi",
            "notlar": note,
        },
        list(plan["entries"]),
        dict(plan["plans"]),
    ...
    try:
        ...
        conn.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (ISS_LASER_BULK_MARKER, str(batch_id), now),
        )
        conn.commit()
        _clear_data_caches()
    finally:
...
def render_iss_bulk_laser_tool():
    """ISS lazer geçmişini tek seferlik yönetici aracıyla kaydeder."""
    success_message = st.session_state.pop("iss_laser_bulk_success", None)
    if success_message:
        st.success(success_message)

    st.markdown("**ISS toplu lazer kaydı**")
    st.caption(
        "22/07/2026 · Aykut Akili · 30 dakika · Çelik · 1,5 mm · "
        "plaka bilgisi bilinmiyor"
    )

    already_saved, batch_value = _iss_laser_bulk_already_saved()
    if already_saved:
        st.success("Bu toplu lazer kaydı daha önce uygulandı; ikinci kez eklenmeyecek.")
        if batch_value:
            st.caption(f"Oluşturulan kayıt numarası: {batch_value}")
        return

    items = _get_iss_laser_items()
    if items.empty:
        st.error("Adında 'ISS' geçen proje bulunamadı.")
        return

    project_options = (
        items[["oc_no", "project_name"]]
        .drop_duplicates()
        .sort_values(["project_name", "oc_no"])
    )
    labels = [
        f"{row.project_name} · OC {row.oc_no}"
        for row in project_options.itertuples(index=False)
    ]
    label_to_oc = {
        label: str(row.oc_no)
        for label, row in zip(labels, project_options.itertuples(index=False))
    }

    if len(labels) == 1:
        selected_label = labels[0]
        st.info(selected_label)
    else:
        selected_label = st.selectbox(
            "ISS projesi / OC",
            labels,
            key="iss_laser_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_iss_laser_bulk_plan(selected_oc)
    preview_rows = [
        {
            "POS": entry["pos"],
            "Tamamlanacak Adet": int(entry["good_qty"]),
            "Malzeme": entry["material_type"],
            "Kalınlık (mm)": float(entry["thickness_mm"]),
            "Plaka": "Bilinmiyor",
            "Süre (saat)": float(entry["operation_hours"]),
        }
        for entry in plan.get("entries", [])
    ]

    if not preview_rows:
        st.success("Bu ISS projesinde kaydedilecek kalan lazer adedi bulunamadı.")
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3 = st.columns(3)
    m1.metric("POS satırı", int(len(preview)))
    m2.metric("Toplam adet", int(preview["Tamamlanacak Adet"].sum()))
    m3.metric("Toplam süre", "30 dakika")
    st.dataframe(preview, use_container_width=True, hide_index=True)
    st.caption(
        "ISS projesindeki her POS'un kalan lazer adedi tamamlanır. Toplam 30 dakika "
        "adetlere orantılı dağıtılır. Plaka sayısı bilinmediği için 0 olarak saklanır."
    )

    confirm = st.checkbox(
        "ISS lazer kaydını kontrol ettim ve kaydetmek istiyorum",
        key="iss_laser_bulk_confirm",
    )
    if st.button(
        "ISS Lazer Kaydını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="iss_laser_bulk_save_button",
    ):
        try:
            batch_id = _save_iss_laser_bulk_plan(plan)
            st.session_state["iss_laser_bulk_success"] = (
                f"ISS toplu lazer kaydı tamamlandı. Kayıt numarası: {batch_id}"
            )
            st.rerun()
        except Exception as exc:
            st.error(f"ISS toplu lazer kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "ISS toplu lazer kaydını aç",
                value=False,
                key="lazy_iss_bulk_laser",
                help="22/07/2026 tarihli ISS lazer geçmiş kaydını tek seferde oluşturur.",
            ):
                render_iss_bulk_laser_tool()

        ...
        for entry in entries:
            ...
            cur.execute(
                ...
                INSERT INTO operation_work_logs (
                    ...
                    abkant_short_bend_qty, abkant_manual_override,
                    ...
                    processed = (
                        int(
                            st.session_state.get(
                                processed_key,
                                saved_entry.get(
                                    "processed_qty", calculated_from_teams
                                ),
                            )
                            or 0
                        )
                        if manual_override
                        else calculated_from_teams
                    )
                else:
                    abkant_teams_per_piece = 1
                    abkant_team_qty = 0
...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            bend1, bend2, team1, team2 = st.columns(4)
                            with bend1:
                                st.number_input(
                                    "Uzun kenar bükümü (adet)",
                                    min_value=0,
                                    step=1,
                            ...
                            qty1, qty2, qty3, qty4, qty5 = st.columns(
                                [1.15, 1.0, 1.0, 1.0, 1.0]
                            )
                            with qty1:
                                manual_override = st.checkbox(
                                    "Adedi manuel düzelt",
                                    key=abkant_manual_override_key,
                                    help=(
                                        "Kapalıyken adet takım sayısından otomatik hesaplanır. "
                                        "Açıksa işlem yapılan adedi elle değiştirebilirsin."
                                    ),
                                )
                            with qty2:
                                if manual_override:
                            ...
                            manual_processed_qty = int(
                                st.session_state.get(processed_key, 0) or 0
                            )
                            registered_processed_qty = (
                                manual_processed_qty
                                if manual_override
                                else current_processed_from_teams
                            )
                            st.info(
                                f"**{current_team_qty} takım ÷ "
                                f"{current_teams_per_piece} takım/adet = "
                                f"{current_processed_from_teams} adet** · "
                                f"Kayda geçecek: **{registered_processed_qty} adet**"
                            )
    ...
    for entry in entries:
        summary_rows.append({
            ...
            "Uzun Kenar Büküm": entry.get("abkant_long_bend_qty", 0) or "",
            "Kısa Kenar Büküm": entry.get("abkant_short_bend_qty", 0) or "",

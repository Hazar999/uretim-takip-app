...
# -----------------------------
# 21-23 TEMMUZ 2026 HAZIR GEÇMİŞ KAYITLAR
# -----------------------------
JULY_2026_HISTORICAL_PRESETS = [
    {
        "marker": "HIST-20260721-SERCAN-NEOVAC-KESIM",
        "date": "2026-07-21",
        "worker": "Sercan Kaygusuz",
        "project_key": "neo_vac",
        "project_label": "Neo Vac",
        "operation_stage": "cutting",
        "operation_label": "Kesim",
        "quantity": 130,
        "hours": 12.0,
        "allocation_link": "neo_vac_20260721",
    },
    {
        "marker": "HIST-20260721-OTABEK-NEOVAC-TASLAMA",
        "date": "2026-07-21",
        "worker": "Otabek Seytimbetov",
        "project_key": "neo_vac",
        "project_label": "Neo Vac",
        "operation_stage": "grinding",
        "operation_label": "Taşlama",
        "quantity": 130,
        "hours": 12.0,
        "allocation_link": "neo_vac_20260721",
    },
    {
        "marker": "HIST-20260722-SERCAN-HERAKLION-POS20-KESIM",
        "date": "2026-07-22",
        "worker": "Sercan Kaygusuz",
        "project_key": "heraklion",
        "project_label": "Heraklion",
        "operation_stage": "cutting",
        "operation_label": "Kesim",
        "quantity": 259,
        "hours": 10.0,
    ...
    {
        "marker": "HIST-20260722-SERCAN-NEOVAC-KESIM",
        "date": "2026-07-22",
        "worker": "Sercan Kaygusuz",
        "project_key": "neo_vac",
        "project_label": "Neo Vac",
    ...
    {
        "marker": "HIST-20260722-OTABEK-NEOVAC-TASLAMA",
        "date": "2026-07-22",
        "worker": "Otabek Seytimbetov",
        "project_key": "neo_vac",
        "project_label": "Neo Vac",
    ...
    {
        "marker": "HIST-20260722-OTABEK-HERAKLION-POS20-TASLAMA",
        "date": "2026-07-22",
        "worker": "Otabek Seytimbetov",
        "project_key": "heraklion",
        "project_label": "Heraklion",
    ...
    {
        "marker": "HIST-20260723-SERCAN-HERAKLION-POS20-KESIM",
        "date": "2026-07-23",
        "worker": "Sercan Kaygusuz",
        "project_key": "heraklion",
        "project_label": "Heraklion",
    ...
    {
        "marker": "HIST-20260723-SERCAN-ISS-KESIM",
        "date": "2026-07-23",
        "worker": "Sercan Kaygusuz",
        "project_key": "iss",
        "project_label": "ISS",
        ...
        "quantity": 6,
        "hours": 4.5,
        "allocation_link": "iss_20260723",
    },
    {
        "marker": "HIST-20260723-OTABEK-HERAKLION-POS20-TASLAMA",
        "date": "2026-07-23",
        "worker": "Otabek Seytimbetov",
        "project_key": "heraklion",
        "project_label": "Heraklion",
    ...
    {
        "marker": "HIST-20260723-OTABEK-ISS-TASLAMA",
        "date": "2026-07-23",
        "worker": "Otabek Seytimbetov",
        "project_key": "iss",
        "project_label": "ISS",
...
def _historical_stage_key(operation_name: str) -> str:
    normalized = _normalize_header(operation_name)
    if "taslama" in normalized:
        return "grinding"
    if _operation_kind(operation_name) in {"kesim", "laser"}:
        return "cutting"
    return normalized or "other"


@st.cache_data(ttl=120, show_spinner=False)
def get_historical_operation_totals() -> pd.DataFrame:
    conn = get_db_connection()
    try:
        return pd.read_sql_query(
            """
            SELECT item_id, operation_name, COALESCE(SUM(good_qty), 0) AS good_qty
            FROM operation_work_logs
            GROUP BY item_id, operation_name
            """,
            conn,
...
@st.cache_data(ttl=120, show_spinner=False)
def get_historical_existing_markers() -> set[str]:
    conn = get_db_connection()
    try:
        rows = conn.execute(
            """
            SELECT COALESCE(notlar, '')
            FROM operation_batches
            WHERE notlar LIKE '%[HIST-%'
            """
        ).fetchall()
    finally:
        conn.close()
    ...
    found = set()
    for row in rows:
        found.update(re.findall(r"\[(HIST-[^\]]+)\]", str(row[0] or "")))
    return found


def _historical_oc_table(items: pd.DataFrame) -> pd.DataFrame:
    table = (
        items[["oc_no", "project_name"]]
        .drop_duplicates()
        .copy()
    )
    table["oc_no"] = table["oc_no"].astype(str)
    table["project_name"] = table["project_name"].fillna("").astype(str)
    return table.sort_values(["project_name", "oc_no"]).reset_index(drop=True)


def _historical_select_oc(
    oc_table: pd.DataFrame,
    project_label: str,
    project_hint: str,
    widget_key: str,
) -> str:
    records = oc_table.to_dict("records")
    records.sort(
        key=lambda row: (
            0 if project_hint in _normalize_header(row["project_name"]) else 1,
            _normalize_header(row["project_name"]),
            str(row["oc_no"]),
        )
    )
    labels = [
        f"OC {row['oc_no']} · {row['project_name'] or 'İsimsiz Proje'}"
        for row in records
    ]
    label_to_oc = {
        label: str(row["oc_no"])
    ...
    selected = st.selectbox(
        f"{project_label} için OC",
        labels,
        key=widget_key,
    )
    return label_to_oc[selected]


def _historical_sort_items(items: pd.DataFrame) -> pd.DataFrame:
    result = items.copy()
    result["_pos_no"] = (
        result["pos"].astype(str).str.extract(r"(\d+)")[0]
        .fillna(0).astype(int)
    )
    return result.sort_values(["_pos_no", "pos"]).drop(columns=["_pos_no"])


def _historical_progress_map() -> dict[tuple[int, str], int]:
    totals = get_historical_operation_totals()
    progress = {}
    if totals.empty:
        return progress
    for _, row in totals.iterrows():
        key = (int(row["item_id"]), _historical_stage_key(row["operation_name"]))
        progress[key] = progress.get(key, 0) + int(row["good_qty"] or 0)
    return progress


def _historical_allocate_quantity(
    items: pd.DataFrame,
    quantity: int,
    stage_keys: list[str],
    progress: dict[tuple[int, str], int],
) -> tuple[list[dict], int]:
    """Miktarı POS sipariş kapasitesine göre dağıtır; gerekirse son POS'ta fazla üretime izin verir."""
    ordered = _historical_sort_items(items)
    if ordered.empty:
        raise ValueError("Seçilen OC içinde POS bulunamadı.")

    remaining = int(quantity)
    allocations = []
    for _, item in ordered.iterrows():
        item_id = int(item["item_id"])
        requested = int(item["requested_qty"] or 0)
        capacities = [
            max(requested - int(progress.get((item_id, stage), 0)), 0)
            for stage in stage_keys
        ]
        capacity = min(capacities) if capacities else requested
        take = min(remaining, capacity)
        if take > 0:
            allocations.append({
                "item_id": item_id,
                "oc_no": str(item["oc_no"]),
                "project_name": str(item["project_name"]),
                "pos": str(item["pos"]),
                "requested_qty": requested,
                "unit_area_mm2": float(item["unit_area_mm2"] or 0),
                "quantity": int(take),
            })
            remaining -= int(take)
        if remaining <= 0:
    ...
    if remaining > 0:
        ...
        existing = next(
            (row for row in allocations if row["item_id"] == item_id),
            None,
        )
        if existing is None:
            existing = {
                "item_id": item_id,
                "oc_no": str(last["oc_no"]),
                "project_name": str(last["project_name"]),
                "pos": str(last["pos"]),
                "requested_qty": int(last["requested_qty"] or 0),
                "unit_area_mm2": float(last["unit_area_mm2"] or 0),
                "quantity": 0,
            }
            allocations.append(existing)
        existing["quantity"] += int(remaining)
        overproduction = int(remaining)
        remaining = 0

    for allocation in allocations:
        for stage in stage_keys:
            key = (int(allocation["item_id"]), stage)
            progress[key] = progress.get(key, 0) + int(allocation["quantity"])

    return allocations, overproduction


def _historical_fixed_pos_allocation(
    items: pd.DataFrame,
    fixed_pos: str,
    quantity: int,
    stage_key: str,
    ...
    match = items[items["pos"].astype(str).str.upper() == str(fixed_pos).upper()]
    if match.empty:
        raise ValueError(
            f"Seçilen OC içinde {fixed_pos} bulunamadı. Doğru Heraklion OC'sini seçin."
        )
    item = match.iloc[0]
    item_id = int(item["item_id"])
    requested = int(item["requested_qty"] or 0)
    current = int(progress.get((item_id, stage_key), 0))
    overproduction = max(current + int(quantity) - requested, 0)
    progress[(item_id, stage_key)] = current + int(quantity)
    return [{
        "item_id": item_id,
        "oc_no": str(item["oc_no"]),
        "project_name": str(item["project_name"]),
...
def _historical_split_hours(allocations: list[dict], total_hours: float) -> list[float]:
    total_qty = sum(int(row["quantity"]) for row in allocations)
    if total_qty <= 0:
        return [0.0 for _ in allocations]
    ...
    for index, allocation in enumerate(allocations):
        if index == len(allocations) - 1:
            value = round(float(total_hours) - used, 4)
        else:
            value = round(
                float(total_hours) * int(allocation["quantity"]) / total_qty,
                4,
            )
            used += value
        hours.append(max(value, 0.0))
    return hours


def _historical_load_plan_map(item_ids: set[int]) -> dict[int, dict]:
    if not item_ids:
        return {}
    conn = get_db_connection()
    try:
...
def _historical_prepare_item_plan(
    base_plan: dict | None,
    required_stage: str,
) -> tuple[dict, str]:
    operations = list((base_plan or {}).get("operations", []))
    combination_name = str((base_plan or {}).get("combination_name", "") or "")

    normalized = [_normalize_header(operation) for operation in operations]
    if required_stage == "cutting" and not any(value == "kesim" for value in normalized):
        grinding_index = next(
            (index for index, value in enumerate(normalized) if "taslama" in value),
            len(operations),
        )
        operations.insert(grinding_index, "Kesim")
    if required_stage == "grinding" and not any("taslama" in value for value in normalized):
        if not any(value == "kesim" for value in normalized):
            operations.append("Kesim")
        operations.append("Taşlama")

    operations = ensure_cutting_before_grinding(operations)
    if not operations:
        operations = ["Kesim", "Taşlama"]

    operation_name = ""
    if required_stage == "cutting":
        operation_name = next(
            (
                operation for operation in operations
                if _normalize_header(operation) == "kesim"
            ),
            "Kesim",
        )
    else:
    ...
    if not combination_name:
        combination_name = combination_name_from_operations(operations)
    return {
...
def build_july_2026_historical_batches(
    all_items: pd.DataFrame,
    selected_ocs: dict[str, str],
) -> tuple[list[dict], pd.DataFrame, list[str]]:
    project_items = {
        key: all_items[all_items["oc_no"].astype(str) == str(oc)].copy()
        for key, oc in selected_ocs.items()
    }
    progress = _historical_progress_map()
    linked_cache = {}
    linked_overproduction = {}
    warnings = []
    draft_records = []

    linked_stages = {}
    for preset in JULY_2026_HISTORICAL_PRESETS:
        link = preset.get("allocation_link")
        if link:
            linked_stages.setdefault(link, [])
            stage = preset["operation_stage"]
            if stage not in linked_stages[link]:
                linked_stages[link].append(stage)

    for preset in JULY_2026_HISTORICAL_PRESETS:
        items = project_items[preset["project_key"]]
        link = preset.get("allocation_link")
        if preset.get("fixed_pos"):
            allocations, overproduction = _historical_fixed_pos_allocation(
                items,
                preset["fixed_pos"],
                preset["quantity"],
                preset["operation_stage"],
                progress,
            )
        elif link:
            if link not in linked_cache:
                linked_cache[link], linked_overproduction[link] = (
                    _historical_allocate_quantity(
                        items,
                        preset["quantity"],
                        linked_stages[link],
                        progress,
            ...
            allocations = [dict(row) for row in linked_cache[link]]
            overproduction = linked_overproduction[link]
        else:
            allocations, overproduction = _historical_allocate_quantity(
                items,
                preset["quantity"],
                [preset["operation_stage"]],
                progress,
        ...
        if overproduction > 0:
            warnings.append(
                f"{preset['date']} · {preset['worker']} · {preset['project_label']}: "
                f"{overproduction} adet mevcut sipariş kapasitesini aşıyor; son POS'a fazla üretim olarak dağıtıldı."
            )
    ...
    base_plans = _historical_load_plan_map(all_item_ids)
    existing_markers = get_historical_existing_markers()
    batches = []
    preview_rows = []
    ...
    for record in draft_records:
        ...
        allocated_hours = _historical_split_hours(allocations, preset["hours"])
        entries = []
        plans = {}

        for allocation, operation_hours in zip(allocations, allocated_hours):
            item_id = int(allocation["item_id"])
            plan, operation_name = _historical_prepare_item_plan(
                base_plans.get(item_id),
                preset["operation_stage"],
            )
            plans[item_id] = plan
            quantity = int(allocation["quantity"])
            unit_area = float(allocation["unit_area_mm2"] or 0)
            entries.append({
                "item_id": item_id,
                "operation_name": operation_name,
                "processed_qty": quantity,
                "fire_qty": 0,
                "operation_hours": float(operation_hours),
                "unit_area_mm2": unit_area,
                "calculated_area_mm2": quantity * unit_area,
                "fire_note": "",
            })
            preview_rows.append({
                "Tarih": preset["date"],
                "Çalışan": preset["worker"],
                "Proje": preset["project_label"],
                "OC": allocation["oc_no"],
                "POS": allocation["pos"],
                "İşlem": operation_name,
                "Adet": quantity,
                "Saat": round(float(operation_hours), 2),
                "Durum": (
                    "Zaten kayıtlı"
                    if preset["marker"] in existing_markers
                    else "Kayda hazır"
                ),
            })
        ...
        batches.append({
            ...
            "batch_data": {
                "tarih": preset["date"],
                "operator_ismi": preset["worker"],
                "calisma_tipi": "Mesaili",
                "calisma_saati": float(preset["hours"]),
                "neden": "Geçmiş üretim kaydı",
                "notlar": (
                    f"[{preset['marker']}] Kullanıcı tarafından bildirilen "
                    f"21-23 Temmuz 2026 geçmiş üretim kaydı · {preset['project_label']}"
                ),
            },
            "entries": entries,
            "plans": plans,
            "already_exists": preset["marker"] in existing_markers,
        })

    preview = pd.DataFrame(preview_rows)
    return batches, preview, list(dict.fromkeys(warnings))


def save_july_2026_historical_batches(batches: list[dict]) -> dict:
    saved = 0
    skipped = 0
    failed = []
    for batch in batches:
        ...
        try:
            ...
            failed.append(f"{batch['marker']}: {exc}")
    get_historical_existing_markers.clear()
    get_historical_operation_totals.clear()
    _clear_data_caches()
    return {"saved": saved, "skipped": skipped, "failed": failed}


def render_july_2026_historical_import():
    """Yönetici kenar çubuğunda tarihsel kayıtları kontrollü biçimde hazırlar ve kaydeder."""
    st.markdown("**21–23 Temmuz hazır kayıtlar**")
    st.caption(
        "POS'u bilinmeyen adetler seçilen OC'nin POS'larına sırayla dağıtılır. "
        "Eşleştirilen kesim ve taşlama kayıtlarında POS dağılımı aynıdır."
    )

    all_items = get_production_output_summary()
    if all_items.empty:
        st.warning("Önce Neo Vac, Heraklion ve ISS üretim çıktılarını sisteme yükleyin.")
        return

    oc_table = _historical_oc_table(all_items)
    if oc_table.empty:
        st.warning("Seçilebilecek OC bulunamadı.")
        return

    neo_vac_oc = _historical_select_oc(
        oc_table, "Neo Vac", "neovac", "historical_neo_vac_oc"
    )
    heraklion_oc = _historical_select_oc(
        oc_table, "Heraklion", "heraklion", "historical_heraklion_oc"
    )
    iss_oc = _historical_select_oc(
        oc_table, "ISS", "iss", "historical_iss_oc"
    )

    try:
        batches, preview, warnings = build_july_2026_historical_batches(
            all_items,
            {
                "neo_vac": neo_vac_oc,
                "heraklion": heraklion_oc,
                "iss": iss_oc,
            },
        )
    except Exception as exc:
        st.error(f"Hazır kayıtlar oluşturulamadı: {exc}")
        return
    ...
    ready_batches = sum(not batch["already_exists"] for batch in batches)
    existing_batches = sum(batch["already_exists"] for batch in batches)
    st.caption(
        f"{ready_batches} kayıt kayda hazır · {existing_batches} kayıt daha önce eklenmiş."
    )
    confirm = st.checkbox(
        "Ön izlemeyi kontrol ettim",
        key="historical_july_confirm",
    )
    if st.button(
        "Hazır Kayıtları Sisteme Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm or ready_batches == 0,
        key="historical_july_save",
    ):
        result = save_july_2026_historical_batches(batches)
        if result["saved"]:
            st.success(f"{result['saved']} geçmiş kayıt sisteme kaydedildi.")
        if result["skipped"]:
            st.info(f"{result['skipped']} kayıt zaten bulunduğu için tekrar eklenmedi.")
...
def main():
    ...
    st.caption("Sürüm 2.18.0 · HAZIR GEÇMİŞ KAYITLAR")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.18.0 — 21–23 TEMMUZ HAZIR KAYIT AKTARIMI
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.18.0 · HAZIR GEÇMİŞ KAYITLAR
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if is_manager:
            ...
            if st.toggle(
                "21–23 Temmuz hazır kayıtlarını aç",
                value=False,
                key="lazy_historical_july_import",
                help="Sercan ve Otabek Bey'in bildirilen geçmiş kesim/taşlama kayıtlarını ön izleyip kaydet.",
            ):
                render_july_2026_historical_import()

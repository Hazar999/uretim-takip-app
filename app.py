...
def _clear_data_caches():
    ...
    cached_readers = (
        ...
        "get_project_completed_output_daily",
...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS production_output_items (
            ...
            abkant_long_bend_type TEXT NOT NULL DEFAULT '',
            abkant_short_bend_type TEXT NOT NULL DEFAULT '',
            UNIQUE(oc_no, pos)
        )
        """
    )
    ...
    if "abkant_long_bend_type" not in output_item_columns:
        cur.execute(
            "ALTER TABLE production_output_items "
            "ADD COLUMN abkant_long_bend_type TEXT NOT NULL DEFAULT ''"
        )
    if "abkant_short_bend_type" not in output_item_columns:
        cur.execute(
            "ALTER TABLE production_output_items "
            "ADD COLUMN abkant_short_bend_type TEXT NOT NULL DEFAULT ''"
        )
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS operation_work_logs (
            ...
            laser_edge_mode TEXT NOT NULL DEFAULT 'Uzun + kısa kenar',
            ...
            counts_toward_progress INTEGER NOT NULL DEFAULT 1,
            fire_note TEXT,
            created_at TEXT NOT NULL,
    ...
    # Taşlama bulunan eski POS planlarını da Kesim > Taşlama akışına geçirir.
    migrate_existing_grinding_plans(cur)

    # Eski sürümlerde uzun/kısa sütununa ters yazılmış lazer kayıtlarını düzeltir.
    repair_misclassified_laser_edges(cur)
...
def ensure_cutting_before_grinding(operations):
    """
    Taşlama bulunan kombinasyonda Kesim operasyonunu taşlamanın hemen önüne koyar.

    - Kesim yoksa otomatik ekler.
    - Kesim yanlış sıradaysa taşlamanın önüne taşır.
    - Birden fazla Kesim varsa tek kayda indirir.
    - Taşlama yoksa mevcut operasyon sırasına dokunmaz.
    """
    cleaned_operations = [
        " ".join(str(operation or "").split())
        for operation in operations
    ...
    normalized_operations = [
        _normalize_header(operation) for operation in cleaned_operations
    ]
    if not any("taslama" in operation for operation in normalized_operations):
    ...
    cutting_label = next(
        (
            ...
            for operation, normalized in zip(
                cleaned_operations, normalized_operations
            )
            if normalized == "kesim"
        ),
        "Kesim",
    ...
    # Kesim'i bulunduğu eski konumdan kaldırıp ilk taşlamanın hemen önüne koyar.
    without_cutting = [
        operation
        for operation, normalized in zip(
            cleaned_operations, normalized_operations
    ...
    grinding_index = next(
        index
        for index, operation in enumerate(without_cutting)
        if "taslama" in _normalize_header(operation)
    )
    without_cutting.insert(grinding_index, cutting_label)
    return without_cutting


def migrate_existing_grinding_plans(cur) -> int:
    """Daha önce kaydedilmiş POS planlarına da Kesim > Taşlama sırasını uygular."""
    rows = cur.execute(
        """
        SELECT item_id, combination_name, operation_order, operation_name
        FROM pos_operation_plan
        ORDER BY item_id, operation_order, id
        """
    ).fetchall()
    if not rows:
        return 0
    ...
    grouped_plans = {}
    for item_id, combination_name, _, operation_name in rows:
        item_id = int(item_id)
        plan = grouped_plans.setdefault(
            item_id,
            {
                "combination_name": str(combination_name or ""),
                "operations": [],
            },
        )
        plan["operations"].append(str(operation_name or ""))

    migrated_count = 0
    now = datetime.now().isoformat(timespec="seconds")
    for item_id, plan in grouped_plans.items():
        previous_operations = plan["operations"]
        updated_operations = ensure_cutting_before_grinding(previous_operations)
        if updated_operations == previous_operations:
            continue

        updated_combination_name = combination_name_from_operations(
            updated_operations
        )
        cur.execute(
        ...
        cur.executemany(
            ...
            [
                (
                    item_id,
                    updated_combination_name,
                    operation_order,
                    operation_name,
                    now,
                )
                for operation_order, operation_name in enumerate(
                    updated_operations,
                    start=1,
                )
            ],
        ...
        cur.execute(
            """
            UPDATE production_output_items
            SET combination_name = ?
            WHERE id = ?
            """,
            (updated_combination_name, item_id),
        )
        migrated_count += 1
...
def load_config_from_excel_bytes(file_bytes: bytes):
    ...
    for _, row in operation_area.iterrows():
        ...
        ops = ensure_cutting_before_grinding(ops)
    ...
    if not combinations:
        default_operations = ensure_cutting_before_grinding([
            "laser",
            "abkant",
            "çıta",
            "kaynak",
...
def get_production_output_summary(oc_no: str | None = None) -> pd.DataFrame:
    ...
    try:
        query = """
            SELECT
                ...
                COALESCE(i.abkant_long_bend_type, '') AS abkant_long_bend_type,
                COALESCE(i.abkant_short_bend_type, '') AS abkant_short_bend_type,
                i.source_name,
                i.imported_at,
                COALESCE(
                    SUM(
    ...
    # Nihai tamamlanma, planda Sevkiyat varsa doğrudan Sevkiyat'ın sağlam
    # toplamından gelir. Böylece daha önceki etaplardan biri eksik görünse bile
    # sevk edilen ürün tamamlanmış sayılır ve kalan sipariş adedi düşer.
    progress = get_operation_progress(result["item_id"].astype(int).tolist())
    final_completed = _final_completed_qty_map(progress)
    if final_completed:
        operation_completed = result["item_id"].map(final_completed)
        result["produced_qty"] = (
            operation_completed.where(operation_completed.notna(), result["produced_qty"])
            .fillna(0)
            .astype(int)
        )
...
@st.cache_data(ttl=300, show_spinner=False)
def get_project_unique_production_daily(
    oc_no: str,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """
    Aynı parçayı operasyon sayısı kadar çoğaltmadan günlük üretime giren alanı döndürür.

    Bir POS için o güne kadar herhangi bir operasyonda ulaşılan en yüksek sağlam adet
    esas alınır. Aynı adet lazerden sonra abkant, kaynak veya boyaya geçtiğinde ikinci
    kez sayılmaz; yalnızca önceki en yüksek adedi aşan kısım günlük üretime eklenir.
    """
    conn = get_db_connection()
    try:
        logs = pd.read_sql_query(
            ...
            WHERE i.oc_no = ?
              AND b.tarih <= ?
            ORDER BY b.tarih, w.id
            """,
            conn,
            params=(str(oc_no), str(end_date)),
        )
    finally:
        conn.close()

    empty = pd.DataFrame(columns=["tarih_dt", "günlük_m2", "sağlam_adet"])
    if logs.empty:
        return empty

    logs["tarih_dt"] = pd.to_datetime(logs["tarih"], errors="coerce").dt.normalize()
    logs["good_qty"] = pd.to_numeric(logs["good_qty"], errors="coerce").fillna(0).clip(lower=0)
    logs["unit_area_mm2"] = pd.to_numeric(
        logs["unit_area_mm2"], errors="coerce"
    ).fillna(0)
    logs = logs.dropna(subset=["tarih_dt"])
    if logs.empty:
        return empty

    daily_operation = (
        logs.groupby(
            ["tarih_dt", "item_id", "operation_name", "unit_area_mm2"],
            as_index=False,
        )["good_qty"]
        .sum()
        .sort_values(["tarih_dt", "item_id", "operation_name"])
    )

    cumulative_by_operation: dict[tuple[int, str], float] = {}
    previous_unique_by_item: dict[int, float] = {}
    item_area: dict[int, float] = {}
    output_rows = []
    start_ts = pd.Timestamp(start_date).normalize()
    end_ts = pd.Timestamp(end_date).normalize()

    for current_date, day_rows in daily_operation.groupby("tarih_dt", sort=True):
        affected_items: set[int] = set()
        for row in day_rows.itertuples(index=False):
            item_id = int(row.item_id)
            operation_name = str(row.operation_name)
            key = (item_id, operation_name)
            cumulative_by_operation[key] = (
                cumulative_by_operation.get(key, 0.0) + float(row.good_qty or 0)
            )
            item_area[item_id] = float(row.unit_area_mm2 or 0)
        ...
        for item_id in affected_items:
            current_unique = max(
                (
                    quantity
                    for (saved_item_id, _), quantity in cumulative_by_operation.items()
                    if saved_item_id == item_id
                ),
                default=0.0,
            )
            previous_unique = previous_unique_by_item.get(item_id, 0.0)
            increase = max(current_unique - previous_unique, 0.0)
            previous_unique_by_item[item_id] = max(previous_unique, current_unique)

            if start_ts <= current_date <= end_ts and increase > 0:
                day_qty += increase
                day_area_mm2 += increase * item_area.get(item_id, 0.0)

        if start_ts <= current_date <= end_ts and (day_qty > 0 or day_area_mm2 > 0):
            output_rows.append({
                "tarih_dt": current_date,
                "günlük_m2": round(day_area_mm2 / 1_000_000, 1),
                "sağlam_adet": int(round(day_qty)),
            })

    if not output_rows:
        return empty
    return pd.DataFrame(output_rows).sort_values("tarih_dt").reset_index(drop=True)
...
def _operation_kind(operation_name: str) -> str:
    ...
    if "boya" in normalized:
        return "boya"
    if "taslama" in normalized:
        return "taslama"
    if "paket" in normalized:
        return "paketleme"
    if "sevkiyat" in normalized:
        return "sevkiyat"
    if normalized == "kesim" or "kesim" in normalized:
        return "kesim"
    return "other"


AUXILIARY_WORK_BY_OPERATION_KIND = {
    "taslama": "Çıta taşlama",
    "boya": "Çapak alma",
    "abkant": "Zımpara",
}


def _auxiliary_work_label(operation_name: str) -> str:
    """Operasyon içinde üretim adedini ilerletmeyen yardımcı işi döndürür."""
    return AUXILIARY_WORK_BY_OPERATION_KIND.get(
        _operation_kind(operation_name),
        "",
    )


def _work_detail_options(operation_name: str) -> list[str]:
    auxiliary_label = _auxiliary_work_label(operation_name)
    return ["Ana üretim", auxiliary_label] if auxiliary_label else []


def _normalize_work_detail(operation_name: str, value) -> str:
    auxiliary_label = _auxiliary_work_label(operation_name)
    raw = " ".join(str(value or "").split())
    if auxiliary_label and _normalize_header(raw) == _normalize_header(auxiliary_label):
        return auxiliary_label
    return "Ana üretim"


def _entry_counts_toward_progress(entry: dict) -> bool:
    """Yardımcı iş adetlerinin POS üretim ilerlemesine karışmasını engeller."""
    operation_name = str(entry.get("operation_name", "") or "")
    work_detail = _normalize_work_detail(
        operation_name,
        entry.get("work_detail", ""),
    )
    auxiliary_label = _auxiliary_work_label(operation_name)
    if auxiliary_label and work_detail == auxiliary_label:
        return False

    stored_value = entry.get("counts_toward_progress", 1)
    if isinstance(stored_value, str):
        normalized = _normalize_header(stored_value)
        if normalized in {"0", "hayir", "false", "no"}:
            return False
        if normalized in {"1", "evet", "true", "yes"}:
            return True
    return bool(stored_value)


def _completion_operation_from_plan(operations: list[str]) -> str:
    """POS'u tamamen bitiren operasyonu belirler.

    Planda Sevkiyat varsa, listedeki diğer ara etapların miktarından bağımsız
    olarak nihai tamamlanan adet Sevkiyat'ın sağlam adedidir. Sevkiyat yoksa
    planın son operasyonu tamamlanma etabı kabul edilir.
    """
    cleaned = [
        " ".join(str(operation or "").split())
        for operation in operations
    ...
    shipment_operations = [
        ...
        if _operation_kind(operation) == "sevkiyat"
    ]
    return shipment_operations[-1] if shipment_operations else cleaned[-1]


def _final_completed_qty_map(progress: pd.DataFrame) -> dict[int, int]:
    """Etap ilerlemesinden POS bazında nihai tamamlanan adetleri çıkarır."""
    if progress is None or progress.empty:
        return {}

    completed_map: dict[int, int] = {}
    for item_id, group in progress.groupby("item_id", sort=False):
        ordered = group.sort_values(["operation_order", "operation_name"])
        shipment_rows = ordered[
            ordered["operation_name"].map(_operation_kind).eq("sevkiyat")
        ]
        final_row = (
            shipment_rows.iloc[-1]
            if not shipment_rows.empty
            else ordered.iloc[-1]
        )
        completed_map[int(item_id)] = max(
            int(final_row.get("completed_qty", 0) or 0),
            0,
        )
    return completed_map


LASER_EDGE_MODE_OPTIONS = [
    "Uzun + kısa kenar",
    "Yalnız uzun kenar",
    "Yalnız kısa kenar",
]
...
def _normalize_laser_edge_mode(value) -> str:
    """Lazer için POS'ta gerekli kenar kapsamını standart ifadeye çevirir."""
    normalized = _normalize_header(value)
    if normalized in {
        "yalnizuzunkenar", "yalnzuzunkenar", "sadeceuzunkenar",
        "uzunkenar", "long", "longonly"
    }:
        return "Yalnız uzun kenar"
    if normalized in {
        "yalnizkisakenar", "yalnzksakenar", "sadecekisakenar",
        "sadeceksakenar", "kisakenar", "ksakenar", "short", "shortonly"
    }:
        return "Yalnız kısa kenar"
    return "Uzun + kısa kenar"


def _laser_required_edges(edge_mode: str) -> tuple[bool, bool]:
    """Bugün giriş yapılacak uzun/kısa kenar alanlarını belirler.

    Bu seçim tam takımın içeriğini değiştirmez. Tam panel her zaman hem uzun hem
    kısa kenarın birikmiş miktarları birlikte yeterli olduğunda oluşur.
    """
    normalized_mode = _normalize_laser_edge_mode(edge_mode)
    return (
        normalized_mode != "Yalnız kısa kenar",
        normalized_mode != "Yalnız uzun kenar",
    )


def _laser_completed_qty_from_edges(
    total_long_edge_qty: int,
    total_short_edge_qty: int,
    edge_mode: str,
    long_per_team: int,
    short_per_team: int,
    requested_qty: int,
) -> int:
    """Biriken uzun ve kısa kenarlardan oluşan net tam panel/takım adedini bulur.

    ``edge_mode`` yalnızca bugün hangi kenar girişlerinin açık olduğunu belirtir;
    tamamlanan takım hesabında her zaman uzun ve kısa kenar birlikte aranır.
    Böylece önceki gün üretilen kısa kenarlar, bugün üretilen uzun kenarlarla
    otomatik eşleşir.
    """
    del edge_mode  # Giriş kapsamıdır; tam takım hesabını değiştirmez.
    long_team_qty = (
        max(int(total_long_edge_qty or 0), 0)
        // max(int(long_per_team or 1), 1)
    )
    short_team_qty = (
        max(int(total_short_edge_qty or 0), 0)
        // max(int(short_per_team or 1), 1)
    )
    return min(
        long_team_qty,
        short_team_qty,
        max(int(requested_qty or 0), 0),
    )


def _laser_edge_work_equivalent_qty(
    long_edge_qty: int,
    short_edge_qty: int,
    long_per_team: int,
    short_per_team: int,
) -> float:
    """Bugün kesilen kenarların panel/takım karşılığı iş miktarını döndürür.

    Yalnız uzun veya yalnız kısa kenar üretilse bile yapılan iş m² hesabında
    görünür. İki taraf birlikte üretilmişse aynı panel iki kez sayılmasın diye
    büyük olan takım karşılığı kullanılır.
    """
    long_equivalent = (
        max(int(long_edge_qty or 0), 0)
        / max(int(long_per_team or 1), 1)
    )
    short_equivalent = (
        max(int(short_edge_qty or 0), 0)
        / max(int(short_per_team or 1), 1)
    )
    return max(long_equivalent, short_equivalent)



def repair_misclassified_laser_edges(cur) -> int:
    """Eski sürümlerde ters sütuna yazılan tek-kenar lazer kayıtlarını onarır.

    Güvenli onarım yalnızca kayıt kendi kenar moduyla açıkça çelişiyorsa yapılır:
    - ``Yalnız uzun kenar`` kaydında uzun 0, kısa > 0 ise miktar uzuna taşınır.
    - ``Yalnız kısa kenar`` kaydında kısa 0, uzun > 0 ise miktar kısaya taşınır.

    Bu tür tek-kenar kaydı tam panel oluşturmadığından eski processed/good değerleri
    sıfırlanır. Daha önce yanlış düşürülen POS toplamı için de fark kadar otomatik
    düzeltme hareketi yazılır; önceki kayıt geçmişi silinmez.
    """
    rows = cur.execute(
        """
        SELECT
            ...
            w.batch_id,
            w.laser_edge_mode,
            COALESCE(w.laser_long_edge_qty, 0),
            COALESCE(w.laser_short_edge_qty, 0),
            COALESCE(w.laser_long_per_team, 2),
            COALESCE(w.laser_short_per_team, 2),
            COALESCE(i.unit_area_mm2, 0)
        FROM operation_work_logs w
        INNER JOIN production_output_items i ON i.id = w.item_id
        WHERE
            (
                w.laser_edge_mode = 'Yalnız uzun kenar'
                AND COALESCE(w.laser_long_edge_qty, 0) = 0
                AND COALESCE(w.laser_short_edge_qty, 0) > 0
            )
            OR
            (
                w.laser_edge_mode = 'Yalnız kısa kenar'
                AND COALESCE(w.laser_short_edge_qty, 0) = 0
                AND COALESCE(w.laser_long_edge_qty, 0) > 0
            )
        """
    ...
    for (
        ...
        long_qty = max(int(long_qty or 0), 0)
        short_qty = max(int(short_qty or 0), 0)
        long_per_team = max(int(long_per_team or 2), 1)
        short_per_team = max(int(short_per_team or 2), 1)

        if str(edge_mode) == "Yalnız uzun kenar":
            corrected_long_qty = short_qty
            corrected_short_qty = 0
        else:
            corrected_long_qty = 0
            corrected_short_qty = long_qty

        work_equivalent = _laser_edge_work_equivalent_qty(
            corrected_long_qty,
            corrected_short_qty,
            long_per_team,
            short_per_team,
        )
        corrected_area_mm2 = work_equivalent * float(unit_area_mm2 or 0)

        cur.execute(
            """
            UPDATE operation_work_logs
            SET laser_long_edge_qty = ?,
                laser_short_edge_qty = ?,
                processed_qty = 0,
                good_qty = 0,
                fire_qty = 0,
                calculated_area_mm2 = ?
        ...
        affected_item_ids.add(int(item_id))

    # Ters sınıflandırma yüzünden daha önce düşmüş POS toplamını gerçek operasyon
    # ilerlemesine eşitle. Geçmişi silmek yerine yalnızca fark hareketi eklenir.
    now = datetime.now().isoformat(timespec="seconds")
    ...
    for item_id in sorted(affected_item_ids):
        ...
        if item_row is None:
            continue
        requested_qty, unit_area_mm2 = item_row
        requested_qty = max(int(requested_qty or 0), 0)
        unit_area_mm2 = float(unit_area_mm2 or 0)
        ...
        for operation_name in planned_operations:
            if _operation_kind(operation_name) == "laser":
                laser_row = cur.execute(
                    """
                    SELECT
                        COALESCE(SUM(laser_long_edge_qty), 0),
                        COALESCE(SUM(laser_short_edge_qty), 0),
                        COALESCE(MAX(laser_long_per_team), 2),
                        COALESCE(MAX(laser_short_per_team), 2),
                        COALESCE(SUM(fire_qty), 0),
                        COALESCE(MAX(NULLIF(laser_edge_mode, '')), 'Uzun + kısa kenar')
                ...
                completed = _laser_completed_qty_from_edges(
                    int(total_long or 0),
                    int(total_short or 0),
                    str(stored_mode or "Uzun + kısa kenar"),
                    int(long_per_team or 2),
                    int(short_per_team or 2),
                    requested_qty,
                )
                stage_good = max(completed - int(total_fire or 0), 0)
            else:
                stage_good = int(
                    cur.execute(
                        """
            ...
            stage_totals.append(stage_good)

        actual_completed = min(stage_totals) if stage_totals else 0
        logged_completed = int(
            cur.execute(
                """
                SELECT COALESCE(SUM(produced_qty), 0)
                FROM production_output_logs
                WHERE item_id = ? AND source_type = 'operation_tracking'
                """,
                (item_id,),
            ).fetchone()[0]
            or 0
        )
        correction_qty = int(actual_completed) - int(logged_completed)
        if correction_qty == 0:
            continue
        ...
        cur.execute(
            ...
            (
                ...
                "Sistem düzeltmesi",
                correction_qty,
                correction_qty * unit_area_mm2,
                "Lazer uzun/kısa kenar sınıflandırması otomatik düzeltildi.",
                now,
                "operation_tracking",
                None,
            ),
...
ABKANT_OPERATION_TYPE_OPTIONS = [
    "Kenar bükümü",
    "Mesh bükümü",
]


ABKANT_BEND_REQUIREMENT_OPTIONS = [
    "Seçin",
    "Yok",
    "Tek büküm",
    "Çift büküm",
...
def _normalize_abkant_operation_type(value) -> str:
    """Abkant kaydının kenar bükümü mü mesh bükümü mü olduğunu döndürür."""
    normalized = _normalize_header(value)
...
def _normalize_abkant_bend_requirement(value) -> str:
    """Abkant kenar gereksinimini uygulamadaki standart ifadeye çevirir."""
    normalized = _normalize_header(value)
    if normalized in {"yok", "yapilmayacak", "bukumyok"}:
        return "Yok"
    if normalized in {"tek", "tekbukum", "single", "singlebend"}:
        return "Tek büküm"
    if normalized in {"cift", "ciftbukum", "double", "doublebend"}:
        return "Çift büküm"
    return "Seçin"


def _abkant_qty_for_requirement(
    single_qty: int,
    double_qty: int,
    requirement: str,
) -> int:
    """Seçilen büküm türüne ait kenar adedini döndürür."""
    normalized_requirement = _normalize_abkant_bend_requirement(requirement)
    if normalized_requirement == "Tek büküm":
        return max(int(single_qty or 0), 0)
    if normalized_requirement == "Çift büküm":
        return max(int(double_qty or 0), 0)
    return 0


def _abkant_completed_qty(
    long_single_qty: int,
    long_double_qty: int,
    short_single_qty: int,
    short_double_qty: int,
    long_requirement: str,
    short_requirement: str,
    requested_qty: int,
) -> int:
    """Projede gerekli kenarlara göre tamamlanan panel adedini hesaplar."""
    required_edge_totals = []
    normalized_long = _normalize_abkant_bend_requirement(long_requirement)
    normalized_short = _normalize_abkant_bend_requirement(short_requirement)

    if normalized_long in {"Tek büküm", "Çift büküm"}:
        required_edge_totals.append(
            _abkant_qty_for_requirement(
                long_single_qty, long_double_qty, normalized_long
            )
        )
    if normalized_short in {"Tek büküm", "Çift büküm"}:
        required_edge_totals.append(
            _abkant_qty_for_requirement(
                short_single_qty, short_double_qty, normalized_short
            )
        )

    if not required_edge_totals:
        return 0
    return min(min(required_edge_totals), max(int(requested_qty or 0), 0))


def _abkant_completed_qty_from_teams(
    total_team_qty: int,
    teams_per_piece: int,
    requested_qty: int,
) -> int:
    """Toplam takım sayısını ürün adedine çevirir.

    Örnek: 24 takım ve adet başına 2 takım varsa 12 adet tamamlanır.
    Sipariş adedinin üzerinde ilerleme yazılmaması için sonuç istenen adetle sınırlandırılır.
    """
    safe_team_qty = max(int(total_team_qty or 0), 0)
    safe_teams_per_piece = max(int(teams_per_piece or 1), 1)
    safe_requested_qty = max(int(requested_qty or 0), 0)
    return min(safe_team_qty // safe_teams_per_piece, safe_requested_qty)


def _abkant_completed_qty_from_edge_teams(
    total_long_team_qty: int,
    total_short_team_qty: int,
    long_requirement: str,
    short_requirement: str,
    teams_per_piece: int,
    requested_qty: int,
) -> int:
    """Uzun/kısa takım sayılarını adet başına takım sayısına göre ürüne çevirir.

    Örnek: uzun 24 takım, kısa 28 takım ve adet başına 2 takım ise uzun kenar
    12 adet, kısa kenar 14 adet karşılar. İki kenar da gerekli olduğunda sağlam
    ilerleyen miktar düşük olan 12 adettir.
    """
    required_piece_totals = []
    normalized_long = _normalize_abkant_bend_requirement(long_requirement)
    normalized_short = _normalize_abkant_bend_requirement(short_requirement)
    safe_teams_per_piece = max(int(teams_per_piece or 1), 1)

    if normalized_long in {"Tek büküm", "Çift büküm"}:
        long_teams = max(int(total_long_team_qty or 0), 0)
        required_piece_totals.append(long_teams // safe_teams_per_piece)
    if normalized_short in {"Tek büküm", "Çift büküm"}:
        short_teams = max(int(total_short_team_qty or 0), 0)
        required_piece_totals.append(short_teams // safe_teams_per_piece)

    if not required_piece_totals:
        return 0
    return min(
        min(required_piece_totals),
        max(int(requested_qty or 0), 0),
    )


def _abkant_effective_previous_edge_teams(
    previous_long_team_qty: int,
    previous_short_team_qty: int,
    long_requirement: str,
    short_requirement: str,
    teams_per_piece: int,
    requested_qty: int,
    ...
    """Eski kayıtlarda eksik kalan kenar takım bilgisini güvenli biçimde tamamlar.

    Önceki sürümlerde abkant adedi kaydedilmiş olmasına rağmen uzun/kısa takım
    alanları boş kalmış olabilir. Bu durumda yeni girilen takım sayısı doğrudan
    önceki adetten düşülünce ekranda 0 adet görünüyordu. Eksik geçmiş adet,
    yalnızca hesaplama için sanal takım karşılığına çevrilir; veritabanındaki
    eski kayıt değiştirilmez.
    """
    safe_teams_per_piece = max(int(teams_per_piece or 1), 1)
    safe_requested_qty = max(int(requested_qty or 0), 0)
    safe_previous_processed = min(
        max(int(previous_processed or 0), 0),
        safe_requested_qty,
    )
    normalized_long = _normalize_abkant_bend_requirement(long_requirement)
    normalized_short = _normalize_abkant_bend_requirement(short_requirement)

    effective_long = max(int(previous_long_team_qty or 0), 0)
    effective_short = max(int(previous_short_team_qty or 0), 0)
    edge_completed = _abkant_completed_qty_from_edge_teams(
        effective_long,
        effective_short,
        normalized_long,
        normalized_short,
        safe_teams_per_piece,
        safe_requested_qty,
    )
    missing_previous_qty = max(safe_previous_processed - edge_completed, 0)
    virtual_team_qty = missing_previous_qty * safe_teams_per_piece

    if normalized_long in {"Tek büküm", "Çift büküm"}:
        effective_long += virtual_team_qty
    if normalized_short in {"Tek büküm", "Çift büküm"}:
        effective_short += virtual_team_qty
    ...
    st.markdown(
        ...
        <div class="area-summary">
            <div class="area-summary-item">
                <div class="area-summary-label">Üretim karşılığı alan</div>
                <div class="area-summary-value">{_format_m2(good_area_mm2)} m²</div>
                <div class="area-summary-detail">
                    Kenar üretimi dâhil yapılan işin alan karşılığı
                </div>
            </div>
            <div class="area-summary-item">
    ...
    try:
        history = pd.read_sql_query(
            ...
            SELECT
                ...
                SUM(
                    CASE
                        WHEN COALESCE(w.calculated_area_mm2, 0) > 0
                        THEN w.calculated_area_mm2
                        ELSE w.good_qty * i.unit_area_mm2
                    END
                ) AS good_area_mm2
            ...
            WHERE b.calisma_saati > 0
              AND COALESCE(w.counts_toward_progress, 1) = 1
            GROUP BY b.id, b.calisma_saati
            HAVING SUM(w.good_qty) > 0
            """,
            conn,
        )
    finally:
...
def calculate_entries_performance(
    ...
    production_entries = [
        entry for entry in entries if _entry_counts_toward_progress(entry)
    ]
    auxiliary_entries = [
        entry for entry in entries if not _entry_counts_toward_progress(entry)
    ]

    total_processed = sum(
        max(int(entry.get("processed_qty", 0) or 0), 0)
        for entry in production_entries
    )
    total_fire = sum(
        max(int(entry.get("fire_qty", 0) or 0), 0)
        for entry in production_entries
    )
    total_good = sum(
        max(int(entry.get("good_qty", 0) or 0), 0)
        for entry in production_entries
    )
    total_auxiliary_qty = sum(
        max(int(entry.get("processed_qty", 0) or 0), 0)
        for entry in auxiliary_entries
    )
    auxiliary_hours = round(
        sum(
            max(float(entry.get("operation_hours", 0) or 0), 0.0)
            for entry in auxiliary_entries
        ),
        2,
    )
    ...
    safe_hours = production_hours if production_hours > 0 else 0.0
    ...
    for entry in production_entries:
        ...
        area = float(
            entry.get(
                "calculated_area_mm2",
                good_qty * float(entry.get("unit_area_mm2", 0)),
            )
            or 0
        )
    ...
    if ratios:
        ...
        score_pct = None
        reference_source = "Kayıt yalnız yardımcı iş içeriyor"

    return {
        "total_processed": total_processed,
        "total_fire": total_fire,
        "total_good": total_good,
        "total_auxiliary_qty": total_auxiliary_qty,
        "auxiliary_hours": auxiliary_hours,
        "production_hours": production_hours,
        "entered_total_hours": round(float(work_hours or 0), 2),
...
def get_operation_progress(item_ids: list[int] | None = None) -> pd.DataFrame:
        ...
        query = f"""
            ...
            operation_totals AS (
                SELECT
                    ...
                    COALESCE(SUM(
                        CASE
                            WHEN COALESCE(counts_toward_progress, 1) = 1
                            THEN good_qty
                            ELSE 0
                        END
                    ), 0) AS new_good_qty,
                    ...
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN laser_long_edge_qty ELSE 0 END), 0) AS laser_long_edge_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN laser_short_edge_qty ELSE 0 END), 0) AS laser_short_edge_qty,
                    COALESCE(MAX(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN laser_long_per_team ELSE NULL END), 2) AS laser_long_per_team,
                    COALESCE(MAX(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN laser_short_per_team ELSE NULL END), 2) AS laser_short_per_team,
                    COALESCE(
                        MAX(CASE
                            WHEN COALESCE(counts_toward_progress, 1) = 1
                            THEN NULLIF(laser_edge_mode, '') ELSE NULL END),
                        'Uzun + kısa kenar'
                    ) AS laser_edge_mode,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_team_qty ELSE 0 END), 0) AS abkant_team_qty,
                    COALESCE(MAX(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_teams_per_piece ELSE NULL END), 1) AS abkant_teams_per_piece,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_long_bend_qty ELSE 0 END), 0)
                        AS abkant_long_team_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_short_bend_qty ELSE 0 END), 0)
                        AS abkant_short_team_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_long_single_bend_qty ELSE 0 END), 0)
                        AS abkant_long_single_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_long_double_bend_qty ELSE 0 END), 0)
                        AS abkant_long_double_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_short_single_bend_qty ELSE 0 END), 0)
                        AS abkant_short_single_qty,
                    COALESCE(SUM(CASE
                        WHEN COALESCE(counts_toward_progress, 1) = 1
                        THEN abkant_short_double_bend_qty ELSE 0 END), 0)
                        AS abkant_short_double_qty
                FROM operation_work_logs
            ...
            SELECT
                ...
                COALESCE(o.laser_long_edge_qty, 0) AS laser_long_edge_qty,
                COALESCE(o.laser_short_edge_qty, 0) AS laser_short_edge_qty,
                COALESCE(o.laser_long_per_team, 2) AS laser_long_per_team,
                COALESCE(o.laser_short_per_team, 2) AS laser_short_per_team,
                COALESCE(o.laser_edge_mode, 'Uzun + kısa kenar') AS laser_edge_mode,
                COALESCE(o.abkant_team_qty, 0) AS abkant_team_qty,
                COALESCE(o.abkant_teams_per_piece, 1) AS abkant_teams_per_piece,
                COALESCE(o.abkant_long_team_qty, 0) AS abkant_long_team_qty,
                COALESCE(o.abkant_short_team_qty, 0) AS abkant_short_team_qty,
                COALESCE(o.abkant_long_single_qty, 0)
                    AS abkant_long_single_qty,
                COALESCE(o.abkant_long_double_qty, 0)
                    AS abkant_long_double_qty,
                COALESCE(o.abkant_short_single_qty, 0)
                    AS abkant_short_single_qty,
                COALESCE(o.abkant_short_double_qty, 0)
                    AS abkant_short_double_qty
    ...
    # Eski lazer kayıtlarında yalnız uzun veya yalnız kısa kenar, yanlışlıkla
    # tamamlanmış takım olarak yazılmış olabilir. Lazer ilerlemesini kayıtlı
    # processed/good toplamından değil, birikmiş iki kenarı eşleştirerek düzelt.
    laser_mask = result["operation_name"].map(_operation_kind).eq("laser")
    for row_index in result.index[laser_mask]:
        row = result.loc[row_index]
        completed_laser_qty = _laser_completed_qty_from_edges(
            int(row.get("laser_long_edge_qty", 0) or 0),
            int(row.get("laser_short_edge_qty", 0) or 0),
            str(row.get("laser_edge_mode", "Uzun + kısa kenar") or ""),
            int(row.get("laser_long_per_team", 2) or 2),
            int(row.get("laser_short_per_team", 2) or 2),
            int(row.get("requested_qty", 0) or 0),
        )
        laser_fire_qty = max(int(row.get("fire_qty", 0) or 0), 0)
        result.at[row_index, "processed_qty"] = completed_laser_qty
        result.at[row_index, "operation_good_qty"] = max(
            completed_laser_qty - laser_fire_qty,
            0,
        )
    ...
    try:
        query = """
            SELECT
                ...
                COALESCE(NULLIF(w.work_detail, ''), 'Ana üretim') AS is_detayi,
                COALESCE(w.counts_toward_progress, 1) AS uretim_ilerlemesine_sayar,
                ...
                w.laser_edge_mode AS laser_kenar_kapsami,
                ...
                w.abkant_long_bend_type AS abkant_uzun_kenar_turu,
                w.abkant_short_bend_type AS abkant_kisa_kenar_turu,
                ...
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.processed_qty ELSE 0
                END AS islem_yapilan,
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 0
                    THEN w.processed_qty ELSE 0
                END AS yardimci_is_adedi,
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.fire_qty ELSE 0
                END AS fire,
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.good_qty ELSE 0
                END AS saglam_ilerleyen,
                i.unit_area_mm2,
                w.good_qty * i.unit_area_mm2 AS saglam_alan_mm2,
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.processed_qty * i.unit_area_mm2
                    ELSE 0
                END AS islenen_alan_mm2,
...
def get_operation_overview() -> pd.DataFrame:
    ...
    # Üretilen miktar, bir POS'un herhangi bir operasyonda ulaştığı en yüksek
    # sağlam adettir. Böylece aynı parça farklı etaplarda tekrar tekrar sayılmaz.
    overview["unique_produced_qty"] = overview[[
        "furthest_stage_qty", "produced_qty"
    ]].max(axis=1).fillna(0).astype(int)
    overview["unique_produced_area_mm2"] = (
        overview["unique_produced_qty"] * overview["unit_area_mm2"]
    )
    overview["in_production_qty"] = (
        overview["unique_produced_qty"]
    ...
    overview["production_progress_pct"] = (
        overview["unique_produced_qty"]
        / overview["requested_qty"].replace(0, 1)
        * 100
    ).round(1)
...
def save_operation_batch(batch_data: dict, entries: list[dict], plans: dict[int, dict]) -> int:
    ...
    try:
        ...
        for entry in entries:
            ...
            processed_qty = max(int(entry.get("processed_qty", 0) or 0), 0)
            fire_qty = max(int(entry.get("fire_qty", 0) or 0), 0)
            operation_kind = _operation_kind(operation_name)
            work_detail = _normalize_work_detail(
                operation_name,
                entry.get("work_detail", ""),
            )
            counts_toward_progress = int(_entry_counts_toward_progress(entry))
            operation_hours_value = max(
                float(entry.get("operation_hours", 0) or 0),
                0.0,
            )

            # Yardımcı işlerde girilen adet yalnızca yapılan iş miktarını gösterir.
            # POS üretim ilerlemesini etkilemez; süre proje işçiliğine eklenir.
            if not counts_toward_progress:
                fire_qty = 0
                if processed_qty <= 0 and operation_hours_value <= 0:
                    continue

            elif operation_kind == "laser":
                edge_mode = _normalize_laser_edge_mode(
                    entry.get("laser_edge_mode", "Uzun + kısa kenar")
                )
                long_required, short_required = _laser_required_edges(edge_mode)
                new_long_edge_qty = max(
                    int(entry.get("laser_long_edge_qty", 0) or 0),
                    0,
                )
                new_short_edge_qty = max(
                ...
                if not long_required and new_long_edge_qty > 0:
                    raise ValueError(
                        f"{operation_name}: Yalnız kısa kenar seçildiği için uzun kenar adedi 0 olmalıdır."
                    )
                if not short_required and new_short_edge_qty > 0:
                    raise ValueError(
                        f"{operation_name}: Yalnız uzun kenar seçildiği için kısa kenar adedi 0 olmalıdır."
                    )
                ...
                previous_long_edge_qty, previous_short_edge_qty = cur.fetchone()
                cur.execute(
                    "SELECT requested_qty FROM production_output_items WHERE id = ?",
                    (item_id,),
                )
                requested_for_item = int(cur.fetchone()[0] or 0)
                completed_before = _laser_completed_qty_from_edges(
                    int(previous_long_edge_qty or 0),
                    int(previous_short_edge_qty or 0),
                    edge_mode,
                    long_per_team,
                    short_per_team,
                    requested_for_item,
                )
                completed_after = _laser_completed_qty_from_edges(
                    int(previous_long_edge_qty or 0) + new_long_edge_qty,
                    int(previous_short_edge_qty or 0) + new_short_edge_qty,
                    edge_mode,
                    long_per_team,
                    short_per_team,
                    requested_for_item,
                ...
                has_partial_work = any([
                    new_long_edge_qty > 0,
                    new_short_edge_qty > 0,
                    float(entry.get("operation_hours", 0) or 0) > 0,
                    int(entry.get("laser_plate_qty", 0) or 0) > 0,
                    fire_qty > 0,
                ])
                if not has_partial_work:
                    continue

            elif operation_kind == "abkant":
                abkant_operation_type = _normalize_abkant_operation_type(
                    entry.get("bend_type", "")
                )

                if abkant_operation_type == "Mesh bükümü":
                    # Mesh bükümünde uzun/kısa kenar veya takım hesabı yoktur.
                    # Operatör doğrudan bükülen mesh adedini girer.
                    processed_qty = max(int(entry.get("processed_qty", 0) or 0), 0)
                    if processed_qty <= 0:
                        continue
                else:
                    long_bend_type = _normalize_abkant_bend_requirement(
                        entry.get("abkant_long_bend_type", "")
                    )
                    short_bend_type = _normalize_abkant_bend_requirement(
                        entry.get("abkant_short_bend_type", "")
                    )
                    if "Seçin" in {long_bend_type, short_bend_type}:
                        raise ValueError(
                            f"{operation_name}: Uzun ve kısa kenar için projede istenen "
                            "büküm durumunu seçin. Büküm olmayan kenarda Yok seçilmelidir."
                        )
                    if long_bend_type == "Yok" and short_bend_type == "Yok":
                        raise ValueError(
                            f"{operation_name}: İki kenar da Yok seçilemez. Bu POS'ta abkant "
                            "gerekmiyorsa operasyon planından abkantı çıkarın."
                        )

                    new_long_team_qty = max(
                        int(entry.get("abkant_long_bend_qty", 0) or 0),
                        0,
                    )
                    new_short_team_qty = max(
                        int(entry.get("abkant_short_bend_qty", 0) or 0),
                        0,
                    )

                    if long_bend_type == "Yok" and new_long_team_qty > 0:
                        raise ValueError(
                            f"{operation_name}: Uzun kenar Yok seçildiği için uzun kenar takım "
                            "sayısı 0 olmalıdır."
                        )
                    if short_bend_type == "Yok" and new_short_team_qty > 0:
                        raise ValueError(
                            f"{operation_name}: Kısa kenar Yok seçildiği için kısa kenar takım "
                            "sayısı 0 olmalıdır."
                        )
                    ...
                    cur.execute(
                        """
                        SELECT
                            COALESCE(SUM(abkant_long_bend_qty), 0),
                            COALESCE(SUM(abkant_short_bend_qty), 0),
                            COALESCE(SUM(
                                CASE
                                    WHEN COALESCE(counts_toward_progress, 1) = 1
                                    THEN processed_qty
                    ...
                    previous_long_team_qty, previous_short_team_qty, previous_processed = (
                        cur.fetchone()
                    )
                    cur.execute(
                        "SELECT requested_qty FROM production_output_items WHERE id = ?",
                    ...
                    requested_for_item = int(cur.fetchone()[0] or 0)
                    teams_per_piece = max(
                        int(entry.get("abkant_teams_per_piece", 1) or 1),
                        1,
                    )
                    completed_after = _abkant_completed_qty_from_edge_teams(
                        int(previous_long_team_qty or 0) + new_long_team_qty,
                        int(previous_short_team_qty or 0) + new_short_team_qty,
                        long_bend_type,
                        short_bend_type,
                        teams_per_piece,
                        requested_for_item,
                    ...
                    processed_qty = max(
                        completed_after - int(previous_processed or 0),
                        0,
                    )
                    ...
                    cur.execute(
                        """
                        UPDATE production_output_items
                        SET abkant_long_bend_type = ?,
                            abkant_short_bend_type = ?
                        WHERE id = ?
                        """,
                        (long_bend_type, short_bend_type, item_id),
                    )

                    has_partial_work = any([
                        new_long_team_qty > 0,
                        new_short_team_qty > 0,
                        float(entry.get("operation_hours", 0) or 0) > 0,
                        fire_qty > 0,
                    ])
                    if not has_partial_work:
            ...
            if counts_toward_progress and (fire_qty < 0 or fire_qty > processed_qty):
                if operation_kind == "laser" and processed_qty == 0 and fire_qty > 0:
                    raise ValueError(
                        f"{operation_name}: Bu kenar girişiyle henüz tamamlanan takım oluşmadığı "
                        "için fire takım girilemez."
                    )
                if (
                    operation_kind == "abkant"
                    and _normalize_abkant_operation_type(entry.get("bend_type", ""))
                    == "Kenar bükümü"
                    and processed_qty == 0
                    and fire_qty > 0
                ):
                    raise ValueError(
                        f"{operation_name}: Girilen uzun/kısa kenar takım sayılarıyla henüz tamamlanan ürün oluşmadığı "
                        "için fire adedi girilemez."
                    )
                raise ValueError(f"{operation_name}: Fire, tamamlanan adetten büyük olamaz.")

            good_qty = (processed_qty - fire_qty) if counts_toward_progress else 0
            if counts_toward_progress and operation_kind == "laser":
                laser_work_equivalent_qty = _laser_edge_work_equivalent_qty(
                    int(entry.get("laser_long_edge_qty", 0) or 0),
                    int(entry.get("laser_short_edge_qty", 0) or 0),
                    int(entry.get("laser_long_per_team", 2) or 2),
                    int(entry.get("laser_short_per_team", 2) or 2),
                )
                entry["calculated_area_mm2"] = max(
                    laser_work_equivalent_qty - fire_qty,
                    0.0,
                ) * float(entry.get("unit_area_mm2", 0) or 0)
            ...
            cur.execute(
                ...
                INSERT INTO operation_work_logs (
                    ...
                    laser_short_per_team, laser_edge_mode, abkant_work_mode,
                    ...
                    abkant_short_double_bend_qty, abkant_long_bend_type,
                    abkant_short_bend_type, abkant_manual_override,
                    participants_text, piece_weight_kg, bend_type,
                    calculated_area_mm2, work_detail, counts_toward_progress,
                    fire_note, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    batch_id,
                    ...
                    str(entry.get("abkant_long_bend_type", "") or ""),
                    str(entry.get("abkant_short_bend_type", "") or ""),
                    ...
                    (
                        0.0
                        if not counts_toward_progress
                        else float(entry.get("calculated_area_mm2", 0) or 0)
                        if operation_kind == "laser"
                        else float(good_qty) * float(entry.get("unit_area_mm2", 0) or 0)
        ...
        for item_id in affected_item_ids:
            ...
            completion_operation = _completion_operation_from_plan(
                planned_operations
            )
            if not completion_operation:
                continue

            completion_kind = _operation_kind(completion_operation)
            if completion_kind == "laser":
                cur.execute(
                    """
                    SELECT
                        COALESCE(SUM(laser_long_edge_qty), 0),
                ...
                laser_completed = _laser_completed_qty_from_edges(
                    int(total_long_edges or 0),
                    int(total_short_edges or 0),
                    str(stored_edge_mode or "Uzun + kısa kenar"),
                    int(long_per_team or 2),
                    int(short_per_team or 2),
                    int(requested_qty or 0),
                )
                after_completed = max(
                    laser_completed - int(total_fire or 0),
                    0,
                )
            else:
                cur.execute(
                ...
                after_completed = max(int(cur.fetchone()[0] or 0), 0)

            # Sevkiyat varsa nihai tamamlanan miktarı doğrudan sevk edilen sağlam
            # adet belirler. Önceki ara etapların daha düşük görünmesi sevkiyatla
            # bitmiş ürünü tekrar "kalan" durumuna düşürmez.
            ...
            if delta > 0:
                cur.execute(
                    ...
                    (
                        ...
                        (
                            "Sevkiyatla tamamlanan ürün"
                            if completion_kind == "sevkiyat"
                            else "Son operasyonda tamamlanan ürün"
                        ),
                        created_at,
                        "operation_tracking",
                        batch_id,
...
def set_session_values(values: dict):
    """Tek düğmeyle birden fazla giriş alanını doldurur."""
...
def main():
    ...
    st.caption("Sürüm 2.17.3 · SEVKİYAT = TAMAMLANAN ÜRÜN")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.17.3 — SEVKİYAT NİHAİ TAMAMLANMA ETABI
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.17.2 · KESİM > TAŞLAMA AKIŞI
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    def build_entries(selected_items: pd.DataFrame, plans: dict, selected_map: dict):
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                work_detail_key = (
                    f"workdetail_{item_id}_{normalized_operation}_{version}"
                )
                saved_work_detail = (
                    saved_entries[0].get("work_detail", "")
                    if saved_entries
                    else ""
                )
                work_detail = _normalize_work_detail(
                    operation,
                    st.session_state.get(work_detail_key, saved_work_detail),
                )
                counts_toward_progress = (
                    work_detail != _auxiliary_work_label(operation)
                )
                entry_operation_kind = (
                    operation_kind if counts_toward_progress else "auxiliary"
                )
                ...
                if operation_kind == "laser_cut_out":
                    ...
                    if processed > 0:
                        entries.append({
                            ...
                            "counts_toward_progress": int(counts_toward_progress),
                ...
                if operation_kind == "laser":
                    ...
                    edge_mode_key = (
                        f"laser_edgemode_{item_id}_{normalized_operation}_{version}"
                    )
                    saved_edge_mode = (
                        saved_entries[0].get("laser_edge_mode", "")
                        if saved_entries
                        else ""
                    )
                    progress_edge_mode = (
                        str(progress_row.get("laser_edge_mode", "") or "")
                    ...
                    edge_mode = _normalize_laser_edge_mode(
                        st.session_state.get(
                            edge_mode_key,
                            saved_edge_mode or progress_edge_mode or "Uzun + kısa kenar",
                        )
                    )
                    long_required, short_required = _laser_required_edges(edge_mode)
                    previous_laser_long_qty = (
                        int(progress_row.get("laser_long_edge_qty", 0) or 0)
                        if progress_row is not None
                        else 0
                    )
                    previous_laser_short_qty = (
                        int(progress_row.get("laser_short_edge_qty", 0) or 0)
                        if progress_row is not None
                        else 0
                    )
                    form_laser_long_qty = 0
                    form_laser_short_qty = 0
                    form_laser_processed = 0
                    ...
                    for lot_index in range(1, lot_count + 1):
                        ...
                        if not long_required:
                            long_edge_qty = 0
                        if not short_required:
                            short_edge_qty = 0
                        completed_before = _laser_completed_qty_from_edges(
                            previous_laser_long_qty + form_laser_long_qty,
                            previous_laser_short_qty + form_laser_short_qty,
                            edge_mode,
                            long_per_team,
                            short_per_team,
                            requested,
                        ...
                        processed = max(completed_after - completed_before, 0)
                        fire = int(
                            st.session_state.get(
                                fire_key,
                        ...
                        good_qty = max(processed - fire, 0)
                        work_equivalent_qty = _laser_edge_work_equivalent_qty(
                            long_edge_qty,
                            short_edge_qty,
                            long_per_team,
                            short_per_team,
                        )
                        work_good_equivalent_qty = max(
                            work_equivalent_qty - fire,
                            0.0,
                        )
                        has_partial_work = any([
                            long_edge_qty > 0,
                        ...
                        if has_partial_work:
                            entries.append({
                                ...
                                "piece_weight_kg": 0.0,
                                "bend_type": "",
                                "calculated_area_mm2": work_good_equivalent_qty
                                * float(item["unit_area_mm2"]),
                            })
                            form_laser_long_qty += long_edge_qty
                            form_laser_short_qty += short_edge_qty
                            form_laser_processed += processed
                ...
                abkant_long_bend_type_key = (
                    f"abkantlongtype_{item_id}_{normalized_operation}_{version}"
                )
                abkant_short_bend_type_key = (
                    f"abkantshorttype_{item_id}_{normalized_operation}_{version}"
                )
                ...
                abkant_operation_type_key = (
                    f"abkantoperationtype_{item_id}_{normalized_operation}_{version}"
                )
                saved_entry = saved_entries[0] if saved_entries else {}
                abkant_long_bend_type = _normalize_abkant_bend_requirement(
                    st.session_state.get(
                        abkant_long_bend_type_key,
                        saved_entry.get(
                            "abkant_long_bend_type",
                            item.get("abkant_long_bend_type", ""),
                        ),
                    )
                )
                abkant_short_bend_type = _normalize_abkant_bend_requirement(
                    st.session_state.get(
                        abkant_short_bend_type_key,
                        saved_entry.get(
                            "abkant_short_bend_type",
                            item.get("abkant_short_bend_type", ""),
                        ),
                    )
                )
                # Abkant dışındaki operasyonlarda da aşağıdaki toplam hesabı çalışır.
                # Bu nedenle değişken her operasyon turunda güvenli biçimde başlatılır.
                previous_processed = 0

                if entry_operation_kind == "abkant":
                    abkant_operation_type = _normalize_abkant_operation_type(
                        st.session_state.get(
                            abkant_operation_type_key,
                    ...
                    previous_processed = (
                        int(progress_row.get("processed_qty", 0) or 0)
                        if progress_row is not None
                        else 0
                    )

                    if abkant_operation_type == "Mesh bükümü":
                        # Mesh doğrudan adet olarak kaydedilir; kenar/takım alanları kullanılmaz.
                        processed = max(
                            int(
                                st.session_state.get(
                                    processed_key,
                        ...
                        abkant_teams_per_piece = 1
                        abkant_team_qty = 0
                        abkant_long_team_qty = 0
                        abkant_short_team_qty = 0
                        abkant_long_single_qty = 0
                        abkant_long_double_qty = 0
                        abkant_short_single_qty = 0
                        abkant_short_double_qty = 0
                        previous_long_team_qty = 0
                        previous_short_team_qty = 0
                        abkant_long_bend_type = ""
                        abkant_short_bend_type = ""
                    else:
                        # Uzun ve kısa kenar takım sayıları ayrı girilir. Bir panelin
                        # tamamlanması için projede gerekli tüm kenar takımlarının hazır
                        # olması gerekir. İki kenar isteniyorsa düşük takım sayısı adedi belirler.
                        abkant_long_team_qty = max(
                            int(
                                st.session_state.get(
                                    abkant_long_bend_qty_key,
                                    saved_entry.get("abkant_long_bend_qty", 0),
                                )
                                or 0
                            ),
                            0,
                        )
                        abkant_short_team_qty = max(
                            int(
                                st.session_state.get(
                                    abkant_short_bend_qty_key,
                                    saved_entry.get("abkant_short_bend_qty", 0),
                                )
                                or 0
                            ),
                            0,
                        )
                        if abkant_long_bend_type == "Yok":
                            abkant_long_team_qty = 0
                        if abkant_short_bend_type == "Yok":
                            abkant_short_team_qty = 0

                        previous_long_team_qty = (
                            int(progress_row.get("abkant_long_team_qty", 0) or 0)
                            if progress_row is not None
                            else 0
                        )
                        previous_short_team_qty = (
                            int(progress_row.get("abkant_short_team_qty", 0) or 0)
                            if progress_row is not None
                            else 0
                        )
                        abkant_teams_per_piece = max(
                            int(
                                st.session_state.get(
                                    abkant_teams_per_piece_key,
                                    saved_entry.get(
                                        "abkant_teams_per_piece",
                                        progress_row.get("abkant_teams_per_piece", 1)
                                        if progress_row is not None
                                        else 1,
                                    ),
                                )
                        ...
                        (
                            previous_long_team_qty,
                            previous_short_team_qty,
                        ) = _abkant_effective_previous_edge_teams(
                            previous_long_team_qty,
                            previous_short_team_qty,
                            abkant_long_bend_type,
                            abkant_short_bend_type,
                        ...
                        completed_after = _abkant_completed_qty_from_edge_teams(
                            previous_long_team_qty + abkant_long_team_qty,
                            previous_short_team_qty + abkant_short_team_qty,
                            abkant_long_bend_type,
                            abkant_short_bend_type,
                            abkant_teams_per_piece,
                            requested,
                        ...
                        processed = min(
                            max(completed_after - previous_processed, 0),
                            remaining,
                        )

                        abkant_team_qty = (
                            abkant_long_team_qty + abkant_short_team_qty
                        )

                        abkant_long_single_qty = (
                            abkant_long_team_qty
                            if abkant_long_bend_type == "Tek büküm"
                            else 0
                        )
                        abkant_long_double_qty = (
                            abkant_long_team_qty
                            if abkant_long_bend_type == "Çift büküm"
                            else 0
                        )
                        abkant_short_single_qty = (
                            abkant_short_team_qty
                            if abkant_short_bend_type == "Tek büküm"
                            else 0
                        )
                        abkant_short_double_qty = (
                            abkant_short_team_qty
                            if abkant_short_bend_type == "Çift büküm"
                            else 0
                        )
                else:
                    abkant_operation_type = ""
                    abkant_teams_per_piece = 1
                    abkant_team_qty = 0
                    abkant_long_team_qty = 0
                    abkant_short_team_qty = 0
                ...
                processed_total = previous_processed + processed
                abkant_activity = False
                if entry_operation_kind == "abkant":
                    if abkant_operation_type == "Mesh bükümü":
                        abkant_activity = processed > 0 or operation_hours > 0
                    else:
                        abkant_activity = (
                            abkant_long_team_qty > 0
                            or abkant_short_team_qty > 0
                            or operation_hours > 0
                        )

                if processed > 0 or operation_hours > 0 or abkant_activity:
                    entries.append({
                        "item_id": item_id,
                        "pos": str(item["pos"]),
                        "operation_name": operation,
                        ...
                        "processed_qty": processed,
                        "fire_qty": fire,
                        "good_qty": (processed - fire) if counts_toward_progress else 0,
                        "remaining_before": remaining,
                        "unit_area_mm2": float(item["unit_area_mm2"]),
                        "boy_mm": float(item["boy_mm"]),
                        "en_mm": float(item["en_mm"]),
                        ...
                        ) if entry_operation_kind == "abkant" else "",
                        "abkant_coworker": (
                            str(
                                st.session_state.get(
                                    coworker_key,
                        ...
                        "abkant_team_qty": (
                            abkant_team_qty if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_teams_per_piece": (
                            abkant_teams_per_piece
                            if entry_operation_kind == "abkant"
                        ...
                        "abkant_team_excess": (
                            sum(
                                max(total_team_qty - processed_total * abkant_teams_per_piece, 0)
                                for total_team_qty, requirement in (
                                    (
                                        previous_long_team_qty + abkant_long_team_qty,
                                        abkant_long_bend_type,
                                    ),
                                ...
                                if requirement in {"Tek büküm", "Çift büküm"}
                            )
                            if entry_operation_kind == "abkant"
                            else 0
                        ),
                        "abkant_long_single_bend_qty": (
                            abkant_long_single_qty if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_long_double_bend_qty": (
                            abkant_long_double_qty if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_short_single_bend_qty": (
                            abkant_short_single_qty if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_short_double_bend_qty": (
                            abkant_short_double_qty if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_long_bend_qty": (
                            abkant_long_single_qty + abkant_long_double_qty
                            if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_short_bend_qty": (
                            abkant_short_single_qty + abkant_short_double_qty
                            if entry_operation_kind == "abkant" else 0
                        ),
                        "abkant_long_bend_type": (
                            abkant_long_bend_type if entry_operation_kind == "abkant" else ""
                        ),
                        "abkant_short_bend_type": (
                            abkant_short_bend_type if entry_operation_kind == "abkant" else ""
                        ),
                        ...
                        "bend_type": (
                            "Mesh bükümü"
                            if entry_operation_kind == "abkant"
                            and abkant_operation_type == "Mesh bükümü"
                            else (
                                f"Kenar bükümü · Uzun: {abkant_long_bend_type} | "
                                f"Kısa: {abkant_short_bend_type}"
                                if entry_operation_kind == "abkant"
                                else ""
                            )
                        ),
                        "calculated_area_mm2": (
                            max(processed - fire, 0) * float(item["unit_area_mm2"])
        ...
        for _, item in selected_items.sort_values("_sort_pos").iterrows():
            ...
            with st.expander(
                ...
                for operation in selected_ops:
                    ...
                    work_detail_options = _work_detail_options(operation)
                    saved_work_detail = (
                        saved_entries_for_operation[0].get("work_detail", "")
                        if saved_entries_for_operation
                        else ""
                    )
                    default_work_detail = _normalize_work_detail(
                        operation, saved_work_detail
                    )
                    if work_detail_key not in st.session_state:
                        st.session_state[work_detail_key] = default_work_detail
                    if len(work_detail_options) > 1:
                        st.selectbox(
                            "Yapılan iş türü",
                            work_detail_options,
                            key=work_detail_key,
                            help=(
                                "Yardımcı iş seçildiğinde adet yapılan iş miktarı olarak saklanır; "
                                "POS üretim ilerlemesini azaltmaz. Girilen süre proje işçiliğine eklenir."
                            ),
                        )
                    selected_work_detail = _normalize_work_detail(
                        operation, st.session_state.get(work_detail_key, "")
                    )
                    is_auxiliary_work = (
                        selected_work_detail == _auxiliary_work_label(operation)
                    )
                    entry_operation_kind = (
                        operation_kind if not is_auxiliary_work else "auxiliary"
                    )
                    if is_auxiliary_work:
                        st.info(
                            f"**{selected_work_detail} yardımcı iştir.** Girilen adet bilgi olarak "
                            "saklanır; üretim tamamlanma adedine ve fireye eklenmez. Süre proje saatine yazılır."
                        )

                    if entry_operation_kind == "laser":
                        lot_count_key = f"laserlotcount_{item_id}_{normalized_operation}_{version}"
                        ...
                        st.caption(
                            ...
                            "Bugün yalnızca uzun ya da kısa kenar üretilebilir. Yapılan kenar işi "
                            "m² karşılığında hemen görünür; net takım ise birikmiş uzun ve kısa "
                            "kenarlar eşleştiğinde ilerler."
                        )
                        ...
                        saved_edge_mode = (
                            saved_entries_for_operation[0].get("laser_edge_mode", "")
                            if saved_entries_for_operation
                            else ""
                        )
                        progress_edge_mode = (
                        ...
                        default_edge_mode = _normalize_laser_edge_mode(
                            saved_edge_mode or progress_edge_mode or "Uzun + kısa kenar"
                        )
                        if edge_mode_key not in st.session_state:
                            st.session_state[edge_mode_key] = default_edge_mode
                        st.selectbox(
                            "Bugün üretilen lazer kenarı",
                            LASER_EDGE_MODE_OPTIONS,
                            key=edge_mode_key,
                            help=(
                                "Bu seçim yalnızca bugün hangi kenar girişlerinin açık olacağını belirler. "
                                "Tam takım hesabında uzun ve kısa kenar her zaman birlikte aranır."
                            ),
                        )
                        selected_edge_mode = _normalize_laser_edge_mode(
                            st.session_state.get(edge_mode_key)
                        )
                        long_required, short_required = _laser_required_edges(
                            selected_edge_mode
                        )
                        previous_laser_long_qty = (
                            int(progress_row.get("laser_long_edge_qty", 0) or 0)
                        ...
                        previous_laser_good = completed_qty
                        form_laser_long_qty = 0
                        form_laser_short_qty = 0
                        form_laser_processed = 0
                        form_laser_good = 0
                        ...
                        for lot_index in range(1, lot_count + 1):
                            ...
                            if not long_required:
                                st.session_state[long_qty_key] = 0
                            if not short_required:
                                st.session_state[short_qty_key] = 0
                            ...
                            with edge1:
                                st.number_input(
                                    ...
                                    step=1,
                                    key=long_qty_key,
                                    disabled=not long_required,
                                    help=(
                                        "Bugün uzun kenar kesilmediyse 0 bırak. "
                                        "Kısa kenar kaydı yine ayrı olarak saklanır."
                                    ),
                                )
                            with edge2:
                                st.number_input(
                                    ...
                                    step=1,
                                    key=short_qty_key,
                                    disabled=not short_required,
                                    help=(
                                        "Bugün kısa kenar kesilmediyse 0 bırak. "
                                        "Uzun kenar kaydı yine ayrı olarak saklanır."
                                    ),
                                )
                            ...
                            cumulative_short_qty = (
                                previous_laser_short_qty
                                + form_laser_short_qty
                                + short_qty
                            )
                            completed_before_lot = _laser_completed_qty_from_edges(
                                previous_laser_long_qty + form_laser_long_qty,
                                previous_laser_short_qty + form_laser_short_qty,
                                selected_edge_mode,
                                long_per_team,
                            ...
                            cumulative_completed_teams = _laser_completed_qty_from_edges(
                                cumulative_long_qty,
                                cumulative_short_qty,
                                selected_edge_mode,
                                long_per_team,
                            ...
                            completed_teams = max(
                                cumulative_completed_teams - completed_before_lot,
                                0,
                            )
                            fire_teams = int(st.session_state.get(fire_key, 0) or 0)
                            good_teams = max(completed_teams - fire_teams, 0)
                            long_used = cumulative_completed_teams * long_per_team
                            short_used = cumulative_completed_teams * short_per_team
                            long_excess = max(cumulative_long_qty - long_used, 0)
                            short_excess = max(cumulative_short_qty - short_used, 0)
                            unit_area_m2 = float(item["unit_area_mm2"]) / 1_000_000
                            work_equivalent_teams = _laser_edge_work_equivalent_qty(
                                long_qty,
                                short_qty,
                                long_per_team,
                                short_per_team,
                            )
                            work_good_equivalent_teams = max(
                                work_equivalent_teams - fire_teams,
                                0.0,
                            )
                            work_area_m2 = work_good_equivalent_teams * unit_area_m2
                            net_area_m2 = good_teams * unit_area_m2
                            current_hours = float(st.session_state.get(hours_key, 0.0) or 0.0)
                            work_m2_hour = (
                                work_area_m2 / current_hours
                                if current_hours > 0
                                else 0.0
                            )
                            net_m2_hour = (
                                net_area_m2 / current_hours
                                if current_hours > 0
                                else 0.0
                            )
                            ...
                            result2.metric("Bugün tamamlanan", f"{completed_teams} takım")
                            result3.metric(
                                "POS toplam sağlam",
                                f"{previous_laser_good + form_laser_good + good_teams} takım",
                            )
                            area_result1, area_result2 = st.columns(2)
                            area_result1.metric(
                                "Bugünkü kenar üretim karşılığı",
                                f"{work_area_m2:.2f} m²",
                            )
                            area_result2.metric(
                            ...
                            selected_labels = []
                            if long_required:
                                selected_labels.append("uzun")
                            if short_required:
                                selected_labels.append("kısa")
                            st.info(
                                f"**Bugün seçilen kenar:** {' + '.join(selected_labels)} · "
                                f"**Tam takım:** uzun + kısa · "
                                f"**Biriken fazla/bekleyen:** Uzun {long_excess} · Kısa {short_excess}"
                            )
                            if (long_qty > 0 and short_qty == 0) or (
                                short_qty > 0 and long_qty == 0
                            ):
                                st.warning(
                                    "Bugün yalnızca bir kenar üretildi. Yapılan iş m² karşılığına yazılır; "
                                    "net panel ise birikmiş diğer kenarla eşleşen takım kadar ilerler."
                                )
                            long_work_teams = long_qty / long_per_team
                            short_work_teams = short_qty / short_per_team
                            st.markdown(
                                f"""**1 panel alanı:**  
{_format_mm2(item['boy_mm'])} × {_format_mm2(item['en_mm'])} ÷ 1.000.000 = **{unit_area_m2:.2f} m²**

**Bugünkü kenar üretimi:**  
Uzun: {long_qty} ÷ {long_per_team} = **{long_work_teams:.2f} takım karşılığı**  
Kısa: {short_qty} ÷ {short_per_team} = **{short_work_teams:.2f} takım karşılığı**

**Bugünkü kenar üretim karşılığı alan:**  
{work_good_equivalent_teams:.2f} × {unit_area_m2:.2f} = **{work_area_m2:.2f} m²**

**Bugün tamamlanan net takım:** {completed_teams}  
**Kenar toplamından oluşan kümülatif net takım:** {cumulative_completed_teams}

**Üretilen net panel alanı:**  
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                            )
                            st.caption(
                                f"İş karşılığı alan verimi: {work_m2_hour:.2f} m²/saat · "
                                f"Net panel alan verimi: {net_m2_hour:.2f} m²/saat"
                            )
                            form_laser_long_qty += long_qty
                            form_laser_short_qty += short_qty
                            form_laser_processed += completed_teams
                            form_laser_good += good_teams
                    ...
                    elif entry_operation_kind == "laser_cut_out":
                        processed_key = (
                            f"processed_{item_id}_{normalized_operation}_{version}"
                        )
                        saved_entry = (
                        ...
                        defaults_for_widgets = {
                            ...
                            abkant_long_bend_type_key: _normalize_abkant_bend_requirement(
                                saved_entry.get(
                                    "abkant_long_bend_type",
                                    item.get("abkant_long_bend_type", ""),
                                )
                            ),
                            abkant_short_bend_type_key: _normalize_abkant_bend_requirement(
                                saved_entry.get(
                                    "abkant_short_bend_type",
                                    item.get("abkant_short_bend_type", ""),
                                )
                            ...
                            bend_key: str(saved_entry.get("bend_type", "") or ""),
                            abkant_operation_type_key: _normalize_abkant_operation_type(
                                saved_entry.get("bend_type", "")
                            ),
                        }
                        for widget_key, widget_default in defaults_for_widgets.items():
                            if widget_key not in st.session_state:
                                st.session_state[widget_key] = widget_default
                        if is_auxiliary_work:
                            st.session_state[fire_key] = 0

                        if entry_operation_kind == "abkant":
                            st.markdown("**Abkant büküm şekli**")
                            st.selectbox(
                                "Büküm şekli",
                                ABKANT_OPERATION_TYPE_OPTIONS,
                                key=abkant_operation_type_key,
                                help=(
                                    "Kenar bükümünde uzun/kısa kenar takım bilgileri kullanılır. "
                                    "Mesh bükümünde doğrudan bükülen adet girilir."
                                ),
                            )
                            selected_abkant_operation_type = _normalize_abkant_operation_type(
                                st.session_state.get(abkant_operation_type_key)
                            )

                            if selected_abkant_operation_type == "Mesh bükümü":
                                # Eski kenar değerlerinin mesh kaydına karışmasını engelle.
                                for edge_key in (
                                    abkant_long_bend_qty_key,
                                    abkant_short_bend_qty_key,
                                    abkant_long_single_bend_qty_key,
                                    ...
                                    st.session_state[edge_key] = 0

                                st.info(
                                    "Mesh bükümünde uzun ve kısa kenar hesabı yapılmaz. "
                                    "Bükülen mesh miktarını doğrudan adet olarak girin."
                                )
                                mesh1, mesh2, mesh3, mesh4 = st.columns(
                                    [1.2, 1.0, 1.0, 1.0]
                                )
                                with mesh1:
                                    st.number_input(
                                ...
                                st.markdown("**Projede istenen kenar ve büküm şekli**")
                                requirement1, requirement2 = st.columns(2)
                                with requirement1:
                                    st.selectbox(
                                        "Uzun kenar",
                                        ABKANT_BEND_REQUIREMENT_OPTIONS,
                                        key=abkant_long_bend_type_key,
                                        help="Uzun kenarda büküm yoksa Yok seçin.",
                                    )
                                with requirement2:
                                    st.selectbox(
                                        "Kısa kenar",
                                        ABKANT_BEND_REQUIREMENT_OPTIONS,
                                        key=abkant_short_bend_type_key,
                                        help="Kısa kenarda büküm yoksa Yok seçin.",
                                    )

                                selected_long_type = _normalize_abkant_bend_requirement(
                                    st.session_state.get(abkant_long_bend_type_key)
                                )
                                selected_short_type = _normalize_abkant_bend_requirement(
                                    st.session_state.get(abkant_short_bend_type_key)
                                )

                                if "Seçin" in {selected_long_type, selected_short_type}:
                                    st.warning(
                                        "Uzun ve kısa kenar için projede istenen durumu seçin. "
                                        "Büküm yapılmayacak kenarda Yok seçilmelidir."
                                    )
                                elif selected_long_type == "Yok" and selected_short_type == "Yok":
                                    st.error(
                                        "Bu POS için uzun ve kısa kenarın ikisi de Yok seçildi. "
                                        "Abkant işlemi gerekmiyorsa bu operasyonu kayıttan çıkarın."
                                    )

                                st.caption(
                                    "Uzun ve kısa kenar takım sayılarını ayrı girin. Sistem her iki "
                                    "kenarı da 1 adet için gereken takım sayısına bölerek sağlam adedi bulur."
                                )

                                long_edge_required = selected_long_type in {
                                    "Tek büküm", "Çift büküm"
                                }
                                short_edge_required = selected_short_type in {
                                    "Tek büküm", "Çift büküm"
                                }
                                if not long_edge_required:
                                    st.session_state[abkant_long_bend_qty_key] = 0
                                if not short_edge_required:
                                    st.session_state[abkant_short_bend_qty_key] = 0
                                ...
                                with team1:
                                    st.number_input(
                                        "Uzun kenar yapılan takım",
                                        min_value=0,
                                        step=1,
                                        key=abkant_long_bend_qty_key,
                                        disabled=not long_edge_required,
                                        help="Bu kayıtta tamamlanan uzun kenar takım sayısı.",
                                    )
                                with team2:
                                    st.number_input(
                                        "Kısa kenar yapılan takım",
                                        min_value=0,
                                        step=1,
                                        key=abkant_short_bend_qty_key,
                                        disabled=not short_edge_required,
                                        help="Bu kayıtta tamamlanan kısa kenar takım sayısı.",
                                    )
                                with team3:
                                    st.number_input(
                                        "1 adet için gereken takım",
                                        min_value=1,
                                        step=1,
                                        key=abkant_teams_per_piece_key,
                                        help=(
                                            "Her kenar grubunda 1 ürün için gereken takım sayısı. "
                                            "Örneğin iki uzun veya iki kısa kenar gerekiyorsa 2 girin."
                                        ),
                                    )

                                team4, team5 = st.columns(2)
                                with team4:
                                    st.number_input(
                                        "Fire (adet)",
                                        min_value=0,
                                        step=1,
                                        key=fire_key,
                                        help="Kenar takımları adede çevrildikten sonra fire çıkan panel adedi.",
                                    )
                                with team5:
                                    st.number_input(
                                        "Süre (saat)",
                                ...
                                current_long_team_qty = max(
                                    int(st.session_state.get(abkant_long_bend_qty_key, 0) or 0),
                                    0,
                                )
                                current_short_team_qty = max(
                                    int(st.session_state.get(abkant_short_bend_qty_key, 0) or 0),
                                    0,
                                )
                                ...
                                (
                                    ...
                                    selected_long_type,
                                    selected_short_type,
                                    current_teams_per_piece,
                                    requested_qty,
                                    previous_processed,
                                )
                                long_needed_for_remaining = (
                                    max(
                                        requested_qty * current_teams_per_piece
                                        - previous_long_team_qty,
                                        0,
                                    )
                                    if long_edge_required
                                    else 0
                                )
                                short_needed_for_remaining = (
                                    max(
                                        requested_qty * current_teams_per_piece
                                        - previous_short_team_qty,
                                        0,
                                    )
                                    if short_edge_required
                                    else 0
                                )
                                st.button(
                                    "Kalan adet için uzun/kısa takım sayılarını yaz",
                                    disabled=(remaining_qty == 0),
                                    use_container_width=True,
                                    key=(
                                        f"fill_abkant_edge_teams_{item_id}_"
                                        f"{normalized_operation}_{version}"
                                    ),
                                    on_click=set_session_values,
                                    args=({
                                        abkant_long_bend_qty_key: long_needed_for_remaining,
                                        abkant_short_bend_qty_key: short_needed_for_remaining,
                                    },),
                                )

                                total_long_after = (
                                    previous_long_team_qty + current_long_team_qty
                                )
                                total_short_after = (
                                    previous_short_team_qty + current_short_team_qty
                                )
                                completed_after = _abkant_completed_qty_from_edge_teams(
                                    total_long_after,
                                    total_short_after,
                                ...
                                if long_edge_required:
                                    calculation_parts.append(
                                        f"uzun {current_long_team_qty} takım "
                                        f"(toplam {total_long_after})"
                                    )
                                if short_edge_required:
                                    calculation_parts.append(
                                        f"kısa {current_short_team_qty} takım "
                                        f"(toplam {total_short_after})"
                                    )

                                used_team_qty = completed_after * current_teams_per_piece
                                long_excess = (
                                    max(total_long_after - used_team_qty, 0)
                                    if long_edge_required
                                    else 0
                                )
                                short_excess = (
                                    max(total_short_after - used_team_qty, 0)
                                    if short_edge_required
                                    else 0
                                )
                                excess_parts = []
                                if long_excess:
                                    excess_parts.append(f"uzun kenarda {long_excess}")
                                if short_excess:
                                    excess_parts.append(f"kısa kenarda {short_excess}")
                                excess_text = (
                                    f" Artan takım: {', '.join(excess_parts)}."
                                    if excess_parts
                                    else " Artan takım yok."
                                )

                                st.info(
                                    f"**Hesap:** {', '.join(calculation_parts)} · "
                                    f"{current_teams_per_piece} takım/adet → bu kayıtta "
                                    f"**{current_completed_qty} adet** ilerler.{excess_text}"
                                )
                                ...
                                st.text_input(
                                    ...
                                    value=(
                                        ...
                                        if selected_abkant_operation_type == "Mesh bükümü"
                                            ...
                                            f"Uzun: {selected_long_type} | "
                                            f"Kısa: {selected_short_type}"
                                        )
                                    ),
                                    disabled=True,
                                    key=(
                                        f"bend_summary_{item_id}_{normalized_operation}_"
                                        f"{selected_abkant_operation_type}_{selected_long_type}_"
                                        f"{selected_short_type}_{version}"
                                    ),
                                )
                            with abkant3:
                                coworker_options = [""] + [
                            ...
                            with row1:
                                st.number_input(
                                    ...
                                    key=processed_key,
                                    help=(
                                        "Bu adet üretimden düşmez; yalnız yapılan yardımcı iş miktarını gösterir."
                                        if is_auxiliary_work
                                        else None
                                    ),
                                )
                            ...
                            with row4:
                                ...
                                st.button(
                                    "Kalanı yaz",
                                    disabled=(remaining_qty == 0 or is_auxiliary_work),
                                    use_container_width=True,
                                    key=(
                                        f"fill_{item_id}_"
                                        f"{normalized_operation}_{version}"
                        ...
                        current_fire = (
                            0
                            if is_auxiliary_work
                            else int(st.session_state.get(fire_key, 0) or 0)
                        )
                        current_good = (
                            0
                            if is_auxiliary_work
                            else max(current_processed - current_fire, 0)
                        )
                        ...
                        if is_auxiliary_work:
                            st.info(
                                f"{selected_work_detail}: **{current_processed} adet · "
                                f"{current_hours:.2f} saat**. POS üretim ilerlemesi değişmez."
                            )
                            st.caption(
                                "Adet yardımcı iş miktarıdır; süre proje toplam işçiliğine eklenir."
                            )
                        else:
                            st.info(
        ...
        if performance["total_auxiliary_qty"] > 0 or performance["auxiliary_hours"] > 0:
            auxiliary_names = sorted({
                str(entry.get("work_detail", "Yardımcı iş"))
                for entry in entries
                if not _entry_counts_toward_progress(entry)
            })
            st.info(
                f"Yardımcı işler: **{performance['total_auxiliary_qty']} adet · "
                f"{performance['auxiliary_hours']:.2f} saat** "
                f"({', '.join(auxiliary_names)}). Adet üretim ilerlemesine dahil değildir; "
                "süre proje saatine eklenir."
            )
        ...
        with b2:
            if st.button("Kayıt Özetini Gör →", type="primary", use_container_width=True):
                ...
                for entry in entries:
                    counts_toward_progress = _entry_counts_toward_progress(entry)
                    if (
                        not counts_toward_progress
                        and int(entry.get("processed_qty", 0) or 0) <= 0
                    ):
                        errors.append(
                            f"{entry['pos']} / {entry['operation_name']} / "
                            f"{entry.get('work_detail', 'Yardımcı iş')}: Yapılan yardımcı iş adedini girmelisin."
                        )
                    if (
                        counts_toward_progress
                        and entry["fire_qty"] > entry["processed_qty"]
                    ):
                        errors.append(
                    ...
                    if operation_kind == "laser":
                        ...
                        long_required, short_required = _laser_required_edges(edge_mode)
                        long_edge_qty = max(
                            int(entry.get("laser_long_edge_qty", 0) or 0), 0
                        )
                        short_edge_qty = max(
                            int(entry.get("laser_short_edge_qty", 0) or 0), 0
                        )
                        if long_edge_qty <= 0 and short_edge_qty <= 0:
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: "
                                "Bugün kesilen uzun veya kısa kenar adedinden en az birini girmelisin."
                            )
                        if not long_required and long_edge_qty > 0:
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: "
                                "Yalnız kısa kenar seçildiği için uzun kenar adedi 0 olmalıdır."
                            )
                        if not short_required and short_edge_qty > 0:
                            errors.append(
                                f"{entry['pos']} / {entry['operation_name']}: "
                                "Yalnız uzun kenar seçildiği için kısa kenar adedi 0 olmalıdır."
                            )
                    ...
                    if counts_toward_progress and operation_kind == "abkant":
                        abkant_operation_type = _normalize_abkant_operation_type(
                            entry.get("bend_type", "")
                        )
                        if abkant_operation_type == "Mesh bükümü":
                            if int(entry.get("processed_qty", 0) or 0) <= 0:
                                errors.append(
                                    f"{entry['pos']} / {entry['operation_name']}: "
                                    "Mesh bükülen adedi girmelisin."
                                )
                        else:
    ...
    if performance["total_auxiliary_qty"] > 0 or performance["auxiliary_hours"] > 0:
        ...
        st.info(
            f"Yardımcı işler: **{performance['total_auxiliary_qty']} adet · "
            f"{performance['auxiliary_hours']:.2f} saat** "
            f"({', '.join(auxiliary_names)}). Üretim tamamlanması değişmez; "
            "süre proje toplamına dahildir."
        )
    ...
    for entry in entries:
        summary_rows.append({
            ...
            "İş Detayı": entry.get("work_detail", "Ana üretim"),
            "Üretim İlerlemesine Sayar": (
                "Evet" if _entry_counts_toward_progress(entry) else "Hayır"
            ),
            ...
            "Lazer Kenar Kapsamı": entry.get("laser_edge_mode", "") or "",
            "Lazer Plaka": entry.get("laser_plate_qty", 0) or "",
            "Abkant Toplam Takım": entry.get("abkant_team_qty", 0) or "",
            "Uzun Kenar Takım": entry.get("abkant_long_bend_qty", 0) or "",
            "Kısa Kenar Takım": entry.get("abkant_short_bend_qty", 0) or "",
            "Eşleşmeyen Takım": entry.get("abkant_team_excess", 0) or "",
    ...
    heavy_single_abkant = [
        ...
        if _entry_counts_toward_progress(entry)
        and _operation_kind(entry.get("operation_name", "")) == "abkant"
    ...
    next_day_default = (
        pd.to_datetime(production_date, errors="coerce") + pd.Timedelta(days=1)
    )
    if pd.isna(next_day_default):
        next_day_default = pd.Timestamp(date.today())

    with st.expander("Hızlı ikinci gün", expanded=False):
        st.caption(
            "Cuma kaydını tamamladıktan sonra cumartesi kaydına geçeceksen; "
            "çalışan, OC, POS, işlemler ve teknik seçimler korunur. "
            "Yeni günün adet, fire ve süre alanları boş açılır."
        )
        quick_next_date = st.date_input(
            "Hazırlanacak diğer gün",
            value=next_day_default.date(),
            key=f"operation_quick_next_date_{version}",
        )

    b1, b2, b3 = st.columns([1, 1.05, 1.45])
    with b1:
    ...
    with b2:
        save_only = st.button(
            ...
            key=f"operation_save_only_{version}",
        )
    with b3:
        save_and_prepare = st.button(
            "Kaydet ve Başka Günü Hazırla →",
            use_container_width=True,
            disabled=not confirmed,
            key=f"operation_save_and_prepare_{version}",
            help=(
                "Mevcut kaydı kaydeder; çalışan, OC, POS ve işlem seçimlerini "
                "koruyarak seçilen tarihin adet/saat ekranını açar."
            ),
        )

    if save_only or save_and_prepare:
        final_errors = []
        if not operator_name:
        ...
        if save_and_prepare and str(quick_next_date) == str(production_date):
            final_errors.append(
                "Hızlı ikinci gün için mevcut kayıttan farklı bir tarih seçmelisin."
            )
        for entry in entries:
            counts_toward_progress = _entry_counts_toward_progress(entry)
            if (
        ...
        st.session_state.operation_form_version = next_version

        if save_and_prepare:
            reset_numeric_fields = {
                "processed_qty",
                "fire_qty",
                "good_qty",
                "operation_hours",
            ...
            for current_entry in entries:
                template_entry = dict(current_entry)
                for field in reset_numeric_fields:
                    if field in template_entry:
                        template_entry[field] = 0.0 if field in {
                            "operation_hours", "calculated_area_mm2"
                        } else 0
                template_entry["fire_note"] = ""
                template_entries.append(template_entry)

            copied_oc_label = str(
                wizard_data.get("selected_oc_label", "")
                or next(
                    (
                        label for label, oc_value in oc_map.items()
                        if str(oc_value) == str(selected_oc)
                    ),
                    "",
                )
            )
            copied_wizard_data = {
                "production_date": quick_next_date,
                "operator_name": operator_name,
                "declared_work_type": declared_work_type,
                "work_type": declared_work_type,
                "reason": reason,
                "selected_oc_label": copied_oc_label,
                "selected_oc": selected_oc,
                "selected_positions": list(selected_positions),
                "selected_combo": wizard_data.get("selected_combo"),
                "change_plan": False,
                "same_operations": bool(
                    wizard_data.get("same_operations", True)
                ),
                "show_other_operations": bool(
                    wizard_data.get("show_other_operations", False)
                ),
                "common_operations": list(
                    wizard_data.get("common_operations", [])
                ),
                "selected_operation_map": dict(
                    wizard_data.get("selected_operation_map", {})
                ),
            ...
            st.session_state[
                f"operation_wizard_data_{next_version}"
            ] = copied_wizard_data
            st.session_state[
                f"operation_wizard_step_{next_version}"
            ] = 4
            st.session_state.operation_entry_success = (
                f"{production_date} kaydı tamamlandı. Kayıt no: {batch_id}. "
                f"{quick_next_date} için aynı çalışan, OC, POS ve işlemler hazırlandı; "
                "yalnızca yeni günün adet, fire ve sürelerini gir."
            )
        else:
            st.session_state.operation_entry_success = (
                f"Kayıt tamamlandı. {len(entries)} POS/işlem satırı kaydedildi. "
                f"Kayıt no: {batch_id}."
            )
        st.rerun()
...
def operasyon_takibi_page():
    ...
    total_completed_qty = int(filtered["produced_qty"].sum())
    total_produced_qty = int(filtered["unique_produced_qty"].sum())
    total_requested_m2 = float(filtered["requested_area_mm2"].sum()) / 1_000_000
    total_produced_m2 = (
        float(filtered["unique_produced_area_mm2"].sum()) / 1_000_000
    )
    qty_ratio = (
        total_produced_qty / total_requested_qty * 100
        if total_requested_qty else 0.0
    )
    m2_ratio = (
        total_produced_m2 / total_requested_m2 * 100
        if total_requested_m2 else 0.0
    )

    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("İstenen", total_requested_qty)
    k2.metric("Tam biten", total_completed_qty)
    ...
    r1.metric(
        "Üretime giren adet / toplam adet",
        f"%{qty_ratio:.1f}",
        f"{total_produced_qty} / {total_requested_qty}",
    )
    r2.metric(
        "Üretilen m² / toplam m²",
        f"%{m2_ratio:.1f}",
        f"{total_produced_m2:.1f} / {total_requested_m2:.1f} m²",
    )
...
def rebuild_operation_completion_logs(
    ...
    """
    POS'un nihai tamamlanan adet günlüklerini operasyon geçmişinden yeniler.

    Planda Sevkiyat varsa tamamlanma kaynağı yalnızca Sevkiyat'ın sağlam
    adedidir. Sevkiyat yoksa planın son operasyonu kullanılır.
    """
    if not item_ids:
        return
    ...
    for item_id in sorted({int(value) for value in item_ids}):
        ...
        requested_qty = int(item_row[0] or 0)
        unit_area = float(item_row[1] or 0)
        ...
        completion_operation = _completion_operation_from_plan(operations)
        if not completion_operation:
            continue
        ...
        cur.execute(
            ...
            SELECT
                ...
                w.good_qty,
                w.fire_qty,
                w.laser_long_edge_qty,
                w.laser_short_edge_qty,
                w.laser_long_per_team,
                w.laser_short_per_team,
                w.laser_edge_mode
            FROM operation_work_logs w
            INNER JOIN operation_batches b ON b.id = w.batch_id
            WHERE w.item_id = ?
              AND w.operation_name = ?
        ...
        def apply_batch():
            ...
            nonlocal cumulative_good, cumulative_fire
            nonlocal cumulative_long, cumulative_short
            nonlocal long_per_team, short_per_team, edge_mode
            ...
            if completion_kind == "laser":
                for entry in batch_entries:
                    ...
                    cumulative_fire += max(int(fire_qty or 0), 0)
                    cumulative_long += max(int(long_edge_qty or 0), 0)
                    cumulative_short += max(int(short_edge_qty or 0), 0)
                    long_per_team = max(int(row_long_per_team or 2), 1)
                    short_per_team = max(int(row_short_per_team or 2), 1)
                    edge_mode = str(row_edge_mode or edge_mode)

                completed = max(
                    _laser_completed_qty_from_edges(
                        cumulative_long,
                        cumulative_short,
                        edge_mode,
                        long_per_team,
                ...
                cumulative_good += sum(
                    max(int(entry[0] or 0), 0)
                    for entry in batch_entries
                )
                completed = cumulative_good

            delta = max(int(completed) - int(previous_completed), 0)
            if delta > 0:
                cur.execute(
                    """
                    INSERT INTO production_output_logs (
                    ...
                    (
                        ...
                        delta * unit_area,
                        (
                            "Sevkiyat geçmişinden yeniden hesaplandı"
                            if completion_kind == "sevkiyat"
                            else "Son operasyon geçmişinden yeniden hesaplandı"
                        ),
                        created_at,
                        "operation_tracking",
                        current_batch_id,
...
def update_operation_batch_record(
    ...
    try:
        ...
        for _, row in edited_rows.iterrows():
            ...
            work_detail = _normalize_work_detail(
                operation_name,
                row.get("İş Detayı", "Ana üretim"),
            )
            counts_toward_progress = int(
                work_detail != _auxiliary_work_label(operation_name)
            )
            fire_qty = max(int(float(row.get("Fire", 0) or 0)), 0)
            if not counts_toward_progress:
                fire_qty = 0
            if counts_toward_progress and fire_qty > processed_qty:
                raise ValueError(
            ...
            if log_id is None:
                cur.execute(
                    ...
                    (
                        ...
                        fire_qty, good_qty, operation_hours,
                        good_qty * unit_area_mm2, work_detail, counts_toward_progress,
                        fire_note, created_at,
                    ),
                )
                inserted_count += 1
                ...
                cur.execute(
                    ...
                    SET item_id = ?, operation_name = ?, processed_qty = ?,
                        fire_qty = ?, good_qty = ?, operation_hours = ?,
                        calculated_area_mm2 = ?, work_detail = ?,
                        counts_toward_progress = ?, fire_note = ?
                    WHERE id = ? AND batch_id = ?
                    """,
                    (
                        item_id, operation_name, processed_qty, fire_qty, good_qty,
                        operation_hours, good_qty * unit_area_mm2, work_detail,
                        counts_toward_progress, fire_note, log_id, int(batch_id),
                    ),
                )
        ...
        result = pd.read_sql_query(
            ...
            SELECT
                ...
                GROUP_CONCAT(DISTINCT CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 0
                    THEN COALESCE(NULLIF(w.work_detail, ''), 'Yardımcı iş')
                    ELSE NULL
                END) AS yardimci_isler,
                SUM(CASE WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.processed_qty ELSE 0 END) AS islem_yapilan,
                SUM(CASE WHEN COALESCE(w.counts_toward_progress, 1) = 0
                    THEN w.processed_qty ELSE 0 END) AS yardimci_is_adedi,
                SUM(CASE WHEN COALESCE(w.counts_toward_progress, 1) = 0
                    THEN w.operation_hours ELSE 0 END) AS yardimci_is_saati,
                SUM(CASE WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.fire_qty ELSE 0 END) AS fire,
                SUM(CASE WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN w.good_qty ELSE 0 END) AS saglam_ilerleyen,
    ...
    try:
        return pd.read_sql_query(
            ...
            SELECT
                ...
                COALESCE(NULLIF(w.work_detail, ''), 'Ana üretim') AS İş_Detayı,
                CASE
                    WHEN COALESCE(w.counts_toward_progress, 1) = 1
                    THEN 'Evet' ELSE 'Hayır'
                END AS Üretime_Sayar,
...
def render_operation_day_end_section() -> bool:
    ...
    progress_mask = (
        pd.to_numeric(
            day.get("uretim_ilerlemesine_sayar", 1),
            errors="coerce",
        ).fillna(1).astype(int).eq(1)
    )
    production_day = day.loc[progress_mask].copy()
    auxiliary_day = day.loc[~progress_mask].copy()

    total_processed = int(production_day["islem_yapilan"].sum())
    total_good = int(production_day["saglam_ilerleyen"].sum())
    total_fire = int(production_day["fire"].sum())
    total_good_area = float(production_day["saglam_alan_mm2"].sum())
    auxiliary_qty = int(auxiliary_day["yardimci_is_adedi"].sum())
    auxiliary_hours = float(auxiliary_day["operasyon_saati"].sum())
        ...
        day_entries.append(
            {
                ...
                "processed_qty": int(
                    row["islem_yapilan"]
                    if int(row.get("uretim_ilerlemesine_sayar", 1) or 0) == 1
                    else row.get("yardimci_is_adedi", 0)
                ),
                "fire_qty": int(row["fire"]),
                "good_qty": int(row["saglam_ilerleyen"]),
                "unit_area_mm2": float(row["unit_area_mm2"]),
                "operation_hours": float(row.get("operasyon_saati", 0) or 0),
                "work_detail": str(row.get("is_detayi", "Ana üretim") or "Ana üretim"),
                "counts_toward_progress": int(row.get("uretim_ilerlemesine_sayar", 1) or 0),
            }
        )
    ...
    if auxiliary_qty > 0 or auxiliary_hours > 0:
        auxiliary_labels = sorted(
            set(auxiliary_day["is_detayi"].dropna().astype(str).tolist())
        )
        st.info(
            f"Yardımcı işler: **{auxiliary_qty} adet · {auxiliary_hours:.2f} saat** "
            f"({', '.join(auxiliary_labels)}). Bu adetler üretim ilerlemesine dahil değildir; "
            "süreleri toplam çalışma ve proje işçiliğine dahildir."
        )
...
def render_project_stage_records():
    """Kayıtlar ekranında proje/OC bazlı etap ilerlemesini gösterir."""
    progress = get_operation_progress()
    if progress.empty:
        render_empty_state(
            "Henüz proje etap planı yok",
            "POS'lara operasyon kombinasyonu atandığında proje bazlı etaplar burada görünür.",
            "▦",
        )
        return
    ...
    numeric_columns = [
        "requested_qty", "completed_qty", "remaining_qty",
        "overproduction_qty", "fire_qty", "processed_qty",
        "completion_pct", "operation_order",
    ]
    for column in numeric_columns:
        progress[column] = pd.to_numeric(
            progress[column], errors="coerce"
        ).fillna(0)

    projects = (
        progress[["oc_no", "project_name"]]
        .drop_duplicates()
        .sort_values(["project_name", "oc_no"], key=lambda series: series.astype(str).str.lower())
        .reset_index(drop=True)
    )
    project_labels = projects.apply(
        lambda row: f"{row['project_name']} · OC {row['oc_no']}",
        axis=1,
    ).tolist()
    label_to_project = {
        label: (str(row["oc_no"]), str(row["project_name"]))
        for label, (_, row) in zip(project_labels, projects.iterrows())
    }

    selected_label = st.selectbox(
        "Proje / OC seç",
        project_labels,
        key="records_project_stage_select",
    )
    selected_oc, selected_project = label_to_project[selected_label]
    selected = progress[
        progress["oc_no"].astype(str).eq(selected_oc)
    ].copy()

    selected["_pos_no"] = pd.to_numeric(
        selected["pos"].astype(str).str.extract(r"(\d+)")[0],
        errors="coerce",
    ).fillna(0)

    unique_items = selected.drop_duplicates("item_id")
    last_stages = (
        selected.sort_values(["item_id", "operation_order"])
        .groupby("item_id", as_index=False)
        .tail(1)
    )

    total_requested = int(unique_items["requested_qty"].sum())
    final_stage_completed = int(last_stages["completed_qty"].sum())
    total_fire = int(selected["fire_qty"].sum())
    total_positions = int(unique_items["item_id"].nunique())

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("POS sayısı", total_positions)
    m2.metric("Proje siparişi", f"{total_requested} adet")
    m3.metric("Son etap tamamlanan", f"{final_stage_completed} adet")
    m4.metric("Toplam fire", f"{total_fire} adet")
    ...
    stage_summary = (
        ...
        .agg(
            pos_sayisi=("item_id", "nunique"),
            siparis_adedi=("requested_qty", "sum"),
            islem_yapilan=("processed_qty", "sum"),
            tamamlanan_adet=("completed_qty", "sum"),
            kalan_adet=("remaining_qty", "sum"),
            fazla_uretim=("overproduction_qty", "sum"),
            fire=("fire_qty", "sum"),
        )
        .sort_values(["operation_order", "operation_name"])
        .reset_index(drop=True)
    )
    stage_summary["ilerleme_yuzde"] = (
        stage_summary["tamamlanan_adet"]
        / stage_summary["siparis_adedi"].replace(0, 1)
        * 100
    ).round(1)

    def _stage_status(row):
        requested = int(row["siparis_adedi"] or 0)
        completed = int(row["tamamlanan_adet"] or 0)
        over = int(row["fazla_uretim"] or 0)
        if completed <= 0:
            return "Başlanmadı"
        if requested > 0 and completed >= requested:
            return "Fazla üretim" if over > 0 else "Tamamlandı"
        return "Devam ediyor"

    stage_summary["durum"] = stage_summary.apply(_stage_status, axis=1)
    stage_display = stage_summary.rename(
        columns={
            "operation_order": "Etap Sırası",
            "operation_name": "Etap",
            "pos_sayisi": "POS Sayısı",
            "siparis_adedi": "Sipariş",
            "islem_yapilan": "İşlem Yapılan",
            "tamamlanan_adet": "Sağlam Tamamlanan",
            "kalan_adet": "Kalan",
            "fazla_uretim": "Fazla Üretim",
            "fire": "Fire",
            "ilerleme_yuzde": "İlerleme %",
            "durum": "Durum",
        }
    )
    integer_columns = [
        "Etap Sırası", "POS Sayısı", "Sipariş", "İşlem Yapılan",
        "Sağlam Tamamlanan", "Kalan", "Fazla Üretim", "Fire",
    ]
    for column in integer_columns:
        stage_display[column] = stage_display[column].astype(int)

    st.markdown(f"### {selected_project} etap özeti")
    st.dataframe(
        stage_display,
        use_container_width=True,
        hide_index=True,
    ...
    st.markdown("### POS bazlı etap ayrıntısı")
    detail_display = (
        selected.sort_values(["_pos_no", "operation_order", "operation_name"])
        .rename(
            columns={
                "project_name": "Proje",
                "oc_no": "OC",
                ...
                "operation_order": "Etap Sırası",
                "operation_name": "Etap",
                "requested_qty": "Sipariş",
                "processed_qty": "İşlem Yapılan",
                "completed_qty": "Sağlam Tamamlanan",
                "remaining_qty": "Kalan",
                "overproduction_qty": "Fazla Üretim",
                "fire_qty": "Fire",
                "completion_pct": "İlerleme %",
        ...
        "Proje", "OC", "POS", "Kombinasyon", "Etap Sırası", "Etap",
        "Sipariş", "İşlem Yapılan", "Sağlam Tamamlanan", "Kalan",
        "Fazla Üretim", "Fire", "İlerleme %",
    ]]
    for column in [
        "Etap Sırası", "Sipariş", "İşlem Yapılan", "Sağlam Tamamlanan",
        "Kalan", "Fazla Üretim", "Fire",
    ]:
        detail_display[column] = detail_display[column].astype(int)
...
def render_filtered_person_records():
    """Operasyon hareketlerini kayıt, kişi, proje, POS ve etap bazında filtreler."""
    history = get_operation_history()
    if history.empty:
        render_empty_state(
            "Filtrelenecek operasyon kaydı yok",
            "Yeni operasyon kayıtları oluşturulduğunda kişi ve proje filtreleri burada görünecek.",
            "⌕",
        )
        return
    ...
    history["batch_id"] = pd.to_numeric(
        history["batch_id"], errors="coerce"
    ).fillna(0).astype(int)
    history["islem_yapilan"] = pd.to_numeric(
        history["islem_yapilan"], errors="coerce"
    ).fillna(0)
    history["saglam_ilerleyen"] = pd.to_numeric(
        history["saglam_ilerleyen"], errors="coerce"
    ).fillna(0)
    history["fire"] = pd.to_numeric(
        history["fire"], errors="coerce"
    ).fillna(0)
    history["operasyon_saati"] = pd.to_numeric(
        history["operasyon_saati"], errors="coerce"
    ).fillna(0.0)
    history["calisma_saati"] = pd.to_numeric(
        history["calisma_saati"], errors="coerce"
    ).fillna(0.0)
    history["saglam_alan_mm2"] = pd.to_numeric(
        history["saglam_alan_mm2"], errors="coerce"
    ).fillna(0.0)

    min_date = history["tarih_dt"].min().date()
    max_date = history["tarih_dt"].max().date()

    st.markdown("### Kayıt ve kişi bazlı filtreleme")
    st.caption(
        "Her satır tek bir çalışan–OC–POS–etap hareketidir. Aynı ürün farklı "
        "etaplardan geçtiğinde etap adetleri ayrı satırlarda görünür."
    )
    ...
    with f2:
        end_date = st.date_input(
            ...
            min_value=min_date,
            max_value=max_date,
            key="record_filter_end_date",
        )
    with f3:
        record_number = st.text_input(
            "Kayıt no (isteğe bağlı)",
            key="record_filter_batch_id",
            placeholder="Örnek: 125",
        ).strip()
    ...
    filtered = history[
        history["tarih_dt"].dt.date.between(start_date, end_date)
    ].copy()

    worker_options = sorted(
        filtered["operator_ismi"].dropna().astype(str).unique().tolist()
    )
    project_rows = (
        filtered[["OC", "proje_adi"]]
        .dropna(subset=["OC"])
        .drop_duplicates()
        .sort_values(["proje_adi", "OC"])
    )
    project_labels = ["Tümü"] + [
        f"{str(row.proje_adi)} · OC {str(row.OC)}"
        for row in project_rows.itertuples(index=False)
    ]
    project_label_to_oc = {
        f"{str(row.proje_adi)} · OC {str(row.OC)}": str(row.OC)
        for row in project_rows.itertuples(index=False)
    }
    ...
    with f4:
        selected_workers = st.multiselect(
            ...
            worker_options,
            default=[],
            key="record_filter_workers",
            help="Boş bırakırsan bütün çalışanlar gösterilir.",
        )
    with f5:
        selected_project_label = st.selectbox(
            "Proje / OC",
            project_labels,
            key="record_filter_project",
        )

    if selected_workers:
        filtered = filtered[
            filtered["operator_ismi"].astype(str).isin(selected_workers)
        ].copy()
    if selected_project_label != "Tümü":
        selected_oc = project_label_to_oc[selected_project_label]
        filtered = filtered[filtered["OC"].astype(str) == selected_oc].copy()

    pos_options = sorted(
        filtered["POS"].dropna().astype(str).unique().tolist(),
        key=lambda value: int(re.search(r"\d+", value).group())
        if re.search(r"\d+", value)
        else 0,
    )
    operation_options = sorted(
        filtered["operasyon"].dropna().astype(str).unique().tolist()
    )
    ...
    with f6:
        selected_positions = st.multiselect(
            ...
            pos_options,
            default=[],
            key="record_filter_positions",
            help="Boş bırakırsan seçilen projedeki bütün POS'lar gösterilir.",
        )
    with f7:
        selected_operations = st.multiselect(
            ...
            operation_options,
            default=[],
            key="record_filter_operations",
            help="Boş bırakırsan bütün etaplar gösterilir.",
        )

    if selected_positions:
        filtered = filtered[
            filtered["POS"].astype(str).isin(selected_positions)
        ].copy()
    if selected_operations:
        filtered = filtered[
            filtered["operasyon"].astype(str).isin(selected_operations)
        ].copy()

    if record_number:
        if record_number.isdigit():
            filtered = filtered[
                filtered["batch_id"] == int(record_number)
            ].copy()
        else:
            st.warning("Kayıt no yalnızca rakamlardan oluşmalıdır.")
            return
    ...
    unique_batches = filtered[
        ["batch_id", "operator_ismi", "calisma_saati"]
    ].drop_duplicates("batch_id")
    total_batch_hours = float(unique_batches["calisma_saati"].sum())
    total_good_area_m2 = float(filtered["saglam_alan_mm2"].sum()) / 1_000_000

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("Kayıt", int(filtered["batch_id"].nunique()))
    m2.metric("Çalışan", int(filtered["operator_ismi"].nunique()))
    m3.metric("Etap satırı", int(len(filtered)))
    m4.metric("Kayıt saati", f"{total_batch_hours:.2f}")
    m5.metric("Etap sağlam alanı", f"{total_good_area_m2:.1f} m²")

    st.info(
        "Etap adetleri fiziksel bitmiş ürün toplamı değildir. Örneğin aynı 20 parça "
        "Kesim ve Taşlama kayıtlarında ayrı ayrı 20 adet görünür."
    )

    person_stage = (
        filtered.groupby("operator_ismi", as_index=False)
        .agg(
            kayit_sayisi=("batch_id", "nunique"),
            etap_satiri=("batch_id", "size"),
            islem_yapilan=("islem_yapilan", "sum"),
            saglam_etap_adedi=("saglam_ilerleyen", "sum"),
            fire=("fire", "sum"),
    ...
    person_hours = (
        filtered[["batch_id", "operator_ismi", "calisma_saati"]]
        .drop_duplicates("batch_id")
        .groupby("operator_ismi", as_index=False)["calisma_saati"]
        .sum()
    )
    person_stage = person_stage.merge(
        person_hours,
        on="operator_ismi",
        how="left",
    )
    person_stage["saglam_alan_m2"] = (
        person_stage["saglam_alan_mm2"] / 1_000_000
    ).round(1)
    person_stage["fire_yuzde"] = (
        person_stage["fire"]
        / person_stage["islem_yapilan"].replace(0, 1)
        * 100
    ).round(2)
    person_display = person_stage.rename(
        columns={
            "operator_ismi": "Çalışan",
            "kayit_sayisi": "Kayıt Sayısı",
            "etap_satiri": "Etap Satırı",
            "calisma_saati": "Kayıt Saati",
            "operasyon_saati": "Operasyon Saati",
            "islem_yapilan": "İşlem Yapılan",
            "saglam_etap_adedi": "Etap Sağlam Toplamı",
            "fire": "Fire",
            "fire_yuzde": "Fire %",
            "saglam_alan_m2": "Etap Sağlam Alanı (m²)",
        }
    )[[
        "Çalışan", "Kayıt Sayısı", "Etap Satırı", "Kayıt Saati",
        "Operasyon Saati", "İşlem Yapılan", "Etap Sağlam Toplamı",
        "Fire", "Fire %", "Etap Sağlam Alanı (m²)",
    ]]

    st.markdown("### Kişi özeti")
    st.dataframe(
        person_display.sort_values(
            ["Kayıt Sayısı", "Çalışan"], ascending=[False, True]
        ),
        use_container_width=True,
        hide_index=True,
    )

    record_display = filtered.copy()
    record_display["Tarih"] = record_display["tarih_dt"].dt.strftime("%d/%m/%Y")
    record_display["Sağlam Alan (m²)"] = (
        record_display["saglam_alan_mm2"] / 1_000_000
    ).round(1)
    record_display["Fire %"] = (
        record_display["fire"]
        / record_display["islem_yapilan"].replace(0, 1)
        * 100
    ).round(2)
    record_display = record_display.rename(
        columns={
            "batch_id": "Kayıt No",
            "operator_ismi": "Çalışan",
            "proje_adi": "Proje",
    ...
    record_display = record_display[[
        "Kayıt No", "Tarih", "Çalışan", "Proje", "OC", "POS", "Etap",
        "Çalışma Tipi", "Kayıt Saati", "Operasyon Saati", "İşlem Yapılan",
        "Sağlam İlerleyen", "Fire", "Fire %", "Sağlam Alan (m²)",
        "Not", "Kayıt Zamanı",
    ]].sort_values(["Tarih", "Kayıt No", "POS", "Etap"], ascending=[False, False, True, True])
    ...
    st.download_button(
        "Filtrelenmiş Kayıtları CSV İndir",
        data=csv,
        file_name=(
            f"filtrelenmis_operasyon_kayitlari_"
            f"{start_date.isoformat()}_{end_date.isoformat()}.csv"
        ),
        mime="text/csv",
        use_container_width=True,
        key="download_filtered_person_records",
    )

    batch_summary = (
        filtered.groupby("batch_id", as_index=False)
        .agg(
            tarih=("tarih", "first"),
            operator_ismi=("operator_ismi", "first"),
            OC=("OC", lambda values: ", ".join(sorted(set(map(str, values))))),
            POS=("POS", lambda values: ", ".join(sorted(set(map(str, values))))),
            etaplar=("operasyon", lambda values: ", ".join(dict.fromkeys(map(str, values)))),
        )
        .sort_values(["tarih", "batch_id"], ascending=[False, False])
    )
    batch_labels = batch_summary.apply(
        lambda row: (
            f"#{int(row['batch_id'])} · {row['tarih']} · {row['operator_ismi']} · "
            f"OC {row['OC']} · {row['POS']} · {row['etaplar']}"
        ),
        axis=1,
    ).tolist()
    selected_batch_label = st.selectbox(
        "Tek bir kaydın bütün satırlarını göster",
        batch_labels,
        key="filtered_record_detail_select",
    )
    selected_batch_id = int(
        batch_summary.iloc[batch_labels.index(selected_batch_label)]["batch_id"]
    )
    st.dataframe(
        get_operation_batch_details(selected_batch_id),
        use_container_width=True,
        hide_index=True,
    )
    st.caption(
        "Bu kaydı değiştirmek veya silmek için Operasyon Kayıtları sekmesini kullan."
    )
...
def kayitlar_page():
    ...
    operation_tab, filtered_records_tab, project_stage_tab, other_tab, legacy_tab = st.tabs(
        [
            "Operasyon Kayıtları",
            "Kişi / Kayıt Filtreleme",
            "Proje Bazlı Etaplar",
            "Diğer Çalışmalar",
    ...
    with operation_tab:
        if operation_batches.empty:
            ...
            editable_details = details.rename(
                ...
                "kayıt_no", "OC", "POS", "İşlem", "İş Detayı",
            ...
            available_operations = sorted(
                ...
                | set(
                    get_performance_targets()["operation_name"]
                    .dropna().astype(str).tolist()
                )
                | set(
                    plan_operations["operation_name"]
                    .dropna().astype(str).tolist()
                )
                | {"Kesim"}
            )
            ...
            edited_details = st.data_editor(
                ...
                column_config={
                    ...
                    "İş Detayı": st.column_config.SelectboxColumn(
                        "İş Detayı",
                        options=[
                            "Ana üretim",
                            "Çıta taşlama",
                        ...
                        required=True,
                        help=(
                            "Yardımcı iş seçildiğinde adet POS ilerlemesine yazılmaz; "
                            "işlem süresi proje saatine dahil edilir."
                        ),
                    ),
    ...
    with filtered_records_tab:
        render_filtered_person_records()

    with project_stage_tab:
        render_project_stage_records()
...
def grafikler_page():
    st.markdown(
        ...
        <div style="padding:20px 22px;border-radius:16px;background:linear-gradient(135deg,#0F2F49,#145E75);color:white;margin-bottom:14px;">
            ...
            <div style="font-size:14px;opacity:.88;margin-top:5px;">Genel bakışta aynı ürün yalnızca bir kez sayılır; operasyon, çalışan, boya ve lazer performansı ayrı analiz edilir.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    min_date = history["tarih_dt"].min().date()
    max_date = history["tarih_dt"].max().date()
    # Proje seçimi operasyon geçmişinden oluşturulur; "Tümü" seçildiğinde ise
    # sisteme yüklenmiş bütün üretim çıktıları toplam alana dahil edilir.
    project_output_all = get_operation_overview()
    all_output_ocs = (
        sorted(project_output_all["oc_no"].dropna().astype(str).unique().tolist())
        if not project_output_all.empty
        else sorted(history["OC"].dropna().astype(str).unique().tolist())
    )
    project_rows = (
    ...
    individual_project_labels = project_rows.apply(
        lambda row: f"{row['proje_adi']} — OC {row['OC']}", axis=1
    ).tolist()
    project_labels = ["Tümü"] + individual_project_labels
    project_to_oc = dict(
        zip(individual_project_labels, project_rows["OC"].astype(str))
    )
    ...
    with st.container(border=True):
        ...
        with f0:
            selected_project_label = st.selectbox(
                "Proje / OC",
                project_labels,
                key="analysis_project_filter_v217_all_projects",
            )
            all_projects_selected = selected_project_label == "Tümü"
            selected_oc = (
                None
                if all_projects_selected
                else project_to_oc[selected_project_label]
            )
            selected_ocs = (
                all_output_ocs
                if all_projects_selected
                else [str(selected_oc)]
            )
        with f1:
        ...
        with f2:
            workers = ["Tümü"] + sorted(
                person_history.loc[
                    person_history["OC"].astype(str).isin(selected_ocs),
                    "operator_ismi",
        ...
        with f3:
            operations = ["Tümü"] + sorted(
                history.loc[
                    history["OC"].astype(str).isin(selected_ocs),
                    "operasyon",
                ].dropna().astype(str).unique().tolist()
            )
    ...
    # Genel bakışta aynı parça lazer, abkant, kaynak, boya vb. her etapta
    # yeniden sayılmaz. Bir POS için herhangi bir operasyonda ulaşılan en yüksek
    # sağlam adet esas alınır; yalnızca önceki en yüksek değeri aşan kısım o güne yazılır.
    if all_projects_selected:
        completed_daily_frames = [
            get_project_unique_production_daily(
                oc_number,
                start_date.isoformat(),
                end_date.isoformat(),
            )
            for oc_number in selected_ocs
        ]
        completed_daily_frames = [
            frame for frame in completed_daily_frames if not frame.empty
        ]
        if completed_daily_frames:
            completed_daily = (
                pd.concat(completed_daily_frames, ignore_index=True)
                .groupby("tarih_dt", as_index=False)
                .agg(
                    günlük_m2=("günlük_m2", "sum"),
                    sağlam_adet=("sağlam_adet", "sum"),
                )
                .sort_values("tarih_dt")
            )
            completed_daily["günlük_m2"] = completed_daily["günlük_m2"].round(1)
        else:
            completed_daily = pd.DataFrame(
                columns=["tarih_dt", "günlük_m2", "sağlam_adet"]
            )
    else:
        completed_daily = get_project_unique_production_daily(
            str(selected_oc),
            start_date.isoformat(),
            end_date.isoformat(),
        )
    operation_daily_stats = (
    ...
    daily = operation_daily_stats.merge(
        completed_daily,
        on="tarih_dt",
        how="outer",
    ).sort_values("tarih_dt")
    for column in ["günlük_m2", "sağlam_adet", "işlem_yapılan", "fire", "çalışan"]:
        if column not in daily.columns:
            daily[column] = 0
        daily[column] = pd.to_numeric(daily[column], errors="coerce").fillna(0)
    daily["günlük_m2"] = daily["günlük_m2"].round(1)
    daily["sağlam_adet"] = daily["sağlam_adet"].astype(int)
    daily["işlem_yapılan"] = daily["işlem_yapılan"].astype(int)
    daily["fire"] = daily["fire"].astype(int)
    daily["çalışan"] = daily["çalışan"].astype(int)
    daily["fire_yüzde"] = (
    ...
    project_output = project_output_all.copy()
    if not all_projects_selected:
        project_output = project_output[
            project_output["oc_no"].astype(str).eq(str(selected_oc))
        ].copy()
    if project_output.empty:
        ...
        project_total_m2 = (
            float(project_output["requested_area_mm2"].sum()) / 1_000_000
        )
        project_produced_m2 = (
            float(project_output["unique_produced_area_mm2"].sum()) / 1_000_000
        )
        project_total_qty = int(project_output["requested_qty"].sum())
        project_produced_qty = int(project_output["unique_produced_qty"].sum())

    period_unique_m2 = float(daily["günlük_m2"].sum()) if not daily.empty else 0.0
    ...
    if all_projects_selected:
        project_count = int(project_output["oc_no"].astype(str).nunique()) if not project_output.empty else 0
        st.caption(f"Gösterilen kapsam: **Tüm projeler ({project_count} OC)**")
        area_metric_title = "Tüm projelerde üretime giren alan"
        qty_metric_title = "Tüm projelerde üretime giren adet"
    else:
        st.caption(f"Gösterilen proje: **{selected_project_label}**")
        area_metric_title = "Proje üretilen alan"
        qty_metric_title = "Proje üretilen adet"
    ...
    k3.metric("Seçilen dönemde üretime giren", f"{period_unique_m2:.1f} m²")
    ...
    with tab_general:
        entered_m2 = min(project_produced_m2, project_total_m2)
        not_entered_m2 = max(project_total_m2 - project_produced_m2, 0.0)
        progress_chart = pd.DataFrame({
            "Durum": ["Üretime giren", "Henüz üretime girmeyen"],
            "Alan (m²)": [entered_m2, not_entered_m2],
        })
        ...
        with right:
            _vega_pie_chart(
                ...
                "Alan (m²)",
                (
                    "Tüm projelerin alan ilerlemesi"
                    if all_projects_selected
                    else "Proje alan ilerlemesi"
                ),
            )
            p1, p2 = st.columns(2)
            p1.metric("Üretime giren", f"{entered_m2:.1f} m²")
            p2.metric("Henüz üretime girmeyen", f"{not_entered_m2:.1f} m²")

        st.caption(
            "Genel bakışta bir POS için ulaşılan en yüksek sağlam adet kullanılır; "
            "aynı ürün sonraki operasyonlarda yeniden sayılmaz. Tam biten ürün adedi "
            "Operasyon Takibi sayfasında ayrıca gösterilir."
        )
        st.markdown("#### Günlük yönetim özeti")
        daily_display = daily.rename(columns={
            "tarih_dt": "Tarih",
            "günlük_m2": "Üretime Giren Alan (m²)",
            "sağlam_adet": "Üretime Giren Adet",
            "işlem_yapılan": "Operasyon İşlem Adedi",
            "fire": "Fire",
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Üretime Giren Alan (m²)": st.column_config.NumberColumn(format="%.1f"),

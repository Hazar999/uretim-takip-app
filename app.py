        ...
        for entry in entries:
            ...
            if operation_kind == "abkant":
                ...
                cur.execute(
                    """
                    SELECT
                        COALESCE(SUM(abkant_team_qty), 0),
                ...
                previous_team_qty, previous_processed = cur.fetchone()
                cur.execute(
                    "SELECT requested_qty FROM production_output_items WHERE id = ?",
                    (item_id,),
                )
                requested_for_item = int(cur.fetchone()[0] or 0)
                completed_after = _abkant_completed_qty_from_teams(
                    int(previous_team_qty or 0) + new_team_qty,
                    teams_per_piece,
                    requested_for_item,
                )
                processed_qty = max(
            ...
            if fire_qty < 0 or fire_qty > processed_qty:
                if operation_kind == "abkant" and processed_qty == 0 and fire_qty > 0:
                    raise ValueError(
                        f"{operation_name}: Girilen takım sayısıyla henüz tamamlanan ürün oluşmadığı "
    ...
    def build_entries(selected_items: pd.DataFrame, plans: dict, selected_map: dict):
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                if operation_kind == "abkant":
                    # Operatör doğrudan takım sayısını girer. Sistem takım/adet
                    # oranına göre ürün adedini otomatik hesaplar. Önceki kayıttan
                    # artan takım varsa yeni girişle birleştirilir.
                    abkant_teams_per_piece = max(
                        int(
                            st.session_state.get(
                                abkant_teams_per_piece_key,
                                saved_teams_per_piece,
                            )
                            or 1
                        ),
                        1,
                    ...
                    abkant_team_qty = max(
                        int(
                            st.session_state.get(
                                abkant_team_qty_key,
                                saved_entry.get("abkant_team_qty", 0),
                            )
                            or 0
                        ),
                        0,
                    ...
                    previous_team_qty = (
                        int(progress_row.get("abkant_team_qty", 0) or 0)
                    ...
                    completed_after = _abkant_completed_qty_from_teams(
                        previous_team_qty + abkant_team_qty,
                        abkant_teams_per_piece,
                        requested,
                    )
                    processed = max(completed_after - previous_processed, 0)

                    # Kenar alanları hesap için kullanılmaz; yalnızca seçilen proje
                    # kuralının raporlarda görünmesi için takım sayısı ilgili türe yazılır.
...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            ...
                            st.caption(
                                "Uzun ve kısa kenar seçimleri yalnızca projedeki büküm "
                                "şeklini belirtir. Üretim ilerlemesi takım sayısından adede çevrilir."
                            )
                            ...
                            with team1:
                                st.number_input(
                                    "Yapılan takım sayısı",
                                    min_value=0,
                                    step=1,
                                    key=abkant_team_qty_key,
                                    help="Operatörün bu kayıtta tamamladığı toplam takım sayısı.",
                                )
                            with team2:
                                st.number_input(
                                    ...
                                    help="Genellikle 1'dir. Bir ürün için 2 takım gerekiyorsa 2 seçin.",
                                )
                            with team3:
                                st.number_input(
                                    "Fire (adet)",
                                    ...
                                    step=1,
                                    key=fire_key,
                                    help="Takımdan adede çevrildikten sonra fire çıkan ürün adedi.",
                                )
                            with team4:
                                st.number_input(
                                    "Süre (saat)",
                            ...
                            current_team_qty = max(
                                int(st.session_state.get(abkant_team_qty_key, 0) or 0),
                                0,
                            )
                            ...
                            previous_team_excess = (
                                previous_team_qty % current_teams_per_piece
                            )
                            teams_needed_for_remaining = max(
                                remaining_qty * current_teams_per_piece
                                - previous_team_excess,
                                0,
                            )
                            st.button(
                                "Kalan adet için takım sayısını yaz",
                                disabled=(remaining_qty == 0),
                                use_container_width=True,
                                key=(
                                    f"fill_abkant_team_{item_id}_"
                                    f"{normalized_operation}_{version}"
                                ),
                                on_click=set_session_value,
                                args=(
                                    abkant_team_qty_key,
                                    teams_needed_for_remaining,
                                ),
                            )

                            completed_after = _abkant_completed_qty_from_teams(
                                previous_team_qty + current_team_qty,
                            ...
                            cumulative_team_excess = (
                                previous_team_qty + current_team_qty
                            ) % current_teams_per_piece

                            st.info(
                                f"**Hesap:** {current_team_qty} takım ÷ "
                                f"{current_teams_per_piece} takım/adet → bu kayıtta "
                                f"**{current_completed_qty} adet** ilerler. "
                                f"Artan takım: **{cumulative_team_excess}**"
                            )

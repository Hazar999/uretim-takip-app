...
def main():
    ...
    st.caption("İşçi çalışma süresi, POS, kombinasyon, adet ve fire bilgilerini kaydeder. — Sürüm 1.5")
    st.success("GÜNCEL SÜRÜM 1.5: Grafikler ve Tümünü Seç özellikleri aktiftir.")

    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 1.5")
        page = st.radio("Sayfa", ["Yeni Kayıt", "Kayıtlar", "Grafikler", "Özet"], index=0)
        st.divider()

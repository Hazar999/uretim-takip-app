    ...
    def build_entries(selected_items: pd.DataFrame, plans: dict, selected_map: dict):
        ...
        for _, item in selected_items.iterrows():
            ...
            for operation in selected_map.get(item_id, []):
                ...
                if operation_kind == "abkant":
                    abkant_activity = any([
                        int(st.session_state.get(abkant_long_single_bend_qty_key, 0) or 0) > 0,
                        int(st.session_state.get(abkant_long_double_bend_qty_key, 0) or 0) > 0,
                        int(st.session_state.get(abkant_short_single_bend_qty_key, 0) or 0) > 0,
                        int(st.session_state.get(abkant_short_double_bend_qty_key, 0) or 0) > 0,
                        operation_hours > 0,
                        fire > 0,
                    ])

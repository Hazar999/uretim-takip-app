...
def get_db_connection():
    """
    Her veritabanı işlemi için bağımsız bağlantı açar.

    Streamlit birden fazla yeniden çalıştırmayı farklı iş parçacıklarında
    yürütebildiğinden, tek bağlantıyı kilitleyerek paylaşmak uygulamanın
    beklemesine neden olabilir. Kayıtlar yine aynı Turso veritabanına gider.
    """
...
def _clear_data_caches():
    """
    Yalnızca veritabanından okuyan önbellekleri temizler.

    Tüm Streamlit önbelleğini silmek Excel/PDF ayrıştırmalarını ve diğer ağır
    hesapları da yeniden başlatıyordu. Bu da kayıt sonrasında donmuş gibi bir
    ekran oluşturabiliyordu.
    """
    cached_readers = (
        "get_update_history",
...
def main():
    ...
    st.caption("Sürüm 2.16.4 · KALICI DB / STABİL BAĞLANTI")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.16.4 — DONMA DÜZELTİLDİ · STABİL TURSO BAĞLANTISI
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.16.4 · KALICI DB / STABİL BAĞLANTI
            </div>
            """,
            unsafe_allow_html=True,
        )

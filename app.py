...
# -----------------------------
# TEK SEFERLİK 23 TEMMUZ BOYA / PAKETLEME GEÇMİŞ KAYDI
# -----------------------------
JULY23_TEAM_BULK_MARKER = "team_20260723_neovac_pack_iss_paint_heraklion_paint_v1"
JULY23_TEAM_DATE = "2026-07-23"
JULY23_TEAM_SEGMENTS = [
    {
        "key": "ilk_5_saat",
        "hours": 5.0,
        "operator": "Nezir Azim",
        "participants": ["Salih Kayar", "Köksal Atıl", "Yaşettin Baysal"],
        "label": "İlk 5 saat",
    },
    {
        "key": "son_3_saat",
...
JULY23_ISS_UNKNOWN_TOTAL = 6
JULY23_HERAKLION_POS = "POS10"
JULY23_HERAKLION_BOYA_QTY = 106


def _july23_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Bir ekip diliminin saatini satırlardaki adetlere orantılı dağıtır."""
    if not weights:
        return []
    safe_weights = [max(float(value or 0), 0.0) for value in weights]
    weight_sum = sum(safe_weights)
    if weight_sum <= 0:
        result = [round(float(total) / len(weights), 2) for _ in weights]
    else:
        result = [
            round(float(total) * value / weight_sum, 2)
            for value in safe_weights
        ]
    result[-1] = round(
        result[-1] + round(float(total) - sum(result), 2),
        2,
    )
    return result


def _july23_dedupe_operations(operations: list[str]) -> list[str]:
    """Operasyon sırasını koruyarak aynı operasyonu tekilleştirir."""
    result = []
    seen = set()
    for raw_operation in operations or []:
        ...
        if not operation:
            continue
        key = _normalize_header(operation)
        if key in seen:
            continue
        seen.add(key)
...
def _july23_insert_operation(
    plan_operations: list[str],
    operation_name: str,
    operation_kind: str,
) -> list[str]:
    """Eksik boya/paketleme operasyonunu üretim sırasındaki uygun yere ekler."""
    operations = _july23_dedupe_operations(plan_operations)
    if any(_operation_kind(name) == operation_kind for name in operations):
        return operations

    insert_at = len(operations)
    if operation_kind == "boya":
        before_kinds = {"paketleme", "sevkiyat"}
    elif operation_kind == "paketleme":
        before_kinds = {"sevkiyat"}
    else:
        before_kinds = set()

    for index, current_name in enumerate(operations):
        if _operation_kind(current_name) in before_kinds:
            insert_at = index
            break
    operations.insert(insert_at, str(operation_name))
    return _july23_dedupe_operations(operations)


def _july23_segment_marker(segment_key: str) -> str:
    return f"{JULY23_TEAM_BULK_MARKER}:{segment_key}"


def _july23_iss_marker(segment_key: str) -> str:
    return f"{JULY23_TEAM_BULK_MARKER}:iss_unknown:{segment_key}"


def _july23_bulk_already_saved() -> tuple[bool, str]:
    """Aynı geçmiş kaydın ikinci kez oluşturulmasını engeller."""
    conn = get_db_connection()
    try:
        setting = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (JULY23_TEAM_BULK_MARKER,),
        ).fetchone()
        if setting is not None:
            return True, str(setting[0] or "")

        batch_count = int(conn.execute(
            "SELECT COUNT(*) FROM operation_batches WHERE notlar LIKE ?",
            (f"%{JULY23_TEAM_BULK_MARKER}%",),
        ).fetchone()[0] or 0)
        other_count = int(conn.execute(
            "SELECT COUNT(*) FROM other_work_logs WHERE notlar LIKE ?",
            (f"%{JULY23_TEAM_BULK_MARKER}%",),
        ).fetchone()[0] or 0)
        if batch_count > 0 or other_count > 0:
            return True, f"{batch_count} operasyon kaydı, {other_count} POS'suz kayıt"
        return False, ""
    finally:
        conn.close()


def _july23_project_options(project_token: str) -> pd.DataFrame:
    """Proje adında verilen anahtar geçen OC seçeneklerini getirir."""
    items = get_production_output_summary()
    if items.empty:
        return pd.DataFrame(columns=["oc_no", "project_name"])
    token = _normalize_header(project_token)
    filtered = items[
        items["project_name"]
        .astype(str)
        .map(_normalize_header)
        .str.contains(token, na=False)
    ][["oc_no", "project_name"]].drop_duplicates()
    return filtered.sort_values(["project_name", "oc_no"]).reset_index(drop=True)


def _july23_get_operation_items(
    selected_oc: str,
    operation_kind: str,
    pos_filter: str | None = None,
) -> pd.DataFrame:
    """Seçilen OC için planı ve hedef operasyon ilerlemesini hazırlar."""
    conn = get_db_connection()
    try:
        items = pd.read_sql_query(
            ...
            SELECT
                id AS item_id,
                oc_no,
                COALESCE(NULLIF(project_name, ''), 'OC ' || oc_no) AS project_name,
                pos,
                requested_qty,
                boy_mm,
                en_mm,
                unit_area_mm2,
                COALESCE(combination_name, '') AS combination_name
            FROM production_output_items
            WHERE oc_no = ?
            ORDER BY pos
            """,
            conn,
            params=(str(selected_oc),),
        )
        if items.empty:
            return items
        if pos_filter:
            normalized_pos = _pos_label(pos_filter)
            items = items[items["pos"].astype(str).map(_pos_label) == normalized_pos].copy()
        if items.empty:
            return items

        default_operation = "boya" if operation_kind == "boya" else "paketleme"
        cur = conn.cursor()
        enriched = []
        for _, item in items.iterrows():
            ...
            plan_operations = [str(row[0]) for row in plan_rows]

            log_rows = cur.execute(
                """
                SELECT operation_name, COALESCE(SUM(good_qty), 0), MIN(id)
                FROM operation_work_logs
                WHERE item_id = ?
            ...
            log_operations = [str(row[0]) for row in log_rows]
            matching_logs = [
                (str(name), int(good_qty or 0))
                for name, good_qty, _ in log_rows
                if _operation_kind(str(name)) == operation_kind
            ]

            operation_name = next(
                (
                    name for name in plan_operations
                    if _operation_kind(name) == operation_kind
                ),
                "",
            )
            if not operation_name and matching_logs:
                operation_name = matching_logs[-1][0]
            if not operation_name:
                operation_name = default_operation

            effective_plan = plan_operations or log_operations
            effective_plan = _july23_insert_operation(
                effective_plan,
                operation_name,
                operation_kind,
            )
            combination_name = str(item.get("combination_name", "") or "").strip()
            if not combination_name:
                combination_name = combination_name_from_operations(effective_plan)

            row = item.to_dict()
            row["plan_operations"] = effective_plan
            row["operation_name"] = operation_name
            row["operation_kind"] = operation_kind
            row["existing_good"] = sum(value for _, value in matching_logs)
            row["combination_name"] = combination_name
            row["plan_was_missing"] = not bool(plan_operations)
            enriched.append(row)
...
def _july23_split_quantities(
    rows: list[dict],
) -> dict[str, dict[int, int]]:
    """Kayıt adetlerini 5 saat / 3 saat oranında iki ekip dilimine böler."""
    specs = JULY23_TEAM_SEGMENTS
    result = {str(spec["key"]): {} for spec in specs}
    first, second = specs
    first_key = str(first["key"])
    second_key = str(second["key"])
    ratio = float(first["hours"]) / sum(float(spec["hours"]) for spec in specs)

    total_qty = sum(max(int(row["quantity"]), 0) for row in rows)
    first_target = int(round(total_qty * ratio))
    fractions = []
    first_total = 0
    for row_index, row in enumerate(rows):
        item_id = int(row["item_id"])
        quantity = max(int(row["quantity"]), 0)
        ideal_first = quantity * ratio
        first_qty = int(ideal_first // 1)
        result[first_key][item_id] = first_qty
        result[second_key][item_id] = quantity - first_qty
        first_total += first_qty
        fractions.append((ideal_first - first_qty, row_index, item_id))

    extras_needed = max(first_target - first_total, 0)
    for _, _, item_id in sorted(
        fractions,
        key=lambda value: (-value[0], value[1]),
    )[:extras_needed]:
        if result[second_key][item_id] > 0:
            result[first_key][item_id] += 1
            result[second_key][item_id] -= 1
    return result


def _july23_build_plan(
    neovac_oc: str,
    heraklion_oc: str,
) -> dict:
    """Neo Vac paketleme ve Heraklion boya kayıtlarını iki ekip dilimine hazırlar."""
    rows = []
    warnings = []

    neovac_items = _july23_get_operation_items(
        neovac_oc,
        "paketleme",
    )
    for item in neovac_items.itertuples(index=False):
        requested_qty = max(int(item.requested_qty or 0), 0)
        existing_good = max(int(item.existing_good or 0), 0)
        remaining_qty = max(requested_qty - existing_good, 0)
        if remaining_qty <= 0:
            continue
        if bool(item.plan_was_missing):
            warnings.append(
                f"Neo Vac {item.pos}: operasyon planı geçmiş kayıtlardan oluşturuldu."
            )
        rows.append({
            "item_id": int(item.item_id),
            ...
            "operation_kind": "paketleme",
            "quantity": int(remaining_qty),
            "remaining_before": int(remaining_qty),
            "unit_area_mm2": float(item.unit_area_mm2 or 0),
            "boy_mm": float(item.boy_mm or 0),
            "en_mm": float(item.en_mm or 0),
            "plan_operations": list(item.plan_operations or []),
            "combination_name": str(item.combination_name or ""),
        })

    heraklion_items = _july23_get_operation_items(
        heraklion_oc,
        "boya",
        JULY23_HERAKLION_POS,
    )
    if heraklion_items.empty:
        warnings.append(
            f"Heraklion OC {heraklion_oc} içinde {JULY23_HERAKLION_POS} bulunamadı."
        )
    else:
        item = heraklion_items.iloc[0]
        requested_qty = max(int(item["requested_qty"] or 0), 0)
        existing_good = max(int(item["existing_good"] or 0), 0)
        remaining_before = max(requested_qty - existing_good, 0)
        if bool(item["plan_was_missing"]):
            warnings.append(
                f"Heraklion {item['pos']}: operasyon planı geçmiş kayıtlardan oluşturuldu."
            )
        if JULY23_HERAKLION_BOYA_QTY > remaining_before:
            warnings.append(
                f"Heraklion {item['pos']}: 106 adet, mevcut {remaining_before} kalan adedi "
                "aşıyorsa fazla üretim olarak kaydedilir."
            )
        rows.append({
            "item_id": int(item["item_id"]),
            ...
            "operation_name": str(item["operation_name"]),
            "operation_kind": "boya",
            "quantity": JULY23_HERAKLION_BOYA_QTY,
    ...
    allocations = _july23_split_quantities(rows)
    row_lookup = {int(row["item_id"]): row for row in rows}
    segment_plans = []
    for spec in JULY23_TEAM_SEGMENTS:
        key = str(spec["key"])
        segment_rows = [
            (row_lookup[item_id], int(quantity))
            for item_id, quantity in allocations.get(key, {}).items()
            if int(quantity) > 0
        ]
        hour_allocations = _july23_allocate_hours(
            float(spec["hours"]),
            [quantity for _, quantity in segment_rows],
        )
        participants_text = _join_participants(spec["participants"])
        entries = []
        plans = {}
        for (row, quantity), operation_hours in zip(
            segment_rows,
            hour_allocations,
        ):
            entries.append({
                ...
                "processed_qty": int(quantity),
                "fire_qty": 0,
                "good_qty": int(quantity),
                "remaining_before": int(row["remaining_before"]),
                "unit_area_mm2": float(row["unit_area_mm2"]),
                "boy_mm": float(row["boy_mm"]),
                "en_mm": float(row["en_mm"]),
                "operation_hours": float(operation_hours),
                "participants_text": participants_text,
                "calculated_area_mm2": (
                    int(quantity) * float(row["unit_area_mm2"])
                ),
                "fire_note": "",
                "project_label": str(row["project_label"]),
            })
            plans[int(row["item_id"])] = {
                "combination_name": str(row["combination_name"]),
                "operations": _july23_dedupe_operations(
                    list(row["plan_operations"])
                ),
            }
    ...
    # POS bilinmeyen 6 ISS adedi yanlış bir POS'a yazılmaz. Kayıt, iki ekip
    # dilimine 4 + 2 adet olarak bölünür ve saatleri operasyon batchlerinde kalır.
    iss_first_qty = int(round(
        JULY23_ISS_UNKNOWN_TOTAL
        * JULY23_TEAM_SEGMENTS[0]["hours"]
        / sum(spec["hours"] for spec in JULY23_TEAM_SEGMENTS)
    ))
    iss_allocations = {
        str(JULY23_TEAM_SEGMENTS[0]["key"]): iss_first_qty,
        str(JULY23_TEAM_SEGMENTS[1]["key"]): (
            JULY23_ISS_UNKNOWN_TOTAL - iss_first_qty
        ),
    }
    ...
    return {
        ...
        "iss_allocations": iss_allocations,
    }


def _july23_save_plan(plan: dict) -> dict:
    """İki operasyon batchini ve POS'suz ISS notlarını tek seferde kaydeder."""
    already_saved, saved_value = _july23_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu 23 Temmuz ekip kaydı daha önce uygulandı. "
            f"Kayıt bilgisi: {saved_value or '-'}"
        )
    if not plan or not plan.get("segments"):
        raise ValueError("Kaydedilecek ekip planı bulunamadı.")
    ...
    try:
        for segment in plan["segments"]:
            ...
            if not entries:
                raise ValueError(
                    f"{segment['label']} için kaydedilecek POS işlemi bulunamadı."
                )
            marker = _july23_segment_marker(str(segment["key"]))
            team_text = _join_participants([
                segment["operator"],
                *segment["participants"],
            ])
            batch_id = save_operation_batch(
                {
                    "tarih": JULY23_TEAM_DATE,
                    "operator_ismi": str(segment["operator"]),
                    "calisma_tipi": "Mesaili",
                    "calisma_saati": float(segment["hours"]),
                    "neden": "",
                    "notlar": (
                        f"{marker} | Neo Vac paketleme + Heraklion POS10 boya | "
                        f"Ekip: {team_text}"
                    ),
                },
                entries,
            ...
            created_batches.append(int(batch_id))

            iss_qty = int(
                plan.get("iss_allocations", {}).get(str(segment["key"]), 0)
                or 0
            )
            if iss_qty > 0:
                other_id = save_other_work_log({
                    "tarih": JULY23_TEAM_DATE,
                    "operator_ismi": str(segment["operator"]),
                    "calisma_tipi": "Mesaili",
                    "calisma_saati": 0.0,
                    "is_aciklamasi": (
                        f"ISS boya · {iss_qty} adet · POS bilinmiyor · "
                        f"{segment['label']}"
                    ),
                    ...
                    "neden": "",
                    "notlar": (
                        f"{_july23_iss_marker(str(segment['key']))} | "
                        "Bu adetler 8 saatlik ekip boya kaydına dahildir; "
                        "POS bilinmediği için üretim ilerlemesine yazılmamıştır."
                    ),
                })
                created_other_logs.append(int(other_id))

        conn = get_db_connection()
        try:
            value = (
                "batch=" + ",".join(map(str, created_batches))
                + " | iss_other=" + ",".join(map(str, created_other_logs))
            )
            conn.execute(
                """
                INSERT INTO app_settings (setting_key, setting_value, updated_at)
        ...
        return {
            ...
            "other_log_ids": created_other_logs,
        }
    except Exception:
        for record_id in reversed(created_other_logs):
            try:
                delete_other_work_log(record_id)
            except Exception:
                pass
        for batch_id in reversed(created_batches):
            try:
                delete_operation_batch(batch_id)
            except Exception:
                pass
        raise


def render_july23_team_bulk_tool():
    """23/07/2026 ekip üretimini yönetici ekranından tek seferde kaydeder."""
    st.markdown("**23 Temmuz toplu boya / paketleme kaydı**")
    st.caption(
        "23/07/2026 · Mesaili · Toplam 8 saat · İlk 5 saat: Nezir Azim, "
        "Salih Kayar, Köksal Atıl ve Yaşettin Baysal · Son 3 saat: aynı "
        "ekibe Şükrü Atıl da katılır."
    )

    already_saved, saved_value = _july23_bulk_already_saved()
    if already_saved:
        st.success(
            "Bu toplu kayıt daha önce uygulandı; ikinci kez eklenmeyecek."
        )
        if saved_value:
            st.caption(saved_value)
        return

    neovac_options = _july23_project_options("neovac")
    heraklion_options = _july23_project_options("heraklion")
    iss_options = _july23_project_options("iss")

    if neovac_options.empty:
        st.error("Adında 'Neo Vac' geçen proje bulunamadı.")
        return
    if heraklion_options.empty:
        st.error("Adında 'Heraklion' geçen proje bulunamadı.")
        return
    if iss_options.empty:
        st.warning(
            "Adında 'ISS' geçen proje bulunamadı. 6 adetlik POS'suz ISS notu "
            "yine kaydedilebilir; yanlış bir POS'a dağıtılmaz."
        )

    def choose_oc(frame: pd.DataFrame, label: str, key: str) -> str:
        labels = [
            f"{row.project_name} · OC {row.oc_no}"
            for row in frame.itertuples(index=False)
        ]
        mapping = {
            display: str(row.oc_no)
            for display, row in zip(labels, frame.itertuples(index=False))
        }
        if len(labels) == 1:
            st.info(f"{label}: {labels[0]}")
    ...
    heraklion_oc = choose_oc(
        heraklion_options,
        "Heraklion projesi / OC",
        "july23_heraklion_oc",
    )

    plan = _july23_build_plan(neovac_oc, heraklion_oc)
    ...
    for segment in plan.get("segments", []):
        ...
        for entry in segment.get("entries", []):
            preview_rows.append({
                "Çalışma Dilimi": segment["label"],
                "Ekip": team_text,
                "Proje": entry.get("project_label", ""),
                "POS": entry["pos"],
                "Operasyon": entry["operation_name"],
                "Adet": int(entry["good_qty"]),
                "Saat": float(entry["operation_hours"]),
            })
        iss_qty = int(
            plan.get("iss_allocations", {}).get(str(segment["key"]), 0)
            or 0
    ...
    if not preview_rows:
        st.error("Kaydedilecek üretim satırı oluşturulamadı.")
        return

    preview = pd.DataFrame(preview_rows)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Ekip dilimi", int(preview["Çalışma Dilimi"].nunique()))
    m2.metric("Kayıt satırı", int(len(preview)))
    m3.metric("Toplam adet", int(preview["Adet"].sum()))
    m4.metric("Toplam süre", "8,0 saat")
    st.dataframe(preview, use_container_width=True, hide_index=True)

    st.caption(
        "Neo Vac'ta kalan bütün paketleme adetleri tamamlanır. Heraklion "
        "POS10'a tam 106 boya adedi yazılır. ISS'deki 6 boya adedinin POS'u "
        "bilinmediği için yanlış bir POS'a dağıtılmaz; 4 + 2 adet olarak "
        "POS'suz çalışma notunda saklanır. Fire 0'dır."
    )

    confirm = st.checkbox(
        "Bu 23 Temmuz mesaili ekip kaydını kontrol ettim ve kaydetmek istiyorum",
        key="july23_team_bulk_confirm",
    )
    if st.button(
        "23 Temmuz Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="july23_team_bulk_save",
    ):
        try:
            result = _july23_save_plan(plan)
            st.session_state["july23_team_bulk_success"] = (
                "23 Temmuz kayıtları tamamlandı. Operasyon kayıtları: "
                + ", ".join(map(str, result["batch_ids"]))
                + " · ISS POS'suz kayıtları: "
                + ", ".join(map(str, result["other_log_ids"]))
            )
            st.rerun()
        except Exception as exc:
            st.error(f"23 Temmuz toplu kayıt yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            bulk_success = st.session_state.pop(
                "july23_team_bulk_success",
                None,
            )
            if bulk_success:
                st.success(bulk_success)

            if st.toggle(
                "23 Temmuz toplu boya/paketleme kaydını aç",
                value=False,
                key="lazy_july23_team_bulk",
            ):
                render_july23_team_bulk_tool()

...
@st.cache_data(ttl=300, show_spinner=False)
def get_project_completed_output_daily(
    oc_no: str,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Aynı ürünü operasyon sayısı kadar çoğaltmadan günlük tamamlanan üretimi döndürür."""
    conn = get_db_connection()
    try:
        result = pd.read_sql_query(
            ...
            SELECT
                l.production_date AS tarih,
                SUM(l.produced_qty) AS tamamlanan_adet,
                SUM(l.produced_area_mm2) AS tamamlanan_alan_mm2
            FROM production_output_logs l
            INNER JOIN production_output_items i ON i.id = l.item_id
            WHERE i.oc_no = ?
              AND l.source_type = 'operation_tracking'
              AND l.production_date >= ?
              AND l.production_date <= ?
            GROUP BY l.production_date
            ORDER BY l.production_date
            """,
            conn,
            params=(str(oc_no), str(start_date), str(end_date)),
        )
    finally:
        conn.close()
    ...
    result["tarih_dt"] = pd.to_datetime(result["tarih"], errors="coerce")
    result["günlük_m2"] = (
        pd.to_numeric(result["tamamlanan_alan_mm2"], errors="coerce")
        .fillna(0)
        / 1_000_000
    ).round(1)
    result["sağlam_adet"] = (
        pd.to_numeric(result["tamamlanan_adet"], errors="coerce")
        .fillna(0)
        .astype(int)
    )
    return result[["tarih_dt", "günlük_m2", "sağlam_adet"]].dropna(
        subset=["tarih_dt"]
    )
...
def grafikler_page():
    ...
    # yeniden sayılmaz. Günlük tamamlanan alan, tüm planlı operasyonlardan geçen
    # ürünleri kaydeden production_output_logs tablosundan alınır.
    completed_daily = get_project_completed_output_daily(
        selected_oc,
    ...
    period_completed_m2 = float(daily["günlük_m2"].sum()) if not daily.empty else 0.0
    ...
    k3.metric("Seçilen dönemde tamamlanan", f"{period_completed_m2:.1f} m²")
    ...
    with tab_general:
        ...
        with left:
            _vega_line_chart(
                ...
                "tarih_dt",
                "günlük_m2",
                "Günlük tamamlanan üretim alanı",
                "Alan (m²)",
            )
        with right:
            _vega_pie_chart(
        ...
        st.caption(
            "Genel bakış m² hesabında aynı ürün yalnızca bir kez sayılır. "
            "Operasyon bazlı işlenen alanlar Operasyonlar sekmesinde ayrı gösterilir."
        )
        st.markdown("#### Günlük yönetim özeti")
        daily_display = daily.rename(columns={
            "tarih_dt": "Tarih",
            "günlük_m2": "Tamamlanan Alan (m²)",
            "sağlam_adet": "Tamamlanan Adet",
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Tamamlanan Alan (m²)": st.column_config.NumberColumn(format="%.1f"),

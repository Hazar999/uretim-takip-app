...
NEO_VAC_BULK_MARKER = "neo_vac_abkant_20260720_mucahit_sukru_fire398_v1"
NEO_VAC_BULK_DATE = "2026-07-20"
NEO_VAC_BULK_OPERATOR = "Mücahit Bozkurt"
NEO_VAC_BULK_COWORKER = "Şükrü Atıl"
NEO_VAC_BULK_HOURS = 9.0
NEO_VAC_BULK_FIRE = 398


def _allocate_integer_total(total: int, weights: list[int]) -> list[int]:
    """Bir tam sayı toplamını ağırlıklara orantılı ve toplamı bozmadan dağıtır."""
    ...
    if weight_sum <= 0:
        base = total // len(safe_weights)
        result = [base] * len(safe_weights)
...
def _allocate_decimal_total(total: float, weights: list[int], digits: int = 2) -> list[float]:
    """Saat gibi ondalıklı bir toplamı ağırlıklara dağıtır ve yuvarlama farkını kapatır."""
    if not weights:
        return []
    safe_weights = [max(float(value or 0), 0.0) for value in weights]
    weight_sum = sum(safe_weights)
    if weight_sum <= 0:
        values = [round(float(total) / len(weights), digits) for _ in weights]
    else:
        values = [round(float(total) * value / weight_sum, digits) for value in safe_weights]
    difference = round(float(total) - sum(values), digits)
    values[-1] = round(values[-1] + difference, digits)
    return values


def _neo_vac_bulk_already_saved() -> tuple[bool, str]:
    conn = get_db_connection()
    try:
        row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
...
def _get_neo_vac_project_items() -> pd.DataFrame:
    """Adında Neo Vac geçen üretim çıktısı satırlarını döndürür."""
    conn = get_db_connection()
    try:
        frame = pd.read_sql_query(
            """
    ...
    if frame.empty:
        return frame
    frame = frame[
        frame["project_name"].astype(str).map(_normalize_header).str.contains("neovac", na=False)
    ].copy()
    frame = frame[~frame["pos"].astype(str).isin(["POS10", "POS20"])].copy()
    return frame.reset_index(drop=True)


def _build_neo_vac_bulk_plan(selected_oc: str) -> pd.DataFrame:
    """Seçilen Neo Vac OC'sinin kalan abkant işlerini ve dağıtımları hazırlar."""
    items = _get_neo_vac_project_items()
    if items.empty:
        return items
    ...
    try:
        ...
        for _, item in items.iterrows():
            ...
            abkant_operation = next(
                (name for name in plan_operations if _operation_kind(name) == "abkant"),
                "",
            )
            ...
            abkant_logs = [row for row in log_rows if _operation_kind(str(row[0])) == "abkant"]
            if not abkant_operation and abkant_logs:
                abkant_operation = str(abkant_logs[-1][0])
            if not abkant_operation:
                abkant_operation = "abkant"

            existing_good = sum(int(row[1] or 0) for row in abkant_logs)
            existing_fire = sum(int(row[2] or 0) for row in abkant_logs)
            existing_long = sum(int(row[3] or 0) + int(row[4] or 0) for row in abkant_logs)
            existing_short = sum(int(row[5] or 0) + int(row[6] or 0) for row in abkant_logs)
            requested_qty = int(item["requested_qty"] or 0)
            remaining_good = max(requested_qty - existing_good, 0)
            if remaining_good <= 0:
                continue

            before_output = int(cur.execute(
                """
                SELECT COALESCE(SUM(produced_qty), 0)
                FROM production_output_logs
            ...
            rows.append({
                ...
                "operation_name": abkant_operation,
                "plan_operations": plan_operations,
                "existing_good": existing_good,
                "existing_fire": existing_fire,
    ...
    fire_allocations = _allocate_integer_total(
        NEO_VAC_BULK_FIRE,
        [row["remaining_good"] for row in rows],
    )
    for row, fire_qty in zip(rows, fire_allocations):
        row["fire_qty"] = int(fire_qty)
        row["processed_qty"] = int(row["remaining_good"] + fire_qty)
        target_edges = int(row["requested_qty"] + row["existing_fire"] + fire_qty)
        row["long_double_qty"] = max(target_edges - int(row["existing_long"]), 0)
        row["short_double_qty"] = max(target_edges - int(row["existing_short"]), 0)

    hour_allocations = _allocate_decimal_total(
        NEO_VAC_BULK_HOURS,
        [row["processed_qty"] for row in rows],
        digits=2,
    )
    for row, hours in zip(rows, hour_allocations):
        row["operation_hours"] = float(hours)

    return pd.DataFrame(rows)


def _save_neo_vac_bulk_plan(plan: pd.DataFrame) -> int:
    """Hazırlanan Neo Vac planını tek transaction ile kaydeder."""
    if plan is None or plan.empty:
        raise ValueError("Kaydedilecek Neo Vac abkant satırı bulunamadı.")

    conn = get_db_connection()
    try:
        cur = conn.cursor()
        existing_marker = cur.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_BULK_MARKER,),
        ).fetchone()
        if existing_marker is not None:
            raise ValueError(
                "Bu Neo Vac toplu kaydı daha önce uygulanmış. Aynı kayıt ikinci kez eklenmedi."
            )

        created_at = datetime.now().isoformat(timespec="seconds")
        note = (
            "Neo Vac kalan POS toplu abkant kaydı | POS10 ve POS20 hariç | "
            "uzun+kısa kenar çift büküm | 398 fire kalan adede orantılı dağıtıldı"
        )
        cur.execute(
            """
            INSERT INTO operation_batches (
        ...
        for _, row in plan.iterrows():
            fire_qty = int(row["fire_qty"] or 0)
            good_qty = int(row["remaining_good"] or 0)
            processed_qty = int(row["processed_qty"] or 0)
            fire_note = (
                "Fire etabı: abkant | Açıklama: Neo Vac toplu kaydındaki "
                "398 fire kalan adede orantılı dağıtıldı."
                if fire_qty > 0
                else ""
            )
            cur.execute(
                ...
                (
                    ...
                    fire_qty,
                    good_qty,
                    float(row["operation_hours"] or 0),
                    0,
                    "",
                    0.0,
                    0,
                    ...
                    NEO_VAC_BULK_COWORKER,
                    processed_qty,
                    1,
                    int(row["long_double_qty"] or 0),
                    int(row["short_double_qty"] or 0),
                    0,
                    int(row["long_double_qty"] or 0),
                    0,
                    int(row["short_double_qty"] or 0),
                    ...
                    0.0,
                    "Çift büküm",
                    good_qty * float(row["unit_area_mm2"] or 0),
                    fire_note,
                    created_at,
                ),
            )

        # Abkant bu POS'un son eksik operasyonuysa üretim çıktısı ilerlemesini de günceller.
        for _, row in plan.iterrows():
            item_id = int(row["item_id"])
            plan_operations = [str(value) for value in row["plan_operations"]]
            if not plan_operations:
                continue
            stage_totals = []
            for operation_name in plan_operations:
                operation_good = int(cur.execute(
                    """
                    SELECT COALESCE(SUM(good_qty), 0)
                    FROM operation_work_logs
                    WHERE item_id = ? AND operation_name = ?
                ...
                stage_totals.append(operation_good)
            after_output = min(stage_totals) if stage_totals else 0
            delta = max(after_output - int(row["before_output"] or 0), 0)
            if delta > 0:
                cur.execute(
                    """
                    INSERT INTO production_output_logs (
                    ...
                    (
                        ...
                        NEO_VAC_BULK_OPERATOR,
                        delta,
                        delta * float(row["unit_area_mm2"] or 0),
                        "Operasyonların tamamından geçen ürün",
                        created_at,
                        "operation_tracking",
                        batch_id,
        ...
        cur.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (NEO_VAC_BULK_MARKER, str(batch_id), created_at),
        )
        conn.commit()
        _clear_data_caches()
        return batch_id
...
def render_neo_vac_bulk_abkant_tool():
    """Neo Vac'ın kalan POS'larını tek tıkla belirtilen abkant kaydına dönüştürür."""
    st.markdown("**Neo Vac toplu abkant kaydı**")
    st.caption(
        "20/07/2026 · Mücahit Bozkurt · Şükrü Atıl ile beraber · "
        "uzun ve kısa kenar çift büküm · toplam 9 saat · toplam 398 fire"
    )

    already_saved, batch_value = _neo_vac_bulk_already_saved()
    if already_saved:
        st.success(
            "Bu toplu kayıt daha önce uygulandı. Aynı kayıt ikinci kez eklenmeyecek."
        )
        if batch_value:
            st.caption(f"Oluşturulan kayıt no: {batch_value}")
        return

    items = _get_neo_vac_project_items()
    if items.empty:
        st.error("Adında 'Neo Vac' geçen proje bulunamadı veya POS10/POS20 dışında POS yok.")
        return
    ...
    if len(labels) > 1:
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_bulk_oc_select",
        )
    else:
        st.info(selected_label)
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_bulk_plan(selected_oc)
    if plan.empty:
        st.success("POS10 ve POS20 dışındaki bütün Neo Vac POS'larında abkant adedi tamamlanmış.")
        return

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Kaydedilecek POS", int(len(plan)))
    m2.metric("Sağlam kalan", int(plan["remaining_good"].sum()))
    m3.metric("Dağıtılan fire", int(plan["fire_qty"].sum()))
    m4.metric("Toplam saat", f"{plan['operation_hours'].sum():.2f}")

    preview = plan[[
        "pos", "remaining_good", "fire_qty", "processed_qty",
        "long_double_qty", "short_double_qty", "operation_hours",
    ]].rename(columns={
        "pos": "POS",
        "remaining_good": "Sağlam Kalan",
        "fire_qty": "Fire",
        "processed_qty": "Toplam İşlenen",
        "long_double_qty": "Uzun Kenar Çift Büküm",
        "short_double_qty": "Kısa Kenar Çift Büküm",
        "operation_hours": "Saat",
    })
    st.dataframe(preview, use_container_width=True, hide_index=True)
    st.caption(
        "398 fire, her POS'un sağlam kalan adedine orantılı dağıtılır. "
        "POS10 ve POS20 hiçbir şekilde kayda dahil edilmez."
    )

    confirm = st.checkbox(
        "Bu toplu kaydı kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_bulk_confirm",
    )
    if st.button(
        "Neo Vac Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="neo_vac_bulk_save_button",
    ):
        try:
            batch_id = _save_neo_vac_bulk_plan(plan)
            st.session_state["neo_vac_bulk_success"] = (
                f"Neo Vac toplu abkant kaydı tamamlandı. Kayıt no: {batch_id}."
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu kayıt yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu abkant kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_abkant",
                help="Neo Vac kalan POS'larını 20/07/2026 tarihli tek bir abkant kaydı olarak hazırlar.",
            ):
                success_message = st.session_state.pop("neo_vac_bulk_success", None)
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_abkant_tool()

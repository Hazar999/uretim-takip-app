                ...
                for operation in selected_ops:
                    ...
                    if operation_kind == "laser":
                        ...
                        for lot_index in range(1, lot_count + 1):
                            ...
                            laser5, laser6, laser7 = st.columns([1.15, 1.15, 0.9])
                            with laser5:
                                st.selectbox(
                                    "Malzeme",
                            ...
                            with laser7:
                                st.markdown("**Yeni satır**")
                                add_col, remove_col = st.columns(2)
                                with add_col:
                                    if st.button(
                                        "＋",
                                        key=(
                                            f"add_laser_lot_{item_id}_"
                                            f"{normalized_operation}_{version}_{lot_index}"
                                        ),
                                        help="Yeni kalınlık / plaka satırı ekle",
                                        use_container_width=True,
                                        type="primary",
                                        disabled=(lot_index != lot_count),
                                    ):
                                ...
                                with remove_col:
                                    if st.button(
                                        "−",
                                        key=(
                                            f"remove_laser_lot_{item_id}_"
                                            f"{normalized_operation}_{version}_{lot_index}"
                                        ),
                                        help="Son kalınlık / plaka satırını sil",
                                        use_container_width=True,
                                        disabled=(lot_count <= 1 or lot_index != lot_count),
                                    ):
                                        removed_index = lot_count
                                        removed_suffix = f"laserlot_{removed_index}_{version}"
                                        removable_keys = (
                            ...
                            st.caption(
                                f"Grup verimi: {adet_saat:.2f} adet/saat · "
                                f"{m2_saat:.3f} m²/saat · "
                                f"{adet_plaka:.2f} adet/plaka"
                            )

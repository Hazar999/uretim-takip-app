...
def save_production_output_entries(
    ...
    selected = entries[pd.to_numeric(entries["Bu İşlemde Üretilen"], errors="coerce").fillna(0) > 0].copy()
    if selected.empty:
        return 0
    ...
    try:
        ...
        for _, row in selected.iterrows():
            quantity = int(row["Bu İşlemde Üretilen"])
            cur.execute(
...
def main():
    ...
    st.caption("İşçi çalışma süresi, POS, kombinasyon, adet ve fire bilgilerini kaydeder. — Sürüm 2.2 ÇOKLU DOSYA + PROJE SEÇİMİ")
    if is_manager:
        st.success("Yönetici görünümü: Tüm kayıtlar, grafikler, raporlar ve silme işlemleri açık.")
    else:
        st.info("Çalışan görünümü: Yeni kayıt, üretime devam ve üretim çıktısı ekranları açık.")

    worker_pages = ["Yeni Kayıt", "Üretime Devam Et", "Üretim Çıktısı"]
    manager_pages = [
        "Yeni Kayıt",
        "Üretime Devam Et",
        "Üretim Çıktısı",
    ...
    with st.sidebar:
        st.header("Menü")
        st.info("Sürüm 2.2 ÇOKLU DOSYA + PROJE SEÇİMİ")
        manager_login_panel()
        st.divider()
    ...
    elif page == "Üretim Çıktısı":
        uretim_ciktisi_page(is_manager)
...
def uretim_ciktisi_page(is_manager: bool):
    ...
    st.caption(
        "Yönetici aynı anda birden fazla PDF veya Excel üretim çıktısını sisteme aktarabilir. "
        "Operatör proje/OC seçer; POS, istenen adet, Boy ve En otomatik gelir."
    )
    ...
    if is_manager:
        with st.expander("Üretim çıktısı dosyalarını içe aktar", expanded=True):
    ...
    project_choices = (
        all_items[["project_name", "oc_no"]]
        .drop_duplicates()
        .sort_values(["project_name", "oc_no"])
        .reset_index(drop=True)
    )
    project_labels = project_choices.apply(
        lambda row: f"{row['project_name']} — OC {row['oc_no']}",
        axis=1,
    ).tolist()
    label_to_oc = dict(zip(project_labels, project_choices["oc_no"].astype(str)))

    selected_project_label = st.selectbox(
        "Proje seç",
        project_labels,
        key="production_output_project",
    )
    selected_oc = label_to_oc[selected_project_label]
    items = get_production_output_summary(selected_oc)

    selected_project_name = str(items["project_name"].iloc[0])
    st.info(f"Seçilen proje: **{selected_project_name}** · OC: **{selected_oc}**")
    ...
    if editable.empty:
        ...
        editor = pd.DataFrame({
            ...
            "Proje": editable["project_name"].astype(str),
        ...
        edited = st.data_editor(
            ...
            hide_index=True,
            disabled=[
                "Proje", "OC", "POS", "İstenen Adet", "Boy (mm)", "En (mm)",
                "Birim Alan (mm²)", "Önceden Üretilen", "Kalan",
            ],
            column_config={
                "item_id": None,
                "Bu İşlemde Üretilen": st.column_config.NumberColumn(
                    "Bu İşlemde Üretilen",
                    min_value=0,
                    step=1,
                    required=True,
        ...
        e1, e2 = st.columns(2)
        e1.metric("Bu işlemde girilen adet", int(entered.sum()))
        e2.metric(
            "Bu işlemde üretilecek alan",

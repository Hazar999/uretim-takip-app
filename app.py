        ...
        for entry in entries:
            ...
            cur.execute(
                ...
                (
                    ...
                    float(entry.get("calculated_area_mm2", float(good_qty) * float(entry.get("unit_area_mm2", 0) or 0)) or 0),
                    entry.get("fire_note", ""),
                    created_at,
                ),
            )
                ...
                for operation in selected_ops:
                    ...
                    if operation_kind == "laser":
                        ...
                        st.caption(
                            ...
                            "Usta yalnızca uzun ve kısa kenar adetlerini girer; takım ve net m² otomatik hesaplanır."
                        )
                        ...
                        for lot_index in range(1, lot_count + 1):
                            ...
                            material_col, thickness_col, hours_col, fire_col = st.columns(4)
                            with material_col:
                                st.selectbox(
                                    "Malzeme",
                            ...
                            result1.metric("Tamamlanan takım", f"{completed_teams} takım")
                            result2.metric("Sağlam takım", f"{good_teams} takım")
                            result3.metric("Üretilen net panel alanı", f"{net_area_m2:.2f} m²")

...
NEO_VAC_LASER_BULK_MARKER = "neo_vac_laser_20260716_18_20_aykut_v1"
NEO_VAC_LASER_OPERATOR = "Aykut Akili"
NEO_VAC_LASER_DAY_SPECS = [
    {
        "date": "2026-07-16",
        "positions": ["POS10", "POS20", "POS30", "POS40"],
        "hours": 6.0,
        "plate_qty": 5,
        "material_type": "Çelik",
        "thickness_mm": 3.0,
    ...
    {
        "date": "2026-07-18",
        "positions": ["POS30", "POS190", "POS250", "POS260", "POS270", "POS300"],
        "hours": 5.0,
        "plate_qty": 0,
        "material_type": "",
        "thickness_mm": 0.0,
...
def _bulk_allocate_integer(total: int, weights: list[int]) -> list[int]:
    """Tam sayı toplamını ağırlıklara göre dağıtır; dağıtılan toplam değişmez."""
    if not weights:
        return []
    total = max(int(total or 0), 0)
    safe_weights = [max(int(value or 0), 0) for value in weights]
    weight_sum = sum(safe_weights)
    if total == 0:
        return [0] * len(safe_weights)
    if weight_sum <= 0:
        result = [0] * len(safe_weights)
        for index in range(total):
            result[index % len(result)] += 1
        return result

    raw = [total * value / weight_sum for value in safe_weights]
    result = [int(value) for value in raw]
    remainder = total - sum(result)
    order = sorted(
        range(len(raw)),
        key=lambda index: (raw[index] - result[index], safe_weights[index]),
        reverse=True,
    )
...
def _bulk_allocate_hours(total: float, weights: list[int]) -> list[float]:
    """Günlük toplam saati POS adetlerine orantılı dağıtır."""
    if not weights:
        return []
    ...
    if weight_sum <= 0:
        ...
        result = [round(float(total) * value / weight_sum, 2) for value in safe_weights]
    result[-1] = round(result[-1] + round(float(total) - sum(result), 2), 2)
    return result


def _neo_vac_laser_bulk_already_saved() -> tuple[bool, str]:
    conn = get_db_connection()
    try:
        row = conn.execute(
            "SELECT setting_value FROM app_settings WHERE setting_key = ?",
            (NEO_VAC_LASER_BULK_MARKER,),
        ).fetchone()
        return (row is not None, str(row[0]) if row else "")
    finally:
        conn.close()


def _get_neo_vac_laser_items() -> pd.DataFrame:
    """Adında Neo Vac geçen üretim çıktısı satırlarını ve lazer planını getirir."""
    conn = get_db_connection()
    try:
        ...
        for _, item in items.iterrows():
            ...
            laser_logs = [
                ...
                for operation_name, good_qty in laser_log_rows
                if _operation_kind(str(operation_name)) == "laser"
            ]
            laser_operation = next(
                (name for name in plan_operations if _operation_kind(name) == "laser"),
                "",
            )
            ...
            existing_laser_good = sum(value for _, value in laser_logs)
            row = item.to_dict()
            row["plan_operations"] = plan_operations
            row["laser_operation"] = laser_operation
            row["existing_laser_good"] = existing_laser_good
        ...
        return result.sort_values(["oc_no", "_pos_no"]).drop(columns=["_pos_no"])
    finally:
        conn.close()


def _build_neo_vac_laser_bulk_plan(selected_oc: str) -> dict:
    """16, 18 ve 20 Temmuz Neo Vac lazer kayıtlarını güvenli biçimde hazırlar."""
    items = _get_neo_vac_laser_items()
    if items.empty:
        return {"days": [], "warnings": [], "missing": []}
    ...
    virtual_good = {
        int(row.item_id): int(row.existing_laser_good or 0)
        for row in items.itertuples(index=False)
    }
    ...
    for spec in NEO_VAC_LASER_DAY_SPECS:
        if spec["positions"] is None:
            candidate_rows = list(items.itertuples(index=False))
        else:
            candidate_rows = []
            for pos in spec["positions"]:
                row = item_by_pos.get(pos)
                if row is None:
                    missing.append(f"{spec['date']} · {pos} proje çıktısında bulunamadı.")
                    continue
        ...
        for item in candidate_rows:
            ...
            completed_before = max(int(virtual_good.get(item_id, 0)), 0)
            remaining_qty = max(requested_qty - completed_before, 0)
            if remaining_qty <= 0:
                if spec["positions"] is not None:
                    warnings.append(
                        f"{spec['date']} · {item.pos}: önceki/mevcut lazer kaydında "
                        "tamamlandığı için tekrar yazılmayacak."
                    )
                continue

            plan_operations = [str(value) for value in (item.plan_operations or [])]
            laser_operation = str(item.laser_operation or "laser")
            if not any(_operation_kind(name) == "laser" for name in plan_operations):
                plan_operations = [laser_operation] + plan_operations
            ...
            virtual_good[item_id] = completed_before + remaining_qty

        quantities = [int(row["good_qty"]) for row in rows]
        hour_allocations = _bulk_allocate_hours(float(spec["hours"]), quantities)
        plate_allocations = _bulk_allocate_integer(int(spec["plate_qty"]), quantities)

        entries = []
        plans = {}
        for row, operation_hours, plate_qty in zip(
            rows, hour_allocations, plate_allocations
        ):
    ...
    return {"days": day_plans, "warnings": warnings, "missing": missing}


def _neo_vac_laser_day_existing_batch(day_marker: str) -> int | None:
    conn = get_db_connection()
    try:
        row = conn.execute(
            """
...
def _save_neo_vac_laser_bulk_plan(plan: dict) -> list[int]:
    """Hazırlanan üç günlük lazer planını kaydeder ve tekrar kaydı engeller."""
    if not plan or not plan.get("days"):
        raise ValueError("Kaydedilecek Neo Vac lazer günü bulunamadı.")

    already_saved, saved_value = _neo_vac_laser_bulk_already_saved()
    if already_saved:
        raise ValueError(
            "Bu Neo Vac toplu lazer kaydı daha önce uygulandı. "
    ...
    for day in plan["days"]:
        ...
        if not entries:
            continue
        day_marker = f"{NEO_VAC_LASER_BULK_MARKER}:{day['date']}"
        existing_batch = _neo_vac_laser_day_existing_batch(day_marker)
        if existing_batch is not None:
            batch_ids.append(existing_batch)
            continue

        known_detail = (
            "Çelik · 3 mm · toplam 5 plaka"
            if day["date"] == "2026-07-16"
            else "malzeme, kalınlık ve plaka bilgisi bilinmiyor"
        )
        note = (
            f"{day_marker} | Neo Vac toplu lazer geçmiş kaydı | {known_detail}"
        )
        batch_id = save_operation_batch(
            {
                "tarih": str(day["date"]),
                "operator_ismi": NEO_VAC_LASER_OPERATOR,
    ...
    if not batch_ids:
        raise ValueError("Kalan lazer adedi bulunmadığı için kayıt oluşturulmadı.")
    ...
    try:
        ...
        conn.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
            """,
            (NEO_VAC_LASER_BULK_MARKER, ", ".join(map(str, batch_ids)), now),
        )
        conn.commit()
        _clear_data_caches()
    finally:
...
def render_neo_vac_bulk_laser_tool():
    """Neo Vac lazer geçmişini üç günlük tek seferlik kayıt aracıyla ekler."""
    st.markdown("**Neo Vac toplu lazer kaydı**")
    st.caption(
        "Operatör: Aykut Akili · 16/07/2026: 6 saat, Çelik 3 mm, toplam 5 plaka · "
        "18/07/2026: 5 saat · 20/07/2026: 6 saat"
    )

    already_saved, batch_value = _neo_vac_laser_bulk_already_saved()
    if already_saved:
    ...
    if len(labels) == 1:
        ...
        selected_label = st.selectbox(
            "Neo Vac projesi / OC",
            labels,
            key="neo_vac_laser_bulk_oc_select",
        )
    selected_oc = label_to_oc[selected_label]

    plan = _build_neo_vac_laser_bulk_plan(selected_oc)
    if plan.get("missing"):
        for message in plan["missing"]:
            st.error(message)
    if plan.get("warnings"):
        for message in dict.fromkeys(plan["warnings"]):
            st.warning(message)
    ...
    for day in plan.get("days", []):
        for entry in day.get("entries", []):
            preview_rows.append({
                ...
                "Plaka": int(entry["laser_plate_qty"]),
                "Malzeme": entry["material_type"] or "—",
                "Kalınlık (mm)": entry["thickness_mm"] or "—",
                "Saat": float(entry["operation_hours"]),
            })

    if not preview_rows:
        st.success("Bu bilgiler için kaydedilecek kalan lazer adedi bulunamadı.")
        return
    ...
    m3.metric("Toplam adet", int(preview["Tamamlanacak Adet"].sum()))
    m4.metric("Toplam saat", f"{preview['Saat'].sum():.1f}")
    st.dataframe(preview, use_container_width=True, hide_index=True)
    st.caption(
        "16 Temmuz'daki 5 plaka POS'lara dağıtılır ancak toplamı yine 5 kalır. "
        "18 ve 20 Temmuz kayıtlarında bilinmeyen malzeme, kalınlık ve plaka alanları "
        "veritabanında boş/0 bırakılır. 20 Temmuz'da önceki iki günden sonra kalan bütün POS'lar tamamlanır."
    )

    confirm = st.checkbox(
        "Bu üç günlük lazer kaydını kontrol ettim ve kaydetmek istiyorum",
        key="neo_vac_laser_bulk_confirm",
    )
    if st.button(
        "Neo Vac Lazer Kayıtlarını Tek Seferde Kaydet",
        type="primary",
        use_container_width=True,
        disabled=not confirm,
        key="neo_vac_laser_bulk_save_button",
    ):
        try:
            batch_ids = _save_neo_vac_laser_bulk_plan(plan)
            st.session_state["neo_vac_laser_bulk_success"] = (
                "Neo Vac toplu lazer kayıtları tamamlandı. Kayıt numaraları: "
                + ", ".join(map(str, batch_ids))
            )
            st.rerun()
        except Exception as exc:
            st.error(f"Neo Vac toplu lazer kaydı yapılamadı: {exc}")
...
def main():
    ...
    with st.sidebar:
        ...
        if is_manager:
            ...
            if st.toggle(
                "Neo Vac toplu lazer kaydını aç",
                value=False,
                key="lazy_neo_vac_bulk_laser",
                help="16, 18 ve 20 Temmuz 2026 Neo Vac lazer geçmişini tek seferde kaydeder.",
            ):
                success_message = st.session_state.pop(
                    "neo_vac_laser_bulk_success", None
                )
                if success_message:
                    st.success(success_message)
                render_neo_vac_bulk_laser_tool()

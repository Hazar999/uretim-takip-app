...
def get_db_connection():
    """
    Bulutta kalıcı Turso/libSQL bağlantısı, yerelde SQLite bağlantısı döndürür.
...
def render_manager_backup_download():
    """Yöneticiye tek tıkla tüm veritabanını Excel olarak indirme imkânı verir."""
    try:
        backup_bytes = build_full_database_backup_excel()
    except Exception as exc:
        st.error(f"Yedek hazırlanamadı: {exc}")
        return

    st.download_button(
        "⬇️ Tam Veritabanı Yedeğini İndir",
        data=backup_bytes,
        file_name=f"durlum_uretim_yedek_{date.today().isoformat()}.xlsx",
        mime=(
        ...
        help="Tüm tabloları tek Excel dosyasına indirir.",
    )
...
def main():
    ...
    st.caption("Sürüm 2.16.0 · KALICI BULUT VERİTABANI / HER ETAPTA m²")
    st.markdown(
        """
        ...
        ✓ AKTİF SÜRÜM 2.16.0 — KALICI VERİTABANI · HER ETAPTA m² · VERİ KAYBI KORUMASI
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-version">
                Sürüm 2.16.0 · KALICI DB / m² DASHBOARD
            </div>
            """,
            unsafe_allow_html=True,
        )

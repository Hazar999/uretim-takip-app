...
{good_teams} × {unit_area_m2:.2f} = **{net_area_m2:.2f} m²**"""
                        ...
                        if operation_kind == "abkant":
                            ...
                            with team1:
                                st.number_input(
                                    "Üretilen takım sayısı", min_value=0, step=1,
                                    key=abkant_team_qty_key,
                                )
                            with team2:
                                st.number_input(
                                    "1 adet için gereken takım", min_value=1, step=1,
...
def render_period_operation_pies(history: pd.DataFrame, other_logs: pd.DataFrame):
    ...
    if not filtered.empty:
        ...
        project_labor["Toplam m²"] = pd.to_numeric(project_labor["toplam_alan_mm2"], errors="coerce").fillna(0) / 1_000_000
        ...
        project_labor["m² İlerleme %"] = (
            project_labor["Üretilen m²"] / project_labor["Toplam m²"].replace(0, pd.NA) * 100
        ).fillna(0)
        display_project_labor = project_labor[[
            "proje_adi", "OC", "Toplam m²", "Üretilen m²",
            "Toplam İşçilik (saat)", "Saat / m²", "m² / Saat",
            "Adet İlerleme %", "m² İlerleme %",
        ]].rename(columns={"proje_adi": "Proje"})
        numeric_cols = ["Toplam m²", "Üretilen m²", "Toplam İşçilik (saat)", "Saat / m²", "m² / Saat", "Adet İlerleme %", "m² İlerleme %"]
        display_project_labor[numeric_cols] = display_project_labor[numeric_cols].round(2)
        st.markdown("#### Proje bazlı m² ve işçilik")
